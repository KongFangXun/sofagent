...
sofagent 的三个核心设计选择，在独立研究中得到了印证：

| 我们的设计 | 外部验证 | 来源 |
|------|------|------|
| 结构化备忘录式记忆（think.md + task/logs + 文件系统）| "结构化行为回溯备忘录 + 精准 Tag 路由 + SQL，搞定 95% 以上场景。真正落地的记忆系统应该是极其清爽的。" | Agent 全局记忆系统设计批判 |
| 宪法内联 + 子 Skill 按需加载的分层架构 | Skill Reducer 实证：核心规则仅占技能 38.5%，分层架构实现 token 降 39%、质量升 2.8%。"结构感知是技能压缩的关键。" | Skill Reducer（港科大/清华/浙工大） |
| 闭环反思 + Loop Agent | Self Harness 四层模型（执行→留痕→提案→晋升验证），在 Terminal Bench 2.0 上三个模型通过率提升 14-21%。"Agent 可以提议修改，但不能自己批准。" | Self Harness（上海 AI Lab） |

这些不是我们引用外部研究来证明自己正确——而是两个完全独立的团队，从不同起点出发，得出了重叠度极高的结论。这印证了 sofagent 的架构直觉是结构驱动的，不是巧合。
...
闭环复盘的本质是让执行任务的同一个 Agent 对自己打分——评估者和被评估者是同一个人。上海 AI Lab 的 Self Harness 论文给出了量化证据：**Agent 可以提议修改，但不能自己批准**。一旦自评，Agent 会收敛于「让验证变容易」而非「让结果变好」。

v0.62.2 已按平台分级解决：OpenClaw 使用 `session.spawn` 创建独立子 Agent 完成评审（只读 task/logs，不继承执行上下文）；非 OpenClaw 平台退化到证据驱动评审（以 task/logs 文件为唯一依据，禁止凭记忆补充）。细节见 `loop-agent.md` closure 模式。
