...
> 一个完全不懂代码的产品经理，在设计 Agent 纪律层时都想了些什么。
>
...
> Anthropic 团队每次新模型发布，会读系统提示，移除不再需要的部分——如模型升级后自动具备 to-do list 能力，即删手动注入的 to-do list 功能。

sofagent 同理：每次大模型发布后，检查 SKILL.md 10 条铁律和 engine.md 编排规则中，哪些已被模型内置能力覆盖。删掉冗余规则。**原则：模型自己能做的，sofagent 不重复管。**

这条原则和 Logan Kilpatrick（Google DeepMind）2026-06-23 的判断呼应——"每一行外部脚手架，都是对模型无能的妥协"。sofagent 的差异化在纪律层（先读后写/验证再干/谨慎修改），不在约束层（模型自己就会拒绝的）。模型越强，sofagent 应该越简单——不是越加越多。
...
对 sofagent loop-check closure 模式的指导：当前 closure 是「检查对错」（做完后评九维分数）。Bun 案例指向「假设错误」模式——闭环时不是问「这次做得怎么样」，是问「这次最可能在哪出了问题，你怎么证明没有」。这是 v0.86+ loop-check 反驳层强化的设计参考。
...
| 架构层 | FDE 场景需求 | sofagent 交付物 | 状态 |
|--------|-------------|----------------|:----:|
| **纪律层** | Agent 行为需要边界（先读后写/验证再干/谨慎修改） | 4 底线 + 10 铁律 + 三层加载链 | ✅ v0.85 |
| **执行层** | 长任务需要编排引擎和过程监控 | daemon + 任务编排 + Loop 检查 | ✅ v0.81（v0.91 TS 化） |
| **安全层** | 外部 Skill 进入需要安全审查 + 数据合规声明 | skill-safety-check.sh + 数据存储声明 | ✅ v0.90 |
| **审计层** | 提交时代码审查，不依赖 Agent 配合 | sofagent-audit（看 git diff） | 🔨 v0.92 |
| **协同层** | 团队多设备的 Agent 经验共享 | 多设备信号共享网络 | 📋 v2.x |

**设计原则**：sofagent 是 FDE 场景的纪律底座，不是 FDE 引擎。FDE 引擎跑任务（n8n / Dify / 企业自建编排），sofagent 管跑任务的 Agent 的行为边界。两者是工具和工具箱的关系——工具箱不替你干活，但保证工具不会伤手。
...
> 2026-06-23，两份独立外部评审（GLM-5.2 + DeepSeek V4 Pro，GitHub 大神 + 技术 CTO 双视角）带来了 v0.85 的战略校准。完整分析见 [v0.85 开发日志](./docs/changelog/v0.85.md)，此处只记录影响架构设计的关键洞察。

### 洞察 1：三层差异化——约束层 / 纪律层 / 持久化层

v0.84 的 A/B 数据画出了 sofagent 三层能力的差异化强度：
...
#### 概念分层：解决架构概念过载（DeepSeek 评审洞察 #3）

DeepSeek 指出：新贡献者看到宪法 + 铁律 + 编排 + daemon + 断路器 + 步数闸 + 渐进减薄 + 四级编排……会被吓退。v0.85 的解决方案是把 sofagent 的所有概念拆成两列——**纪律层核心**和**治理层增强**：

| 纪律层核心（所有平台生效） | 治理层增强（OpenClaw 专属 / 按需） |
|------|------|
| 4 底线（安全红线） | Hook 自动注入（加载链第 2/3 层） |
| 10 铁律（工程纪律细则） | 断路器（circuit breaker 三态） |
| 三层加载链架构 | 步数闸（MAX + GRACE 两段式） |
| SKILL.md 宪法内联（契约层） | session.spawn（子 Agent 隔离） |
| think.md 反思区（错题本） | ao compose 编排引擎（需 npm） |
| rules.md 用户规则（最高优先级） | 渐进减薄（orchestrator/ 目录） |
| Loop Agent 三节点顾问模式 | Skill 信任四级（评分驱动） |
| task/logs 审计日志 | tree-loading（树形按需加载） |
| 验证闸门（verify-evidence.sh） | 幂等检查 / 死循环检测 |

**为什么这么分**：左侧是「Agent 读了 MD 文件就生效」的软约束——不需要 Hook、不需要 session、不需要 npm 包，任何平台都可用。右侧是「依赖平台机制才能生效」的治理加固——需要 OpenClaw 的 Hook 架构或外部依赖。

**这解释了几个设计决策**：
- `--no-ao` 为什么升为推荐默认——编排引擎在右侧（增强），不在左侧（核心）
- 非 OpenClaw 平台价值为什么约 30%——右侧全失效，左侧靠 Agent 自觉（执行率受上下文影响）
- v0.85 ROADMAP 为什么砍企业级——右侧的能力都没验证过增量，加再多也是沙子上盖楼

### 洞察 2：运行时治理 vs 提交时审计

当前架构是运行时治理——Agent 跑任务时约束其行为，依赖 Agent 配合。v0.84 数据证明：不配合就全失效（CLI 0/16），甚至可能反向效果（宪法可被篡改，Hermes 2/2 复现）。

两份评审都指向同一方向：**从预防转向检测**。提交时审计（sofagent-audit）不依赖 Agent 配合——它看的是已经发生的 git diff，Agent 没法绕过。
...
| CTO 需求匹配 | 中（过程管控） | 高（结果检测） |

两者互补：运行时治理减少问题发生，提交时审计兜底检测漏网之鱼。v0.9 资源分配从"100% 运行时"调整为"60% 运行时 + 40% 提交时"。
