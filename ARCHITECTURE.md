...
**现有防御**：冷启动保护（前 5 次只记录不判断）+ LLM 自评权重 ×0.5。但这些只是缓冲层，不是解决方案。根治需要独立验证环（见 ROADMAP v0.9 验证门控 + v1.x 外部评估器）。
...
当前反思区已有 `[LLM自评 ×0.5]` 降权。笔记「Skill 自进化」建议加两种新标记：`[多轨迹归纳]`（至少 3 条同类轨迹提取的模式）和 `[外部验证]`（通过独立评估器验证的结论），分别对应不同的置信度权重。
