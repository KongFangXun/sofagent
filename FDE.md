# FDE.md · Forward Deployed Engineer 部署手册

> **这是什么**：FDE 工程师（或企业自己的 CIO/网管/工程师）的操作手册。读它 → 帮企业梳理 workflow → 识别 AI 节点 → 生成部署方案。Agent 全程辅助——你负责和人聊，Agent 负责记录、分析、出方案。
>
> **谁读**：人读（FDE 工程师 / 企业 IT）+ Agent 读（sofagent 检测到 FDE 场景时自动加载）。
>
> **范围声明**：sofagent 是 FDE 的**纪律底座**，不是**运维底座**。覆盖 FDE 问题空间约 20%（Agent 质量层：代码纪律 + 审计 + 经验沉淀），运维层（监控/告警/重启/日志轮转，约 80%）不在范围内。
>
> **为什么 FDE 重要**：组织推进 AI 化时，真正的痛点从来不是缺少 AI 聊天入口或文档库，而是缺少一套能把历史逻辑、协作关联方、活的接口人、共享上下文和组织记忆串联起来的**公共协作环境**。FDE 部署的不是单个 AI 工具——是整个组织的 Agent 协作环境。sofagent 是这个协作环境的纪律底座（参考 Lance Martin「Org Level Agent Harness」定义）。

---

## 编排引擎（engage.md）

> FDE 部署场景专用。当部署流程进行到 🔄/⚡ 节点执行时，编排引擎根据节点复杂度自动选择「拆」或「不拆」。

### 定位

编排引擎（`engage.md`）：
- **什么时候点火**：FDE workflow 中的 🔄/⚡ 节点触发——不是通用 🔴 复杂任务
- **两档拆解**（拆 vs. 不拆）
- **AO Compose**：拆解时调 `ao compose` 一次性生成 DAG，不渐进减薄
- **Agent 模板**：引用 AO Compose 自带角色模板库（不自定义）

### think.md 反馈回路

每次节点执行完毕后，编排引擎自动向 `{SOFAGENT_DATA}/think.md` 写入反思记录：
- 拆解策略（拆/不拆）是否恰当
- 拆解粒度（偏粗/刚好/偏细）
- 角色匹配是否正确

**下次同一节点任务点火时**，编排引擎先读 think.md 历史记录，自动调整策略：
- 上次拆了但实际单步跑完 → 本次不拆
- 上次不拆但实际需要多步 → 本次拆
- 上次角色匹配错误 → 按 think.md 建议调整

这个反馈回路让编排引擎随节点执行次数增加而自动优化——第一次拆最细，越跑越精准。

### 相关文件

| 文件 | 作用 | 何时加载 |
|------|------|---------|
| `sofagent/engage.md` | 编排引擎主体（点火/拆解/执行/闭环） | FDE 节点执行前 |
| `sofagent/engage-fde.md` | FDE 场景引导逻辑（主动检测+引导十步） | 检测到 FDE 场景时 |
| `{SOFAGENT_DATA}/think.md` | 反馈回路记忆 | 每次节点执行后写入，下次执行前读取 |
| `{SOFAGENT_DATA}/orchestrator/` | 拆解结果缓存 | 同一节点复用历史 DAG |

---

## §一 进场：确定场景

部署专家和你一起进入了企业现场。你的工作是确保部署专家搞清楚这家企业的基本情况。

### 你需要向部署专家确认这些

主动问部署专家，确认以下信息已拿到：

- 企业叫什么、做什么行业、多少人
- 哪个部门/团队参与本次部署——是整个公司还是某一个业务线
- 关键岗位有哪些、每个岗位主要产出什么

### 注意

- 提醒部署专家：这一轮不要聊 AI、不要聊工具——只聊业务
- 你负责把这些信息记录成「企业画像」：一段话 + 部门/岗位清单
- ⚠️ 很多企业的真实状态是：推进一个需求时，员工像在「一个黑色布满炸弹的房间里走路」——不知道该找谁、不知道该信谁、也不知道哪段历史规则会突然炸回来。FDE 的工作就是摸清这些隐性关联，让 Agent 接管时不会踩雷

---

## §二 盘点平台与工具

接下来摸清企业的技术环境——这直接决定后面怎么部署。

### 你需要向部署专家确认这些

- 协同平台：用什么？（钉钉 / 飞书 / 企业微信 / Slack / Teams……）
- 业务系统：ERP / CRM / 自研系统 / 纯 Excel？
- 数据可达性：这些系统有 API 吗？能导出数据吗？数据格式是什么？
- 知识库平台：企业已有的知识库放在哪？（钉钉知识库 / 飞书文档 / 语雀 / Confluence / 共享文件夹……）
  → 这个答案很重要：后续部署时知识库就落在这里

### 你需要产出的

「技术环境清单」——把从部署专家那里听到的所有平台、系统、数据源整理成一张清单。

> ⚠️ §一和 §二都是先做、不关门。后续深挖时部署专家说「哦他们还有 XX 忘了说」→ 直接补进对应的清单，不要重新开始。

---

## §三 工作流深挖 ← 循环在这里

引导部署专家逐个岗位、逐段流程地深挖，直到每个节点的五要素全部填满。每次聚焦一个人、一段流程。

### 五要素

你负责追问部署专家直到每一项都不空：

1. **输入**：这个节点接收什么信息/数据？
2. **输出**：这个节点产出什么？
3. **负责人**：谁在做这件事？
4. **耗时**：每次做花多长时间？每天/每周做几次？
5. **最痛的地方**：这个节点最大的痛点是什么？（太慢？太繁琐？容易出错？没人想做？）

### 追问话术

对每个岗位，用以下问题引导部署专家：

- 「这个岗位每天第一件事做什么？」
- 「做完上一步，下一步是什么？」
- 「这一步输入是什么？谁给你的？」
- 「这一步输出是什么？交给谁？」
- 「这一步花多长时间？最烦人的地方是什么？」

当五要素全部填满 → 换下一个岗位。所有关键岗位（从 §一 的企业画像来）的五要素全部填满 → 结束循环，进入 §四。中途发现遗漏的岗位或流程 → 回头补，补完继续。

### 你需要产出的

「完整工作流节点图」——每个节点有完整五要素，按岗位分组，按流程顺序排列。

---

## §四 识别 AI 节点

工作流摸清了，接下来你要帮部署专家判断：哪些节点可以被 AI 接管。

### 决策矩阵

对 §三 产出的每一个工作流节点，逐一过这个判定：

| 类别 | 特征 | 例子 |
|------|------|------|
| 🔄 **自动执行** | 规则明确、高频重复、输入输出可结构化 | 数据拉取、报表生成、定时检查 |
| ⚡ **强化岗位** | 需要判断但规则可描述，AI 做领航员 | 设计稿审核、合同条款检查、IP 预警 |
| 👤 **暂时不动** | 依赖直觉经验，或数据不可达 | 战略决策、面对面谈判、未数字化的流程 |

### 三问判定法

对每个节点，替部署专家问三个问题：

1. 输入能从系统自动取到吗？
2. 处理规则能写清楚吗？（让一个新人照着做，能做对吗？）
3. 输出能自动推到人手里吗？

三个全「是」→ 🔄，两个「是」→ ⚡，否则 → 👤。

### 你需要产出的

「节点分类清单」——每个节点标注 🔄/⚡/👤，并附上一句判定理由（为什么这样判？不做什么假设？不确定的地方标注出来让部署专家确认）。

---

## §五 价值量化

节点分类完成了。但在出方案之前，你需要帮部署专家回答一个最关键的问题：**这个节点到底值多少钱？** 这个问题回答不了，企业负责人不会买单——他需要看到数字，不是概念。

### 两种量化口径

对每个 🔄/⚡ 节点，向部署专家确认以下信息：

| 节点类型 | 量化口径 | 你需要追问 |
|------|------|------|
| 🔄 自动执行 | **替人成本** | 这个节点现在谁在做？什么职级、大致薪资？招一个同等能力的人市场年薪多少？ |
| ⚡ 强化岗位 | **升级成本** | 这个人要升级到「能用 AI 高效完成这个节点」的水平——招一个这样的人年薪多少？和当前工资差多少？如果培训，周期多长、费用多少？ |

对所有节点，额外确认：
- **时间成本**：这个节点每天花多长时间？按「月薪 ÷ 22 ÷ 8 = 时薪」折算年人力成本。
- **隐性成本**（如果能拿到）：这个人离职/请假时节点停摆的损失？出错返工成本？

### 按小时算清楚：三条线

所有计算统一以**时薪**为基准（月薪 ÷ 22 ÷ 8），再按日耗时落到三个区间：

| 日耗时 | 意味着什么 | 怎么算成本 |
|:--:|------|------|
| **< 8 小时** | 占用某个人的一部分工作时间。AI 接管后，这个人的时间被释放出来做更有价值的事 | `日耗时 × 时薪 × 年工作日` = 释放的人力价值 |
| **8 小时** | 刚好占满一个人全天。AI 接管 = 释放一个完整人力 | `1 × 年薪` = 完整人力替代 |
| **> 8 小时** | 超出法定 8 小时工作制。如果不是在付 1.5 倍加班费，就是已经有不止一个人在做 | `等效人数 × 年薪`（等效人数 = 日耗时 ÷ 8，向上取整）。超 8 小时的节点是老板最敏感的——**一个 AI 替掉的不止一个人，而且消除了加班费或多人协作的管理损耗** |

> ⚠️ 提醒部署专家：跟企业负责人算这笔账时，强调劳动法底线——标准工时 8 小时/天、40 小时/周，超出的部分要么付 1.5 倍加班费，要么加人。AI 接管一个日均 12 小时的节点，省的不是一个人，是 1.5 个人 + 加班费 + 管理损耗。

### 你需要产出的

「节点价值清单」——每个 🔄/⚡ 节点一行，包含：

| 节点 | 类型 | 当前人力成本/年 | AI 替代后成本/年 | 年节省 | 一次部署成本 | 回本周期 |
|------|:--:|--:|--:|--:|--:|--:|

> 💡 「年节省」这个数字就是部署专家拿去给企业负责人看的。如果一次部署成本是 5 万，年节省是 30 万——两个月回本的事，老板不会犹豫。

---

## §六 出方案

价值算清楚了。现在你需要基于 sofagent 的能力，给每个 🔄/⚡ 节点生成配置方案。

### 每个 AI 节点你需要确定

| 配置项 | 需要你判断 |
|------|------|
| sofagent 版本 | 🔄 节点 → `full`（编排引擎 + 审计）；⚡ 节点 → 宪法层（内联到 SKILL.md 顶部） |
| 铁律重点 | 这个节点的 Agent 最容易违反哪几条铁律？（研发节点→审计 A7 不存盲改；运营数据→审计 A3 不改越界；……） |
| 审计周期 | 日报 / 周报 / 事件触发——哪种最适合这个节点？ |
| 输入源 | 从 §二 盘点的哪个系统/数据库/API 读数据？ |
| 输出目标 | 产出推到哪个平台 / 通知谁？ |
| 企业专属 Skill | 这个节点是否需要定制企业自己的 Skill？（如"电商文案 Skill""电池研发 Skill"——基于 sofagent 模板，加企业自己的规则和上下文） |

### 你需要产出的

「节点→方案对照表」+ **workflow.yaml**——格式要能直接给企业看。一行一个节点，六列（版本/铁律/审计/输入/输出/Skill），部署专家拿去就能和企业讨论。

**workflow.yaml 格式**（编排引擎输入）：

```yaml
nodes:
  - id: node-1
    name: "节点名称"
    type: 🔄  # 🔄 自动执行 / ⚡ 强化岗位
    input:
      source: "API/数据库/文件"
      format: "JSON/CSV/..."
    output:
      target: "钉钉/飞书/邮件"
      format: "报表/通知/..."
    owner: "节点负责人"
    audit:
      period: "daily/weekly/event"
      focus_rules: [A3, A7]  # 重点审计规则
    skill: "enterprise-custom-skill-name"  # 可选
```

---

## §七 部署与交付

方案定了，开始部署。你现在要帮部署专家拆解任务，并确保每一步都完成。

### 第一阶段：逐节点部署 + 企业 Skill 创建

- 🔄 节点：帮部署专家创建固定 sub-agent session，首次编排确认，跑通一次。
- ⚡ 节点：帮部署专家配置 AI 领航员，确认岗位人员会用。
- 需要在 §六 标注了「企业专属 Skill」的节点：基于 sofagent 模板，注入企业的行业术语、业务规则、常用数据源，生成一个初步的专属 Skill。**第一版不追求完美——后续通过 skillopt 自动迭代优化**。
- 你负责检查：每次跑通后，确认输出是不是预期的。

### 第二阶段：知识库搭建

知识库是企业的 AI 工作记忆——你需要在部署时手动创建这些文档，部署完成后 Agent 节点会继续往里写。

**你现在就要建的（静态地基）**：

| 文档 | 内容 |
|------|------|
| 企业工作流全景图 | §三 的工作流节点图整理成一张总图 |
| 工作流节点手册 | 每个节点一页：谁做 / 输入什么 / 输出什么 / AI 帮了多少 |
| AI 节点运维手册 | 每个 session 的说明书：触发方式 / 预期输出 / 异常处理 / 找谁 |
| 新员工上手文档 | 新人看了这份就能懂系统怎么跑 |
| 常见问题与故障排查 | 部署过程中遇到的所有问题和解决方法 |

**确认**：知识库落地到 §二 盘点的企业已有平台（钉钉知识库 / 飞书文档 / Confluence 等）。

> 💡 **为什么知识库是 FDE 的核心交付物**：组织级 Agent Harness 的关键价值是「新员工即时接入共享上下文」——新人不用问人就知道这个节点以前怎么做、找谁、输出什么。知识库把这层做实了，Agent 才不是在真空中运行，而是在组织记忆中运行。

**部署完成后会自动生长的（你不用管，Agent 节点自己写）**：

- 每个 🔄 节点的执行产出
- 每个 ⚡ 节点的 AI 辅助记录
- 审计周报
- 异常事件与处理记录

### 第三阶段：设置工作流检查点

在 workflow 中，每个 🔄/⚡ 节点由 sub-agent 或 AO 编排引擎执行。就像流水线上每道工序都有质检，你需要帮企业为每个关键节点设置检查点——当 sub-agent 完成任务时，不是直接让输出流到下一步，而是先过一道验证。

- **检查点位置**：每个 🔄/⚡ 节点的输出端——sub-agent 完成任务后、产出交付给下一个节点或推送给企业之前。
- **抽检频率**：默认 10%（每 10 次自动执行随机抽 1 次让人复查）。高敏感节点（财务、合同、审批）可调到 50% 或 100%。
- **检查内容**：输出格式是否正确 / 关键数据是否准确 / 有没有遗漏步骤 / 有没有违反铁律。
- **不合格处理**：标记本次任务 + 反馈给节点主人 → 记录到知识库异常日志 → 如果是企业专属 Skill 节点，触发 skillopt 分析失败模式并自动优化 Skill。
- **检查人**：指定企业的节点主人作为质检员，不是部署专家。
- 你负责：帮部署专家为每个 🔄/⚡ 节点配置检查点规则，跑通一次完整链路（sub-agent 执行 → 检查点触发 → 合格通过 / 不合格拦截）。

### 第四阶段：教会一个人

- 帮部署专家为每个 AI 节点指定一个「主人」（企业的人，不是部署专家）。
- 部署专家带主人跑的时候，你负责回答主人的操作问题。
- 目标：三天后主人能独立操作。

### 第五阶段：独立运行

- 部署专家停止主动介入。
- **退出信号**：连续两天没有人（主人或老板）找部署专家问「这个怎么用」。
- 你负责记录：这两天有没有人来问。

---

## §八 交付物清单

部署完成后，企业留下三样东西。你在离场前逐条确认：

### 📄 部署方案书（一份 .md 文件）

这是给企业决策者看的完整方案，将 §三~§六 的所有产出整合为一个文档：

| 章节 | 来源 | 内容 |
|------|------|------|
| 企业现状 | §一 + §二 | 企业画像 + 技术环境清单 |
| 工作流全景 | §三 | 完整工作流节点图（每个节点含五要素） |
| AI 节点识别 | §四 | 节点分类清单（🔄/⚡/👤 + 判定理由） |
| 价值分析 | §五 | 节点价值清单（当前成本 / 年节省 / 回本周期） |
| 部署方案 | §六 | 节点→方案对照表 + workflow.yaml（版本 / 铁律 / 审计 / 输入 / 输出） |

> 这份 .md 文件就是部署专家拿去给企业负责人看的——一张纸讲清楚：现状什么样、AI 能接管哪些节点、省多少钱、怎么配。

### 📚 知识库（五个 .md 文档，落在企业自有平台）

见 §七 第二阶段。五个文档全部创建后写入 §二 盘点的知识库平台。

### ⚙️ 运行时（在跑的 AI 节点）

| # | 是什么 | 确认什么 |
|---|------|------|
| 🔄 节点 | 每个一个 sub-agent session | 已创建、已跑通 |
| ⚡ 节点 | 每个一个 AI 领航员 | 已配置、岗位人员已使用 |
| 企业 Skill | 每个需要定制的节点一个专属 Skill | 已创建、已注入企业上下文 |
| 工作流检查点 | 每个 🔄/⚡ 节点配置检查规则 | 已配置抽检率、已指定质检员、已跑通一次 check 链路 |
| 审计 | 周报推送 | 已配置、已收到第一期 |

---

## §九 商业模式建议（FDE 顾问参考）

> 本节是给部署专家/FDE 顾问的定价与定位参考，不是给企业看的。部署专家据此和企业谈商业合作。

### 收费分层

| 阶层 | 适用场景 | 包含什么 | 定价参考 |
|:--:|---------|---------|------|
| **宪法层** | 个人开发者 / 小团队 | 4 底线 + 6 铁律（内联到 SKILL.md，装主产品即获得） | 开源免费（MIT） |
| **Full** | 中型企业 / 研发团队 | 宪法层 + 编排引擎 + 审计层 + 反思记忆 | 开源免费（MIT），自部署 |
| **Enterprise** | 大型企业 / 多设备协同 | Full + MCP 推送 + 多设备协同 + 企业专属 Skill 生成 + FDE 驻场服务 | 按年/按节点，咨询制 |

> 核心逻辑：纪律层开源免费（建立信任 + 社区传播），增值在设备协同层和企业定制服务。和 GitLab（开源 CE + 企业 EE）、Sentry（免费层 + 付费层）同一模式。

### 优先五类客户

| 优先级 | 客户类型 | 为什么适合 | 典型痛点 |
|:--:|---------|---------|---------|
| ★★★ | 研发型 SaaS 公司 | 代码就是产品，Agent 代码质量直接影响交付 | Agent 改错代码上线 = 生产事故 |
| ★★★ | 金融科技 / 合规密集型 | 审计是刚需，git diff 审计天然对胃口 | 监管要求每次代码变更可审计 |
| ★★☆ | 电商运营团队 | 大量重复性内容生成任务（文案/详情页/客服话术） | 内容质量参差不齐，人审成本高 |
| ★★☆ | 制造业 R&D | 研发文档 + 实验数据管理，AI 辅助分析 | 研发经验不沉淀，换人重来 |
| ★☆☆ | 法律 / 咨询服务 | 合同审查 / 案例检索 AI 化 | 输出必须有据可查，不能编造 |

### 避开行业

| 不推荐 | 原因 |
|------|------|
| 纯人工密集型服务业 | 没有 AI 节点可识别，部署完用不起来 |
| 高度定制化的一次性项目 | 工作流不重复，编排引擎没价值 |
| 数据完全不可达的传统企业 | API 没有、数据靠纸——连 §二 都过不了 |

> ⚠️ 以上是产品定位参考，不是硬性限制。部署专家发现例外情况可以自行判断——你是现场的人，你说了算。

---

## §十 企业画像速采模板

以下是你需要在 §一/§二 阶段和部署专家一起填的表格。部署专家问、你记录：

```
企业名称：
所在行业：
团队规模：
本次覆盖范围（全公司 / 某部门 / 某项目）：

协同平台（钉钉/飞书/企微/其他）：
业务系统（ERP/CRM/自研/Excel/其他）：
数据可达性（有API/可导出/手动/不可达）：
已有知识库平台：

关键岗位清单：
  1. [岗位名] — 人数：[N] — 主要产出：[描述]
  2. ...
  3. ...
```

---

## §十一 离场检查清单

离场前，你需要逐条确认并汇报给部署专家：

- [ ] 所有 🔄 节点 sub-agent session 已创建、已跑通至少一次
- [ ] 所有 ⚡ 节点 AI 领航员已配置、岗位人员已使用至少一次
- [ ] 所有需要定制 Skill 的节点，企业专属 Skill 已创建并注入
- [ ] 每个 AI 节点有明确的主人（不是部署专家）
- [ ] 工作流检查点已配置（抽检率、质检员、不合格处理流程）
- [ ] 知识库五个静态文档已创建并落地到企业自有平台
- [ ] 审计周报推送已配置并收到第一期
- [ ] 近期连续两天无人主动找部署专家问操作问题
- [ ] 企业负责人确认可以离场

> 你不是在盖章——你是在帮部署专家确认「走之后不会出事」。任何一项没打勾，继续推进，不要假装完成。

---

## 参考资源

**workflow 模板参考**：梳理企业工作流时，可参照 [n8n-workflows](https://github.com/Zie619/n8n-workflows)（4,343 个模板，15 类别，365+ 集成）了解常见行业的自动化实践模式。n8n 模板只覆盖「流程管道」层——Agent 的行为约束、验证门控、反思闭环，由 sofagent 纪律层保证。角色不混淆。

**行业方向参考**：Anthropic Cloudtag / Lance Martin「Org Level Agent Harness」——Agent 正从「个人助手稳定干活」进入「组织协作、组织记忆、公共协作环境重构」。sofagent 是这条主线上的治理底座标准（纪律层 + 审计层），不做全栈 Harness 产品，做的是 Agent 进组织时的纪律标准。
