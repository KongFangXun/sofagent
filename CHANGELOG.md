...
- **v1.4.5** — 🛡️ 可靠性与诚实性修复批 + 🚀 后训模块 · 服务与持续：可靠性修复（webhook SSRF DNS 复验 fail-closed · 审计超时降级 · L1-L3 巡检 cron 接线 · hooksPath 尊重 · S2 写入字段脱敏三层收口）· 训练推理服务（`train_serve` 启停/健康探测/model_switch 联动）· 持续后训练（数据阈值/@weekly/人工三触发 + eval 回退保护）· 合规扫描（`train_compliance` PII 三源检测 + 训练闸门 + 来源标记）· FDE 交付包（`train_deliverable` 五件套 + manifest + HMAC）· 保留策略（retention-policy + @weekly 归档 + 90 天覆写）· 进化实证收口（Dream Cycle 真脑 + skill-impact 台账 + 持续采样 + L4 工具自进化）· 后训 Quickstart（十步端到端）· FDE 进场记忆目录（10 文件自动初始化 + session-stop 捕获 + 跨 session 恢复）· MCP 80→**83** · 测试 3744→**4045**（+301，12 包 workspace 口径）· acceptance 304→**305**（S373/S374，S265 归并 S264 对销）· ⏳ 待发版（开发完成，tag/npm 发版时同步）· [开发日志](./docs/changelog/v1.4/v1.4.5.md)
