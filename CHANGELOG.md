...
- **v1.5.0** — 治理模块 · 可见性与本体成熟：治理 KPI 面板（Dashboard 独立「治理」tab 六卡 + 数据集审阅卡（语义质量人审兜底）+ lineage 合规报告导出 + 周报导出）· 本体数据双时态事实（validFrom/validTo + stateAt 时点快照「系统在某天知道什么」+ 三层渐进加载）· Ontology Validation Engine（DAG 三色 DFS 环链定位 + schema 兼容三态 + 激活前置门 fail-closed）· 跨层证据对账引擎（trace 对账——Agent 自述 vs git diff vs 模型行为三源四态：一致/漏报/幻觉/瞒报 + 回滚闭环豁免 · trace_reconcile 新 tool）· FDE 陪跑期补全（14 天期满总结 + fde_deploy 登记衔接）· 存量清扫（checkHistoryChainIntegrity 退役移除[breaking] + composeWithReactAgent 别名 @public 下线[breaking] + loop --legacy 退役显式拒绝 + audit CLI 三 shim 移除）· 命名债务清扫（@sofagent/harness → @sofagent/inject[breaking]，harness 一词独占给产品叙事）· DSH 插件事件接线（7 seamHandler 接既有 bridge API：audit 4 事件位 + inject/evolve/rollback 各 1，判定逻辑零改动纯接线）· MCP 104→**105** tools · 测试 4805→**4903**（+98，13 包 workspace 口径）· acceptance 352→**357**（S427-S431 五新场景：治理 KPI/双时态快照/Validation Engine/trace 对账四态/FDE 陪跑+事件接线）· release-gate Round 3 PASS（有条件放行 · 7 项签署前置 tag 前清零）· ⏳ 待发版 · [开发日志](./docs/changelog/v1.5/v1.5.0.md)
- **v1.4.9** — 设备接入与数据承接：G9 设备注册/发现/心跳（Ed25519 身份 + 心跳新鲜度判定 + 任务捎带/回执进审计链）· G10 数据目录白名单授权读取（device_data_query）· G11 采集声明 opt-in 上行（device_data_push——WAL 加密暂存/明文不落盘/断点续传/审计计量 evidence）· 派单语义（在线才派单 + 掉线改派/挂起）· T7 router 过站 session 承接（schema fail-closed + 签名验签 + 脱敏贯通 + cost 台账）· T8 敏感识别三层插槽（L0 正则/L1 词典/L2 NER 外挂；SDK 面已就绪，管线接线排期 v1.5.1）· T9 权重灰度 AB（hash 稳定分流 + 劣化触发 + 噪声保护；SDK 面已就绪，管线接线排期 v1.5.1）· G5b 连接器注册/发现 + G1 workflow 模板导出/导入 + 血缘追踪 · T6 installer skill + T10 模型清单上报/skill 快照 · bugfix 修复批 13 项 · MCP 95→**104** tools · 测试 4429→**4805**（+376：批1 G9/G10/G11 44 / 批3 G5b/G1 50 / 批4 T6/T10 39 / 批5 T7/T8/T9 109 / bugfix 批 134，13 包 workspace 口径）· acceptance 337→**352**（S418-S426 九新场景：G10 授权读取/G11 上行/installer/派单/蒸馏配对/灰度/模型清单/S425 G1 模板分发/S426 审查体系结构锁）· 发版闸门四轮收敛（质量门禁迭代收敛后放行）· ✅ 已发版（2026-09-17）· [开发日志](./docs/changelog/v1.4/v1.4.9.md)
...
- **v1.4.8** — 插件管控与工程效能：插件来源白名单（plugin-gate 三类来源校验：Git URL/主机/本地路径 + 托管 hook 独裁 + install.sh `--policy` fail-closed）· 应用级工具策略（ToolGate app×tool 白名单矩阵双实现：sandbox 中介版 + 规则接线版，未声明即拒绝）· 多 Agent 协作阵型库（六阵型 schema 校验 + 模板兜底实例化 + 交接留痕与边生命周期）· 自动上下文压缩（加载链 3% 预算检测 + 段级压缩，红线铁律不丢/零依赖纪律）· shell 提权分级（三态 classifier：safe 直接跑 / risky 场景白名单或 HITL / dangerous forbid-until-approved fail-closed）· 成本 quota 事前门禁（WARN/HARD 双模式 + cost_query 余量/已用/周期三字段）· 依赖方向架构测试（build 序列 13 包五层边界 + pre-push 3e/CI 强制）· workflow 节点级模型偏好（modelPreference schema + 注册表解析未注册报错不静默降级）· 执行机制纪律批（意图分类纯函数主判 + Git 能力三态×两隔离九组合矩阵 + 并发 Git 纪律守卫 + 作用域显名禁裸 id）· 进化模块重构（`@sofagent/skillopt` → `@sofagent/evolve` 更名 73 文件联动 + 自研 gate 验证器零 Python 依赖 + Skill Proposer 内化）· 独立审查修复批（四轮合并修复 59 项：逃逸洞/信任地基/断链闭合/门禁前移——明细见开发日志第九章）· LOOP 概念归位（三形态定位边界互不重叠声明：`loop/` StateGraph 对错门禁 · `loop-agent/` 工程级崩溃判定 · `refine-agent/` 质量好坏判据——两两判据/状态机不同，**明确不合并**；`optimization-loop` 降 `@internal`——实测 GitHub 全网 code search 无仓外 adopter，9 测试保留、仅撤公开承诺）· `loop --legacy` 弃用标记（stderr 告警 + help 标注；**移除版本 v1.5.0、兼容期一个大版本**，本批不执行移除）· orchestrator 出口收敛（窄入口非破坏新增：`./train` `./loop` `./model-registry` `./team-state` 四 subpath；daemon 侧 6 消费者改走窄入口——real-provider / cloud-train / cloud-exec / team-channel / cloud-events / cloud-train.test；根 barrel 符号集不动，根因「consumers 无窄入口」以新增面而非替换面解决）· 测试 4279→**4429**（+150：独立审查修复批 10 + 开发批 89 + 审查回归批 29（A1/A2/A9/A20/A23 对抗回归）+ 深模块审查修复批 净减 9（audit 规则元数据断言集中减 20 / orchestrator loop 归位与出口契约增 3 / daemon 巡检器金名单增 6 / mcp 知识页收编与 schema 守卫增 2）+ A2 规则修复批 增 3（函数调用 RHS 豁免回归锁）+ 审计链内核与跨链回归批 增 20（审计链内核用例 audit +15 / 跨链用例 orchestrator +5）+ train 拆包与 A21 回归批 增 8（train 拆包搬家 净增 6——新包 train 647 / orchestrator 1982→1341 计 −641；A21 回归用例 audit 1141→1143 计 2），13 包 workspace 口径：plugin-gate 15 / tool-gate 9 / escalation 11 / quota 8 / formations 10 / compact 9 / model-pref 7 / discipline 16 / native-gate 4 / audit 对抗 29）· acceptance **337**（阶段四 A 类分发 +9：S401-S409 九新面锚点——策略门/行为分级/成本压缩/模型进化/纪律/安全豁免）· 2026-09-13 已发版 · [开发日志](./docs/changelog/v1.4/v1.4.8.md)
- **v1.4.7** — 🚀 商业平台接口版：能力缺口与绩效（G2 `workflow_gaps` 三类缺口判定 / G4 `contribution_query` 人机同标准贡献度 + org 过滤）· 可见性与多租户 v0（G6 节点级 `visibility` 三级枚举 / G7 `data/<tenant>/` 路径隔离 + orgId 语义分层）· PR 生命周期（G13 `pr_submit`/`pr_review`/`pr_merge` 三 tool + pr-store 状态机 + confidence 两态绑定三铁律）· workflow CRUD（G14 四 tool：create/update/node_add/diff_preview + schema 校验门含字段路径 + trigger.schedule cron 门 + 审计挂链 decision-log）· FDE 交付三件（workflow 烧进 USB / G8 首部署 cron 包 `--with-first-deploy-cron` + daemon 5 分钟消费 / 上岗 prompt 生成器三段）· 云训练执行收口（`TrainChannel` 标准接口 + ssh 适配器 + 双通道事件归一 HMAC 挂链 + 托管适配规范 + 参考示例 + 执行者互换测试）· daemon 接线收口批（静态加密全量接线 initDataEncryption 五文档翻牌 / data_push 入口接线 gateDataPush 双闸消费）· 运行时审计 repo-hash 隔离（computeRepoHash 引擎化 + data-sovereignty/llm-calls 双日志按仓库分目录 + 读侧 fallback / commit 级主链保持全局）· audit 专职面文档化（SOFAGENT_MCP_ROLES 配置文档 + 9 tools 语义表 + 门禁防漂移）· 审计留痕双层（audit-reducer 规约层三维过滤 + PROV-O 薄出口）· 独立审查修复批收口（批次明细详见开发日志）· MCP 84→**95** tools · 测试 4107→**4279**（+172，12 包 workspace 口径：audit 1046→1090 计 44 / core 416→429 计 13 / mcp 167→190 计 23 / daemon 374→414 计 40 / orchestrator 1880→1932 计 52；基线 4107 = 上版发版 4088 叠同日独立审查 bugfix 批 19）· acceptance 311→**328**（S384-S400 十七新场景：工具面/PR 域/云通道/接线 + G6/G7/G8/上岗/G14 + G2/G4 语义 + 四修复批锚 + USB + 四原则——质量循环闭环明细见开发日志）· 2026-09-11 已发版 · [开发日志](./docs/changelog/v1.4/v1.4.7.md)
- **v1.4.6** — 🚀 后训模块 · 分布式与云端：多卡/多机训练（train submit 支持 `gpu.count`/`nodes` + job.json schema v2（v1 向后兼容）+ torchrun/verl 集群入口 + GPU 队列多卡拓扑感知 + NCCL 分布式诊断第八类）· 云 VM 执行面（`train_cloud` 注册/体检/远程 spawn 命令构造 + 分拣闸三档（敏感拦上云，依据入审计链）+ 标准数据推送接口（schema + 分拣/合规双闸，入口接线 v1.4.7）+ 云 VM 失联止损判定 + 成本入预算；daemon 真实 ssh 执行接线 v1.4.7）· 边界收缩（模板配方迁外部装载 / trainEnvInit 归 shell 脚本 / model-downloader 删除 / TrainExecutor 接口隔离 spawn 执行面）· MCP 83→**84** tools · 测试 4055→**4088**（+33：v1.4.6 四新面 47 / DSH rc.1 适配 6 / 边界收缩删测 20，12 包 workspace 口径）· acceptance 305→**311**（场景数口径；392 断言含子断言——S376-S382 新增 + 防膨胀归并 2 处 + S383）· 2026-09-09 已发版 · [开发日志](./docs/changelog/v1.4/v1.4.6.md)
- **v1.4.5** — 🛡️ 可靠性与诚实性修复批 + 🚀 后训模块 · 服务与持续：可靠性修复（webhook SSRF DNS 复验 fail-closed · 审计链尾部截断防护（链头锚点 + 创世指纹比对判红）· 审计超时降级 · L1-L3 巡检 cron 接线 · hooksPath 尊重 · S2 写入字段脱敏三层收口）· 训练推理服务（`train_serve` 启停/健康探测/model_switch 联动）· 持续后训练（数据阈值/@weekly/人工三触发 + eval 回退保护）· 合规扫描（`train_compliance` PII 三源检测 + 训练闸门 + 来源标记）· FDE 交付包（`train_deliverable` 五件套 + manifest + HMAC）· 保留策略（retention-policy + @weekly 归档 + 90 天覆写）· 进化实证收口（Dream Cycle 真脑 + skill-impact 台账 + 持续采样 + L4 工具自进化）· 后训 Quickstart（十步端到端）· FDE 进场记忆目录（10 文件自动初始化 + session-stop 捕获 + 跨 session 恢复）· MCP 80→**83** tools · 测试 3753→**4055**（+302，12 包 workspace 口径；基线为 git tag v1.4.4 实测）· acceptance 304→**305**（阶段四 S375 train 五新面行为锁净 +1；S373/S374 新增 +2 与 S265/S366 归并 −2 对销）· ✅ 已发版（2026-09-06）· 2026-09-06 定稿 · [开发日志](./docs/changelog/v1.4/v1.4.5.md)
- **v1.4.4** — 🚀 后训模块 · 信号与部署闭环：训练语料导出三件套（`corpus_export` 79→80 tools + 规则 27 编号位 + 五源样本聚合脱敏 + reward 骨架）· 企业模型本地权重部署（sha256 清单 + 篡改拒绝 + `rollback-weights`）· 训练产物→注册衔接（双闸 + 挂载人审）· 多基座对比训练（`train compare` + ROI 排序）· 决策因果链（`causedBy`/`traceDecisionChain`/先例检索）· CI 供应链全 SHA 固定 + dashboard 离线 · 存量升级审查十项 · spec-first 硬禁令 · 文档与门禁加固 17 项 · 五能力叙事升级（注入·审计·回溯·沉淀·进化）· MCP 79→**80** · 测试 3619→**3753**（+134）· acceptance 294→**304** · ✅ 已发版（2026-09-03）· 2026-09-02 开发完成 · 2026-09-03 定稿 · [开发日志](./docs/changelog/v1.4/v1.4.4.md)
- **v1.4.3** — 🚀 后训模块 · 运行与需求：训练监控 + GPU 队列（`train_status`/`train_list` 79 tools + 显存预算 + webhook）· 失败诊断 `train_diagnose` 七类 · 训练沙箱 + 设备打包 · 需求推导 + RL 配方模板 · 后训练 workflow 模板 · FORGE DSH 深化 · 审计聚合指标 `--stats` · 训练反作弊基线 · onboarding 断层走查 · 可靠性与文档修复 14 项 · 存量清扫（ao 退役 / compose 更名 / `checkHistoryChainIntegrity` 退役公告）· doctor 补 Ontology 完整性 · MCP 76→**79** · 测试 3349→**3619**（+270）· acceptance 276→**294** · 2026-09-01 · [开发日志](./docs/changelog/v1.4/v1.4.3.md)
