...
运行时加固 + 设计约束补充。v0.85 改定位，v0.86 补逻辑。

### 计划

1. **读写型复杂任务分流**（engine.md A4）——写型复杂不拆子 Agent，走单 Agent 高质量上下文模式（Anthropic 实践）
2. **7 条学习笔记落地**（已提交 `672f995`）——v2.x 协同补冲突裁定/交接字段/并发上限 + v0.9 审计误报率约束 + 加载链瘦身审查 + daemon 远期方向
