...
> - **v0.97**：证据版本——纪律层证伪实验（反转 + 真实加载链 + n=5）+ 非 OC 平台加载链实测（90 次）+ A9/A10 代码实现（A11 推迟 daemon）+ 编排引擎精简重构 + bash→TS 第二波
> - **v0.98**：审计独立化——审计闭环六步 + GitHub Action 模板 + 首个持续使用案例 ≥7 天 + 部署层 bash 保留决策 + HANDBOOK 场景重组 + 概念精简
> - **v0.99**：v1.0 前收尾——准入条件 9 条核查 + 外部反馈整理 + 文档终审 + FDE 完整链路端到端验证
...
🔴 整个项目的 v1.0 取决于这个版本的结论。核心：纪律层证伪实验（真实加载链 × 反转设计 × n=5 × 盲评）+ 非 OpenClaw 平台加载链可靠性实测（90 次新会话）。审计 A9/A10/A11 从草案变为代码实现。编排引擎精简重构——砍四级编排深度/三档自由度/渐进减薄/信任等级，保留两档拆解 + Agency Agent 模板匹配（引用 AO Compose 自带模板库）。bash→TS 第二波：6 个核心脚本 + daemon 家族迁移。HANDBOOK 按场景重组。概念精简（22 条规则→14 条）。
...
审计独立化——审计工具从「检测器」升级为「持续改进系统」。审计闭环六步实现（检测→分类→根因→改进→回归→上线），基于 v0.97 积累的真实数据。GitHub Action 模板——零配置 CI 集成。首个外部用户持续使用 ≥7 天案例。HANDBOOK 场景重组 + 概念精简（从 v0.97 推迟）。部署层 bash 保留决策——不迁 TS，bootstrap 层留 bash。企业平台 webhook 推送（钉钉优先）。
...
v1.0 前收尾——不进新功能，只做 bugfix + 文档终审 + 准入核查。v1.0 准入条件 9 条逐一打勾。外部反馈整理 + GitHub Issues triage。文档终审（13 份核心文档复核 + 概念去重复查 + 一致性自动化检查）。FDE 完整链路端到端验证（装环境→梳理 workflow→编排→执行→反思→审计→推送）。bus factor 缓解（目标：≥3 个外部 PR 合入）。所有未完成 TODO 要么完成要么诚实标注「推迟 v1.x」。
