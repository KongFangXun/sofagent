...
- **v1.4.4** — 🚀 训练引擎 · 信号与部署闭环：十章交付（① 训练语料导出三件套：`corpus_export` 79→**80** tools + 规则 27 编号位（24 实现 + A12/A13/E3 占位）+ GUIDE 三锚点方法论导出 + 五源样本聚合脱敏 + 规则→reward 骨架（verifiers.json 三桶）+ CLI/MCP 双入口 ② 企业专属模型本地权重部署：`model_register source:'local-path'` + weights-manifest 逐文件 sha256 + 篡改拒绝 + 版本级回滚 `rollback-weights` ③ 训练产物→注册自动衔接：`artifact-register` 双闸（train done + eval pass）+ 幂等 + 挂载建议强制人审 ④ 多基座对比训练：`train compare` 同数据并行提交 + GPU 队列 + ROI 排序（零成本 ∞ 最前/未完成排除）⑤ 决策因果链：`causedBy`/`causalType` 三类因果边 + `traceDecisionChain` 多级回溯叙事 + `findSimilarDecisions` 先例检索（tags×2+rule×3 打分零依赖）+ HMAC 涵盖新字段 ⑥ CI 供应链加固：8 workflow 20 处 `uses:` 全 SHA 固定 + `check-action-pins.sh` 门禁 + dashboard 离线零 CDN + 默认绑定 127.0.0.1 ⑦ 存量升级审查十项（eval 三套统一 / 79 tools 瘦描述复核 / trimMessagesSafe 审计 / 13 包 engines+README 门面 / check-anchors 文件存在性 / install.sh 目录同步 / 过期承诺限时检查 / FDE 模板外置 / DSH 内嵌 cordis 修复）⑧ spec-first 硬禁令：`check-spec-first.mjs` WARN only + SKILL 铁律第 7 则 + doctor 覆盖率展示 ⑨ 六轮审查 17 项收编批（A 文档 13 + B TDD 4 + C 顺手 4）⑩ 五能力叙事升级（注入·审计·回溯·沉淀·进化 · Distill 📚））· MCP 79→**80** tools（`corpus_export`）· 测试 3619→**3744**（+125：audit 929→976 + core 382→393 + orchestrator 1642→1697 + daemon 294→302 + mcp 154→157）· acceptance 294→**303** 场景（S362-S371 十模块验收 + 章九收编锚点，原 S364 归并入 S348 对销）· **⏳ 待发版** · 2026-09-02 开发完成 · [开发日志](./docs/changelog/v1.4/v1.4.4.md)
