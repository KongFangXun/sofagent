...
- **v1.5.2** — 🔍 审计模块 · 对外开放面与判定语义：MCP audit 数据对外（`audit_query` 只读 + 事件订阅推送）· 约束导出与证据链外部可验（`ruleset_export` 机器可读 JSON 双向可逆 + 独立验签工具零依赖三态校验）· should-run 判定链 · 审计结论失效语义 · 网络出口治理面 · 事前授权补环 · DSH 插件 npm 首发面 · ⏳ 待发版（测试/acceptance 数字待发版前按冻结基线终值表补）· MCP 105→**107** tools（+`audit_query`/`ruleset_export`） · [开发日志](./docs/changelog/v1.5/v1.5.2.md)
