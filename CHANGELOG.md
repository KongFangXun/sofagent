...
运行时加固 + 19 项设计约束补充 + 双视角评审反馈落地。v0.85 改定位，v0.86 补逻辑。

### 🔴 读写型复杂任务分流（engine.md A4）

写型复杂（代码生成/修改为主）不拆子 Agent，走单 Agent 高质量上下文模式；读型复杂（查代码/分析数据）正常拆解。来源：Anthropic Cloud Code 实践——交接冲突代价 > 并行收益。同时补开环/闭环区分 + 四大跑偏检测。

### 🔴 Loop 成熟度四问（loop-check.md closure）

闭环模式最终判断时，必须显式回答四个问题：怎么停 / 谁来判通过 / 失败怎么反馈 / 什么时候交还人类。连续 3 轮不过 → 强制交还人类，不再自动重试。

### 19 项学习笔记设计约束落地（两批）

- **第一批**（`672f995`）：7 条散点补充——v2.x 协同补冲突裁定/交接字段/并发上限 + v0.9 审计误报率约束 + 加载链瘦身审查 + daemon 远期方向
- **第二批**（`fb6eca1`）：12 项系统设计——焊死的门原则 / Loop 成熟度四问 / 开环闭环 / 四大跑偏 / 五层回拉映射 / 管道闸门 / Maker-Checker 上下文隔离 / 反驳层 Bun 案例 / 成本仪表盘 100x / 信号文件夹模式

### 8 项 DeepSeek 双视角评审反馈落地（`f7224f0`）

基于 GitHub 开发者 + CTO 双视角评审：README 适用范围声明 + 平台能力表拆「兼容 vs 支持」+ token 数字改相对比例 + 定位从「唯一纪律层」→「跨平台纪律标准」（.editorconfig 类比）+ v0.9 开发顺序调整（先实验再设计规则）+ 数据存储安全声明 + bash→Node.js 技术选型演进 + n8n 评估
...
定位重构 + ROADMAP 砍削——基于双视角评审（GLM + DeepSeek）的战略校准。
