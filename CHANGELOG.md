...
- **v1.4.5** — 🛡️ 可靠性与诚实性修复批：webhook SSRF DNS 解析复验（fail-closed）· 审计引擎超时自动降级（full→rules-only→minimal）· daemon 分层巡检 cron 接线（L1-L3 真实调度）· git core.hooksPath 尊重（hook 安装随自定义目录）· 测试 3744→**3853**（+109，12 包 workspace 口径）· ⏳ 待发版（开发完成，tag/npm 发版时同步）· [开发日志](./docs/changelog/v1.4/v1.4.5.md)
