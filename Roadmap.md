...
> ⚠️ **v0.60 发布自检发现：加载链步进脆弱性。** Agent 声称"跑了 sofagent"，实际三层加载链只读了 1/3（宪法层被跳过）。根因：加载链依赖 Agent 自觉，SKILL.md 的 ⛔ 只是文字警告。**v0.61 候选已落地四项改进**（反向锚点 / Step 指令块 / 平台分级表 / think.md 冷启动）；A0 前置为第 0 层守门员待 v0.7x。详见 think.md 反思区。
