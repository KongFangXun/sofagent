...
<p align="center" style="color:#64748B;font-size:14px;">
  ...
  <strong>让 SMB（中小企业）与 OPC（运营公司）的每个人都具备 FDE（前线部署工程师）的能力</strong>
</p>
...
> 💡 **命名约定**：`FDE/`、`LOOP/`、`workflow-hub/` 是独立产品 / 模板仓库，按需选用；`sofagent/`、`docs/`、`tools/` 是核心代码与配置。clone 主仓库后全部可用。
...
> [!IMPORTANT]
> 💰 **企业先为「可控性」买单，再为「能力」买单。** Agent 像好奇心强的小孩——能力越足，越要装防撞角、把信用卡放高处。模型越强，企业越不敢直接放手：真正卡住落地的不是「AI 能不能做」，而是「出事了谁负责、能不能拦、能不能回溯」。sofagent 不做模型，只做那最后 10%——让企业敢把任务交出去的约束、审计与问责底座。
...
| 工具 | 定位 | 与 sofagent 关系 |
|------|------|------------------|
| pre-commit + 自定义脚本 | 通用代码质量检查 | sofagent 聚焦 Agent 行为（密钥/越界/注入），不重复通用 lint |
| Cursor Rules | IDE 内规则约束 | sofagent 是 commit-msg hook，IDE 无关 |
| Claude Code hooks | Claude 专用钩子 | sofagent 平台无关，任何 Agent + git 仓库可跑 |

<details>
<summary>📊 对比密钥扫描工具</summary>
...
sofagent 的 [Workflow Hub](./workflow-hub/) 采用**混合架构**：外层用 `workflow.yml` 的 Graph 骨架（`nextNodes`）锁定全链路步骤、保证可追溯；内层单个节点保留模型自主规划（节点 `prompt` 即 ReAct Agent）。既拿到 Workflow 的可控性，又保留局部灵活性。FDE 进场梳理出的工作流，直接沉淀为可复用的企业模板。

| 维度 | 纯自主 Agent | Workflow Hub 混合架构 |
|------|:--:|:--:|
| 全链路可追溯 | ❌ | ✅ 节点固定 + 快照 |
| 抗幻觉 | ❌ | ✅ 路径锁定，仅节点内灵活 |
| 节点级并行提效 | ⚠️ | ✅ |
...
Workflow Hub 是社区驱动的行业工作流模板仓库（代码实现在 `sofagent/workflow-hub/`）。开箱已带 **1 个真实模板**：

| 行业 | 模板 | 流程 | 节点 | 配套文件 |
|------|------|------|:--:|------|
| 制造业 | [应付账款审批](./workflow-hub/templates/制造业/应付账款审批/) | 供应商发票 → 三单匹配 → 审批 → 付款 | 4 | `workflow.yml` + README + 知识库(4) + 技能(3) + Sub Agent(2) |
...
> [!NOTE]
> 模板格式规范见 [SPEC.md](./workflow-hub/SPEC.md)；完整模板目录见 [CATALOG.md](./workflow-hub/CATALOG.md)；提交新模板见 [CONTRIBUTING.md](./workflow-hub/CONTRIBUTING.md)。本地校验：`bash workflow-hub/tools/validate.sh templates/制造业/应付账款审批/`
