...
| AgentLoop（SaaS） | 运行时轨迹观测 | **改了什么**（git diff 硬证据，本地不送云，MIT 开源）|
...
| 🟡 **行为边界** | A3 不改越界 · A4 不删配置 · A15 不盲动 | 改了任务没要求的文件、删配置、未声明 actions |
...
**扩展规则（8 条，需 opt-in）**：A14 知识库越权 · A16 非授权文件变更 · A17 异常批量变更 · E1-E4（测试文件 / TODO 未声明 / 大量删除 / 低注释率）。
...
> 💡 **命名约定**：大写目录（`FDE/`、`LOOP/`、`workflow-hub/`）= 独立产品，按需选用；小写目录（`sofagent/`、`docs/`、`tools/`）= 核心代码与配置。
...
| 维度 | 数据 |
|------|------|
| 审计引擎 | 21 条规则全覆盖，`npm test` 728/728 全绿，0 token 消耗 |
