...
```mermaid
graph LR
    A["1️⃣ 梳理工作流<br/>把企业流程画出来"] --> B["2️⃣ 识别 AI 节点<br/>分出两类"]
    B --> C["3️⃣ 装上工具包<br/>闲置设备装 sofagent"]
    C --> D["4️⃣ 自动跑业务<br/>AI 自己干活、汇报、复盘"]

    B -.-> H["👤 强化人的节点<br/>AI 辅助，人做决策"]
    B -.-> I["🤖 AI 自动节点<br/>AI 全权跑，人看审计"]
...
第二步是关键——不是所有环节都适合 AI 全自动：

| 节点类型 | 怎么跑 | 人做什么 | sofagent 做什么 |
|------|------|------|------|
| 👤 **强化人的节点** | AI 辅助出方案，人来审批拍板 | 决策、判断、签字 | 约束底座确保 AI 不越界，审计引擎记录每一次建议 |
| 🤖 **AI 自动节点** | AI 全权执行，自动跑完整个流程 | 看审计报告、定期抽查 | 三引擎全开：约束→执行→审计→反思循环 |
...
> 🆕 **v1.0.5**：Ontology 统一层 + Workflow Hub 行业模板 + Agent Dashboard + 原子写入 + fail-closed 安全 + A9 分级安全 + 首次运行分类器
