...
**对照件状态翻转（三件准入面）**：① nimble **原始 2,676 训练样本与冻结 324 留出集已公开发布**——[v1.6.0 §二](./changelog/v1.6/v1.6.0.md)引用其对比式构造法时的「数据不可下载」限制实际解除（引用前按当前 HEAD 复验）；② kev 密集更新持续（hard-v1 / devtools-v1 套件上主干、服务隔离与 locked-test 限额、documents-v1/v2 写前门禁）——**VALIDATION §单机全流程各读数以 09-21 快照为准的预警仍然有效**，引用前须对 HEAD 重读；③ **ECE 披露面翻转 + 一件反例**——厂商旗舰件（TypeSafe Jev / NeoHorse-Jev）的**模型卡自报项**仍无 ECE，但第三方对照表已给出读数（AutoJev-27B 对照表含 Jev **ECE 0.05274 / Brier 0.25400**，对方自报、本仓未复算）⇒「品类无数」不成立，空白的是**厂商主动披露**（[v1.7.0 ECE 硬线](./changelog/v1.7/v1.7.0.md) 已同步限定主体）；同批另有一件**被误归入判定件**的生成式模型（Tev1-4B，模型卡 `pipeline_tag=text-generation`、保留标准 next-token LM head）**不入对照池**（[v1.9.0 §五](./changelog/v1.9/v1.9.0.md) 准入鉴别同源）。
