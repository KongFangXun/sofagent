...
> **sofagent 是一个开源约束层——装在企业跑 AI 节点的设备上，盯 Agent 干活。** 进场帮你梳理工作流、部署 AI 节点、离场后 7×24 自己跑。装完之后，你在自己的 Agent（WorkBuddy / Codex / Claude Code）里说一句话，它就帮你干活——审计每次变更、沉淀每次经验、越用越好。下面从装到用到查问题，全流程走一遍。
>
...
- [阅读指南](#阅读指南)
- [5 分钟速览](#5-分钟速览)
- [sofagent 能替你干什么](#fde-agent-能替你干什么)
...
| 你想知道的 | 一句话 | 详见 |
|------|------|------|
| 这是什么 | sofagent——开源约束层，装在企业设备上盯 Agent 干活（审计 + 回溯 + 注入 + daemon 监控） | [sofagent 能替你干什么](#fde-agent-能替你干什么) |
...
> 这一节先讲「价值」，再讲「怎么用」。sofagent 不是一个工具包，而是一个**装在企业设备上、盯 Agent 干活的约束层**——审计每次变更、越界拦截、出事回滚、经验自动沉淀，FDE 离场后 7×24 自己跑。完整能力矩阵见 [ARCHITECTURE · 能力与状态总览](./ARCHITECTURE.md#能力与状态总览v131)。
...
**现在还干不了的事（已排期，暂无代码）**：完整多设备协同 L2、组织能力市场、SubAgent 生产级沙箱、本地推理小模型——路线见 [ROADMAP](./ROADMAP.md)。
...
> 这一幕讲「长本事」——常驻 Agent 怎么把每次任务变成企业资产，越用越懂你的业务。以下功能 daemon 自动运行，装完即生效，你不需要做任何配置。完整能力矩阵见 [ARCHITECTURE · 能力与状态总览](./ARCHITECTURE.md#能力与状态总览v131)；下表只列「装完即自动生效」的新能力。
