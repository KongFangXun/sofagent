...
> | 接口总览 / MCP 工具清单 | [API](./API.md) | 参考 | 六大接口面 + 84 tools 分域清单，由 tool-registry.ts 生成（check-docs §17 对账防漂移） |
...
| 测试覆盖 | 4108 测试 / 12 包（统计标准：`tools/check/test-count.sh` 实际执行的 workspace 包；全仓共 26 个 workspace——13 个模块包发布至 npm `@sofagent` scope，9 个 DSH 插件为 private 随 DSH 分发，4 个 OpenClaw 插件（`engine/openclaw-plugins/`）经根 `npm test --workspaces` 统一执行测试。实测见 `tools/check/test-count.sh`、声称数同步校验见 `tools/check/check-test-count.sh`） |
