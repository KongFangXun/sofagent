...
> - **[ROADMAP.md](./ROADMAP.md)**：版本路线图 + 迭代历程。当前 v1.3.2。

---

## 一、一句话

**sofagent 是一个开源约束层**（MIT，同时也是 FDE 方法论的参考实现）——装在企业跑 AI 节点的设备上盯 Agent 干活：进场梳理工作流、把能自动化的环节变成 AI 节点、部署到设备上 7×24 自己跑。底层是 **约束层（Harness）**——约束 Agent 行为、审计每次变更、沉淀经验。
...
| 项 | 值 |
|----|-----|
| 当前版本 | **v1.3.2**（2026-08-09） |
| 下一版 | v1.3.3（规划中，参见 docs/ROADMAP.md） |
| 测试覆盖 | 2016 测试 / 12 包（测试统计标准：有 test script 的 workspace 包；workspace 总数 13 个均发布到 npm，见 check-test-count.sh） |
...
> **维护规则**：本文档由 AI 在每次发版时更新（版本号、文件清单、状态表）。当前版本 v1.3.2 · 孔放勋 · 2026-08-09。
