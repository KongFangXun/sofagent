...
> - **[ARCHITECTURE.md](./ARCHITECTURE.md)**（99KB）：双层架构设计（能力底座 × 生命周期）+ 引擎工程三层嵌套（Harness → Graph → Loop），关键技术决策记录。**3 秒版**：一底座·三引擎管"做对" · 激活链五阶段管"跑起来" · Graph 控制图分波次 · Loop 自迭代闭环。
...
| 测试覆盖 | 1317 测试 / 12 包（共 1317，16 因 safe-delete 环境限制预期失败） |
...
> **维护规则**：本文档由 AI 在每次发版时更新（版本号、文件清单、状态表）。当前版本 v1.2.3 · 孔放勋 · 2026-07-30。
