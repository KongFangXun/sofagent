...
| **审计引擎** | git diff 驱动，21 条规则（13 默认 + 8 扩展），A1→A19 为活跃规则（A12/A13 为预留编号，当前未启用），每次 commit 自动跑 | [ARCHITECTURE §三](./ARCHITECTURE.md) |
...
上述架构全景中，四引擎之间不是并列关系——它们按 Agent 工程三层架构嵌套：
...
> **维护规则**：本文档由 AI 在每次发版时更新（版本号、文件清单、状态表）。当前版本 v1.2.2 · 孔放勋 · 2026-07-29。
