...
| `engine/mcp/` | @sofagent/mcp — MCP Server（知识库 CRUD tool）· **66 个 MCP tool**（以 `engine/mcp/src/tool-registry.ts` SSOT 为准；v1.4.0 工具角色分层 + 瘦描述，插件家族 MCP 面另计） |
...
| 下一版 | v1.4.1 已交付（待发版，2026-08-25 开发完成，参见 docs/ROADMAP.md） |
| 测试覆盖 | 3168 测试 / 12 包（测试统计标准：有 test script 的 workspace 包；workspace 总数 13 个均发布到 npm，实测见 `tools/check/test-count.sh`、声称数同步校验见 `tools/check/check-test-count.sh`） |
...
> **维护规则**：本文档由 AI 在每次发版时更新（版本号、文件清单、状态表）。当前版本 v1.4.0 · 孔放勋 · 2026-08-23。
