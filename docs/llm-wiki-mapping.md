...
> v1.1.6 新增 · 孔放勋 · 2026-07-19（UTC）

本文档把 LLM Wiki 的「raw materials → Wiki entries → spec norms」三层范式与 sofagent 的「Ledger-Views-Policy」三层治理模型做**同构对照**。

**注意**：本文档**不重新定义三层**。Ledger-Views-Policy 的唯一权威文字定义在 [PHILOSOPHY.md §五](./PHILOSOPHY.md)（L168-181），代码级契约在 `sofagent/core/src/memory-contract.ts`，架构映射声明在 [ARCHITECTURE.md L346/L348](./ARCHITECTURE.md)。本表只做映射，不做定义。
