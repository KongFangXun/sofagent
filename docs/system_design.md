...
└── src/
    ...
    └── hub-cli.ts                        # bin: sofagent-workflow-hub
...
classDiagram
    ...
    WorkflowHub ..> OrchestratorRegistry : (same domain)
...
| | `sofagent/workflow-hub/package.json` — 新建 |
| | `sofagent/workflow-hub/tsconfig.json` — 新建 |
...
#### T03: 运行层 6 包迁出 — orchestrator + daemon + ab-test + workflow-hub + think + skillopt

| 字段 | 内容 |
|------|------|
| **Task ID** | T03 |
| **Task Name** | 运行层 6 包迁出：从 audit/src 迁移剩余模块到 orchestrator / daemon / ab-test / workflow-hub / think / skillopt |
| **Source Files** | |
...
| | **→ @sofagent/workflow-hub**: `audit/src/commands/hub.ts` → `workflow-hub/src/hub.ts` |
...
| **验证方式** | 每个包 `npx tsc --noEmit` 通过。验证依赖铁律：orchestrator→harness / daemon→audit+harness+ontology+core / ab-test→orchestrator+harness+eval+core / workflow-hub→orchestrator+ontology+eval / think→audit+core / skillopt→ontology+core。 |
...
| | `audit/package.json` dependencies 更新：删除 `deepagents`（已迁到 orchestrator）; 删除 `isomorphic-git`（已迁到 core）; 新增 `@sofagent/core`, `@sofagent/orchestrator`, `@sofagent/daemon`, `@sofagent/think`, `@sofagent/ab-test`, `@sofagent/workflow-hub`, `@sofagent/skillopt`。 |
...
graph TD
    ...
    T03["T03: 运行层 6 包迁出<br/>orchestrator + daemon + ab-test + workflow-hub + think + skillopt"]
