...
> 发版时的**操作步骤**见 [releasing.md](../changelog/releasing.md) 阶段四/五。
...
每轮开发完成后（releasing.md 阶段五），从以下 5 个来源提取新发现，归入 A/B/C 三类：
...
四份文档持续加法会膨胀。每轮（阶段五步骤 4-6）执行瘦身：

### 行数警戒线

| 文档 | 上限 | 超标怎么办 |
|------|:----:|-----------|
| regression-checklist.md | 1500 行 | **超标上调不删内容**（v1.3.4 从 1400 上调）——更新 check-docs.sh 的 LIMIT 值 |
| acceptance-test.sh | 2500 行 | 同上（v1.3.4 从 2400 上调） |
