...
**Q：这和 v1.2.x 的「完整多设备协同」有什么区别？**
A：v1.1.x 轻量版 = 文件级别的异步同步（你负责传输，sofagent 负责合并）。v1.2.x 完整版 = 内建传输 + Agent 独立身份码 + 跨设备审计聚合 + 实时同步。轻量版够用就别急着升，完整版解决了身份和实时性的问题。

---

> 📖 相关文档：[ROADMAP § 轻量多设备](../../ROADMAP.md#多设备协同规划中已拆分至-v13x-v14x) · [v1.1.0 开发日志](../changelog/v1.1/v1.1.0.md)
