...
A：v1.1.x 轻量版 = 文件级别的异步同步（你负责传输，sofagent 负责合并）。v1.3.x 完整版 = 内建传输 + Agent 独立身份码（v1.2.5 已交付轻量版 KYA）+ 跨设备审计聚合（v1.2.5 已交付轻量版）+ 实时同步 + 团队协作协议 + 能力市场。轻量版够用就别急着升，完整版解决了实时协作和身份互信的问题（L2 协作协议 v1.3.3、L3 能力市场 v1.3.4、权限体系+网关 v1.4.0）。

---

> 📖 相关文档：[ROADMAP](../ROADMAP.md)（多设备完整协同拆分至 v1.3.1-v1.4.0）· [v1.1.0 开发日志](../changelog/v1.1/v1.1.0.md)
