...
> 事件落 JSON 可进 reward 样本。本文逐条判定 25 条审计规则哪些天然适配节点链（可归因到具体 tool 调用），
> 哪些只对 git diff 有意义，定出 cordis-plugin 节点级审计的规则子集（规则面最终对齐 v1.4.9）。

## 判定框架

25 条规则按**证据来源**与**可归因性**两个维度分三类：
...
> ✅ **接线现状（2026-09-26 核）**：判定源 `checkDangerousCommand` 已随 DSH audit 插件 `tools/pre-execute` 真拦截接线（[SEAMS.md](../../engine/dsh-plugins/SEAMS.md) 对账在案）——标题「设计目标」的实现状态从「全部未接线」推进为「判定源已接线」，逐条覆盖度以 SEAMS 对账为准
