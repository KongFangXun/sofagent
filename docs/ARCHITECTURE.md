...
>
> **身份锚定（判定底座施工期起）**：治理层之下长出 **可治理的决策模型底座**——本产品因此是**一个自带 harness 的决策模型**：判定面（判据集 · 三原语 · 校准概率 · 弃权）是**本体**，约束层五种能力（注入·审计·回溯·沉淀·进化）是它**自带的执行与治理环境**。两者是「**本体与载体**」而非两条并列产品线；顺序上**先备载体**（v1.5.x）、**再装本体**（v1.6.0–v1.9.0 判定底座施工期）。**为什么自带 harness 不是负担而是前提**：只交付判定模型就成了卖模型 / 托管模型（越界）；自带执行与治理层，交付的才是**可自持的系统**——离线可跑、判据与权重随本仓开源、不承诺模型性能只提供通道。
>
