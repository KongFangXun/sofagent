...
**拿六维检查清单做覆盖扫描，暴露两个规则缺口**（不是第 25、26 条规则，是既有判定面**完全没有人管**的两件事）：
...
两处缺口**均已在仓内有归属，不另开出处**（SSOT——同一概念不长成两个入口）：

- **凭证范围** → 归口[权限四原则与零凭证沙箱](#权限四原则与零凭证沙箱)第 1 条「**最小权限**：每个 Agent 只拿当前任务必需的最小凭证集」。**原则已在、判定未在**——原则说的是「最小」，而 24 条判定面没有一条判「匹配度」。**v1.5.7 凭证隔离 Vault 的交付面是托管 / 轮换 / 吊销**（Agent 代码碰不到 token），**不含匹配度判定**——该缺口不随 v1.5.7 关闭。
- **动作授权状态** → 归口[路线图探索方向](./ROADMAP.md#探索方向)的「**事前授权（mandate）补环**」。该条已写明「现有审计是**事后**环……缺**事前**环」，与本缺口是同一件事；其动手条件（真实企业客户提出「先批后干」的合规诉求）**不变**，本缺口登记**不改该条的排期纪律**。

两者的处置都是「**随既有归属推进**」，不是本版新建能力面。
