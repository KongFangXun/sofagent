...
**该减的不是规则，是硬编码阈值。** 规则条数不增不减（交付物落点规则带来的 +1 与判定层无关）。真正该收的是规则体内的常数阈值——commit message 最小长度、单次变更文件数与行数上限、注释率下限、超大文件判定线。它们是未校准的拍脑袋数字，判定化后应由**带校准依据的概率阈值**取代，这是「概率必须有意义」在规则层的唯一落点。

**该加的是三个正交维度，不是第 25 条规则。**

| 维度 | 现状（源码实测） | 判定化后 |
|------|-----------------|---------|
| 置信度 | 规则判定面只有 `status` + `details`，无置信度位（模块别处的 confidence 概念未进规则判定面） | 判定结果携带校准概率；无校准依据的 finding 显式标 `uncalibrated` |
| 判定依据 | `details` 为自由文本 | 结构化原语记录（选了什么 / 备选是什么 / 凭什么 / 多大概率）——「审计质量的分水岭不在留痕密度，而在归因密度」 |
| 档位统一 | 规则四态（PASS/WARN/FAIL/SKIPPED）与调用侧三态（allow / requireApproval）是两套 | 收敛为 ALLOW / ASK / DENY / SKIP 一套 |
