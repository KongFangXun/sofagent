...
这与 Shopify River 的设计逻辑一致：全公司共用一个 Agent 身份，背后是 Workflow Hub 模板 + Sub Agent 编排。sofagent 不做 River 本身（那是企业的统一 Agent），而是提供 River 背后运作所需的 Harness 基础设施——约束底座、审计引擎、Ontology 世界模型、知识库自动积累。
