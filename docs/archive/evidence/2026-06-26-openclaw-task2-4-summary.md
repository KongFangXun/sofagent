...
2. **工程模型 本身能力已很强**：在指令明确的场景下，裸 Agent 能做到接近完美的表现。这验证了「纪律层是增量约束，不是基础能力」的定位——它解决的是 Agent 注意到约束但不遵守的问题。
...
1. **单模型**：仅测试了 工程模型。审查模型 或 Claude 可能有不同表现
