...
约束底座 + 三引擎（审计·回溯·进化）形成治理闭环。FORGE LOOP 流水线是项目内部自迭代工具。完整角色划分与触发方式见 [HANDBOOK §心智模型](./HANDBOOK.md#心智模型一底座三引擎)，架构设计见 [ARCHITECTURE §治理架构](./ARCHITECTURE.md#治理架构一底座三引擎)。以下仅补充哲学视角：
