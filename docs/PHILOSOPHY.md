...
**一条河的比喻**（详见 [README §①](../README.md) / [ARCHITECTURE · River](./ARCHITECTURE.md)）：大厂造河（LLM + Agent 平台），我们做河的治理层——堤坝（约束层）+ 自来水厂（沙箱安全）+ 管网（Workflow）+ 水龙头（Sub Agent）。这把「做平台/中间件、不做外包/服务」具象化：我们不做河，我们做河的治理。
