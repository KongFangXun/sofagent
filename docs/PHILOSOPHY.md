...
> **从 FDE 到 FDEing——把 Engineer 岗位变成 Engineering 能力**：两个相位的合成效果，是 Forward Deployed Engineer 从一个岗位变成 Forward Deployed Engineering 一种能力——人的判断冻结进交付物，人离场后判断作为能力持续运转（7×24 执行、审计、进化时写回）。岗位随人走，能力随交付物留。**动词化：FDE 是名词，FDEing 是动词——万物皆可 FDEing**，任何业务对象、流程、节点都能被 FDEing 一遍「梳理 → 判定 → 交付 → 养护」（不限于软件：硬件节点、机器人运动过程同样是 workflow，差别在执行器、不在治理形态）。**FDEing 同时是一种思维方式**：做任何事之前先想三件——workflow 怎么搭 / 其中的 AI 节点是什么 / 怎么让 AI 更好地帮你实现。这是「软件生息」的语言学表达，也是下文「三级杠杆」要抵达的终点——定制递减到零时，企业得到的不是一名工程师的服务，而是这项工程能力本身。
