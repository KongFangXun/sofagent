...
> **部署不是终点**——系统必须学会「说话」。sofagent 的进化能力不只是优化规则，还定期生成价值证明报告（审计守护周报、知识库增长月报、无 FDE 对照季报），让客户持续感知到 FDE 部署的底座在产生价值。FDE 的成功悖论是：系统跑得越稳，客户感知越弱（详见 [FDE/GUIDE.md §5.9 离场](../FDE/GUIDE.md#510-离场五大能力)）。持续存在感是设计需求，不是营销策略。
