...
约束注入链是骨架里的钢筋，审计能力是质检——开发者才需要往下看。审计能力 24 条规则中 19 条纯 git-diff（零 token、不调 LLM）+ 4 条混合需 Agent 日志 + 1 条文件系统扫描；**约束层（一个层四种能力：注入·审计·回溯·进化）× 生命周期（诊断→激活→编排→执行→进化）**双层架构覆盖从治理到自运转。FORGE 自迭代工具链是项目内部开发工具。本文档以下十章讲的就是这套底层引擎的设计哲学。
...
> **为什么"没核心平台就沦为外包"有财报数字佐证**：Palantir 2025 财年毛利率 82.4%，同期一家大型系统集成商为 31.9%。同样是重人力投入的交付，一个是软件级毛利，一个是服务级毛利。差别不在人力便宜，在底下有没有可复用的原语——用 FDE 把系统集成成本内部化了，却没换到任何长期杠杆，就是"用 FDE 的姿势做外包的事"。sofagent 的对策是把约束层（审计规则 + 约束注入链 + FORGE 循环引擎）做成可复用的原语底座，而不是按客户人月堆人力。
