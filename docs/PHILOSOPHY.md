...
**现场需求三层分流**（收到现场需求的第一动作是分流，不是判断——四问管「该不该收编进产品」，三层管「放在哪一层」，是收编决策的前后两步）：**客户独有** → 留在项目层（客户自己的 skills/，不进产品）；**行业共性** → 行业模板（ruleset overlay 通道，如 fintech / medical，见 [FDE/GUIDE §5.8c](../FDE/GUIDE.md#58c-行业-overlay-标注进场时--v143-起)）；**跨客户复现** → 标准产品候选（才进下面的「产品化四问」走收编判断）。
