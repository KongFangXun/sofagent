...
**一条河的比喻**：River（企业 AI 中台）是一条河，大厂抢着建——建江+供水（AI 中台 = 江，模型 = 水）。模型像水：原始、不受控、会泛滥。我们的机会不在河上，而在河的治理——**堤坝**（sofagent 约束层，不让水侵蚀城市）、**管网**（Workflow，把能力引到业务）、**水龙头**（Subagent，让能力真正作用）。大厂提供水和江，我们让企业安全地用自己的水（AI 能力）流进千家万户（业务）。这正把「做平台/中间件、不做外包/服务」具象化：我们不做河，我们做河的治理层。
...
王阳明「知而不行只是未知」——模型在训练里「知道」规则，却在推理时绕过它，说明它从未真正遵守。修复钥匙不是叠加更多规则（规则越多越易被绕过），而是让系统**理解规则的目的**，并在事前拦截（让违规根本发生不了），而非事后审计（违规已发生再追责）。这与 sofagent「约束底座永远在线 + 审计引擎硬证据」的双向设计同构。
...
Harness 的另一卖点是**「不依赖 AI 也能守门」**。当 LLM 不可用 / 不可靠 / 被降级时，确定性规则引擎（纯 git-diff 正则 + 配置化约束）照常运行，作为 **deterministic guardrails** 兜底接管——Agent 的「智力」可以暂时离线，但「纪律」不能停。

行业五层里「纯规则校验可脱离 AI 运行（模式 D）」直接支撑这点：部分「智能体」只需约束规则、不需要大模型。sofagent 21 条规则中 16 条纯 git-diff、零 token、不调 LLM，正是「AI 挂了纪律还在」的工程实例——这与「约束层 = Harness 中间件」互为表里：Harness 的价值不绑定任何单一模型的可用性。

> 📖 来源：31 篇行业笔记跨批研读（2026-07-20）

### 去人化口径：human-in-the-loop 是「可靠优先」卖点（L3）

行业有主张「去掉人」（L4 Hill-Climbing 去人化）。sofagent 反其道——human-in-the-loop 不是能力缺陷，而是**可靠优先于自主**的差异化卖点。
...
王阳明「知而不行只是未知」：模型在训练里「知道」规则，却在推理时绕过它，说明它从未真正遵守。修复钥匙不是叠加更多规则（规则越多越易被绕过，那 10% 越堆越失效），而是让系统**理解规则的目的**并在事前拦截。真正的可控 = **知行合一**：模型之「知」（理解意图）落到 sofagent 之「行」（约束底座永远在线 + 审计硬证据 + 责任归属），让「知道」变成「做到」。
