...
> **术语口径注（2026-08-29 拍板 · 分层豁免）**：对外产品文案（README 首屏 / 发布物）统一用「7×24 自动执行任务」表述；本文档与 [VALIDATION](./VALIDATION.md) 作为深度文档，沿用行业「数字员工 / 硅基员工」术语讨论其管理哲学——转述行业观点（a16z、红杉等）用行业原词是学术诚实，不随品牌口径改写。直接引文（如 VALIDATION 中 a16z「一百万个糟糕的硅基员工」）任何情况下不改。
