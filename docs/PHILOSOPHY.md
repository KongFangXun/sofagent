...
| **三级·产品复制** | 把「反复出现的定制」抽象为平台标准能力——从此这个需求不再需要工程师到场 | Ontology 运行时层（v1.3.1）+ L3 组织能力市场（v1.3.4）+ 训练协议接口（v1.3.6）+ 训练引擎工程骨架（v1.4.1~1.4.6 开源排期，训练资产走商业侧） |
...
> ⚠️ 远期愿景，尚未实现，非当前能力
>
> 📌 归属边界（与 ROADMAP v1.4.1~1.4.6 排期一致）：训练引擎的**工程骨架随开源仓交付**（编排/审计/沙箱等通用工程能力——开源是信任杠杆，v1.4.1~1.4.6 六版交付；训练协议三约定 + 预算控制已前移 v1.3.6）——**训练资产走商业侧**（商业模型层 承载：配方/权重/数据基准 + 规模化运营）——开源管「骨架 + 审计」，商业管「资产 + 运营」。
...
| L5 模型 | 轨迹→数据集→SFT/RL 改权重 | 训练协议 + 预算（v1.3.6 已交付）+ 引擎工程骨架（v1.4.1~1.4.6 开源排期，资产走商业侧） | 🚧 排期中 |
