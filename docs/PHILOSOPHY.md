...
**② FDE Agent 的答案：一个常驻你企业的 AI 员工，帮你把工作流梳理成 AI 节点，部署完它自己跑。**

FDE Agent 不替代大厂 Agent，而是建在它们之上——堤坝（约束底座，让水不泛滥）、自来水厂（沙箱安全，让水变直饮水）、管网（编排引擎，把能力引到业务）、水龙头（Sub Agent，让能力在具体业务出水）。FDE 进场四阶段：梳理→挖掘→交付→离场。离场后 AI 节点自己跑。
...
**一条河的比喻**：大厂造河——大模型 LLM 是水，Agent 平台是河床，没有河床水只是一片汪洋。模型像水：原始、不受控、会泛滥。我们的机会不在河上（河是大厂造的），而在河的治理——**堤坝**（Harness 约束层，不让水侵蚀城市）、**自来水厂**（沙箱/安全机制，让水从"能喝"到"敢喝"）、**管网**（Workflow，把能力引到业务）、**水龙头**（Subagent，让能力在具体业务用水）。大厂造河（LLM + Agent 平台），我们让企业安全地用自己的 AI 能力流进千家万户（业务）。这正把「做平台/中间件、不做外包/服务」具象化：我们不做河，我们做河的治理层。
