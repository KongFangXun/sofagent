...
> **🔄 自举（产品哲学的核心）：FDE Agent 给自己做的第一份 FDE，就是 sofagent 自己。** 我们自己是一家"FDE 公司"——执行「给企业做 AI 落地」这条 workflow 的 Agent 就是 sofagent。它对自己做 FDE：把项目自身梳理成一条 FDE workflow（梳理 → 节点 → 双 graph 交付），确认每个节点全自动（LangGraph 编排 + DeepSeek Harness 执行 + 约束底座审计），训练引擎也围绕 FDE——怎么让 FDE 更好、怎么让数据飞轮转起来。这是自举循环：**FDE Agent 对自己做 FDE → 项目更 AI 化 → 更好地服务企业**。产品形态 = 一个 FDE Agent（以 DeepSeek Harness 为内核，plugin + skill + MCP + dashboard + CLI 为调用面，v1.4.0 封装落地）。
...
> 本节五层论证（定位 → 边界 → 为什么中间件 → 市场实证），可跳读至 [§二](#二怎么用交互范式) 起使用。
...
>
> 📖 **行业印证（2026-08-19 红杉闭门分享吸收）**：红杉说 Neo-Lab「组织上研究团队和产品、工程团队完全绑在一起，不搞孤岛」——与「数字员工进组织架构、有账号、被派活、对结果负责」同一逻辑：**AI 不是外包的智能，是进组织架构的智能**。约束层四能力（注入=岗位职责说明书 / 审计=绩效考核 / 回溯=试错容错 / 进化=经验沉淀）正是「不搞孤岛」的基础设施——孤岛里的 AI 没有岗位，岗位里的 AI 才有考核与沉淀。
...
| **二级·组件复制** | 把「做过的」变成「能直接用的」——集成连接器、评估框架、部署模板 | install.sh 一键部署 + workflow.yml 批量生成（v1.3.2）+ Refine Agent 复用 Onboard 引擎（v1.3.3） |
| **三级·产品复制** | 把「反复出现的定制」抽象为平台标准能力——从此这个需求不再需要工程师到场 | Ontology 运行时层（v1.3.1）+ L3 组织能力市场（v1.3.4）+ 训练协议接口（v1.3.6，引擎实现走商业侧） |
...
这个哲学反映在 sofagent 的设计中：**约束层 = Agent 的世界模型**。SKILL.md + fde.md + knowledge/ 构成了一个微型的、可演进的本体结构——它告诉 Agent"你在什么组织里、有什么红线、过去踩过什么坑"。Agent 的可靠性不是来自它自己知道多少，而是来自这套约束层锚定了多少业务现实。
...
一句话：**对象和链接是企业的名词，动作是企业的动词——光有名词不算建模，能对名词做什么才算。** 这正是 sofagent v1.3.1 Ontology 本体结构的设计原则——Action Type 作为一等公民，与审计规则对齐，Agent 调用必经 Ontology 定义。
...
**SFT 与 RAG 的适用边界（概念对齐 · 远期参考）**：后训练不是万能药，要先分清问题属于哪一层——**prompt / RAG 解决「模型知道什么」（知识新增）**，把企业知识库挂在检索层，不动模型参数；**SFT 解决「模型本身是什么样」（行为固化）**，把企业的风格、流程、判断习惯训进权重。一句话：**知识新增用 RAG，行为固化用 SFT**。这帮 sofagent 厘清两条产品线的定位——知识层（knowledge/ + 本体结构 + RAG 检索）负责「让 Agent 知道企业的业务」，微调层（v3.x QLoRA）负责「让模型养成企业的做事方式」。两者正交，不互相替代。（当前 sofagent 尚未实现 SFT，仅作概念准备）
...
> ⚠️ 远期愿景，尚未实现，非当前能力
>
> 📌 归属边界：后训练引擎的**实现走商业侧**（商业模型层 承载），sofagent 开源侧只留**训练协议接口**（v1.3.6 已交付训练协议三约定 + 预算控制）——开源管「接口 + 审计」，商业管「引擎 + 数据」。
...
行业正把「Agent 自进化」（self evolution）推向台面，其核心判断值得吸收：**反思（Reflection）与自进化的分界线在于 update——反思只解决这一次任务，自进化必须永久保存修改，解决下一次任务**。只有反思没有沉淀的 Agent，干一万次活和第一次没区别。
...
| L5 模型 | 轨迹→数据集→SFT/RL 改权重 | 训练协议 + 预算（v1.3.6，接口开源；引擎实现走商业侧） | 🚧 排期中 |
...
>
> **部门消亡的微观机制**：部门是工业时代「人的两个局限」的产物——① 能力局限（营销做不了财务，需专业分工）② 利益局限（职业经理人最大化自身利益，需互相牵制监督）。AI 没有专业边界、没有个人利益诉求，两个前提同时消失，部门存在的根基不复存在。所以「按部门拆 Agent」是旧地图找新大陆；组织形态应走向 **GitHub 式协作底座**——按最小工作单元组织（仓库森林），人 + AI 跨树提 PR，机器审阅门把关（git diff 硬证据，纯 AI 节点也能被审），ontology 本体结构作能力地图。
>
