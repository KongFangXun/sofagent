...
**实现机制（技术细节详见 DEVELOPMENT 文档）**：基于开源小基座挂业务适配器精调（不碰上游蒸馏 / 剪枝 / 量化），本地小模型即可在消费级硬件上微调与推理，无需 GPU 集群。
...
**结论——本地小模型可跑**：基于开源小基座 + 消费级硬件（如 Mac Mini），业务专属小模型即可在本地微调与推理，无需 GPU 集群；多台设备的价值在推理并发节点，而非训练集群。整个项目对外保持纯 Node/TS 工作流（训练封装 Python、推理绑定 Node）。具体推理 / 训练框架详见 DEVELOPMENT 文档。
...
> 王阳明「知而不行只是未知」的完整论述（模型知道规则却绕过、破局关键在事前拦截而非事后审计），详见上方 [§十 知行合一注脚（T9）](#知行合一注脚t9)。

真正的可控 = **知行合一**：模型之「知」（理解意图）落到 sofagent 之「行」（约束底座永远在线 + 审计硬证据 + 责任归属），让「知道」变成「做到」。模型越强，那 10% 的「行」越值钱。
