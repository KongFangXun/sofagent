...
> 注：命名「受治理的自主性」对齐既有「受控自主性」，不另起术语；4 级框架为净新增定位资产（二手转述：孔放勋《得到大脑》2026-07-27，原始观点 AI酋长 Andy，URL 待核实）。
...
> **外部印证——Claude Code 强模型时代的提示词革命（2026-07-30 温故知新）**：Anthropic 在 Claude 3.5 Sonnet/Opus 上把系统提示词**砍掉 80%**，核心逻辑正是「模型越强，要写的约束越少」——从"教模型做事"转向"让模型自己推导"。其六个转变与 sofagent 的约束哲学同向：规则→判断（写死硬约束多了会互斥矛盾，通用规范模型自己能推）、前置上下文→渐进式披露（Claude.md 只放每轮都用到的，业务规则按需引用）、简单规范→丰富验证（写"什么算完成"比写"怎么做"更重要）。配套新增 `/doctor` 命令自动体检上下文健康度（去重、识别可从代码库推断的冗余、标记慢 Hooks、统计各组件上下文占比）——是"约束也要定期瘦身"这一判断的工具化。这与 sofagent「Skill 会被模型吞噬、能力要长在代码里」的立场互为印证：连最激进的 Coding Agent 都在反向减负，堆文字约束是死路。> 来源：学习笔记《Claude Code 强模型时代提示词革命》2026-07-30。
...
约束底座 + 四引擎（编排·审计·回溯·进化）形成治理闭环。完整角色划分与触发方式见 [HANDBOOK §心智模型](./HANDBOOK.md#心智模型一底座四引擎)，架构设计见 [ARCHITECTURE §治理架构](./ARCHITECTURE.md#治理架构一底座四引擎)。以下仅补充哲学视角：
...
> 📖 来源：行业参考 blog《分身 vs 员工》《拟人化》（2026，具体 URL 待核验）
