...
- [十一、Agent 生态三层模型与 sofagent 的位置](#十一agent-生态三层模型与-sofagent-的位置)
...
> **激活链 × River 比喻**：用 River 的话说——FDE 帮企业画好了"用水规划图"（交付物），但水厂的水还没接到水管里。激活链 = 把规划图变成真在流的自来水：ACTIVATE 打开水龙头（注册 Agent），ORCHESTRATE 接通管网（编排工作流），EXECUTE 加装水表（审计每一步），SUSTAIN 根据用水数据优化整个系统（反思 + 进化）。不是建新水厂，是把已有的水接到该去的地方。

**③ 底层引擎：sofagent 的 Harness 中间件保证每次变更可审计、可回滚、可进化。**

约束底座是骨架里的钢筋，审计引擎是质检——开发者才需要往下看。审计引擎零 token（纯正则，不调 LLM）；**能力底座（一底座·三引擎：约束底座 + 审计/回溯/进化引擎）× 生命周期（诊断→激活→编排→执行→进化）**双层架构覆盖从治理到自运转。FORGE 自迭代工具链是项目内部开发工具。本文档以下十章讲的就是这套底层引擎的设计哲学。
...
sofagent 的 Harness 中间件就是这条原则的工程实现：审计引擎（21 条确定性规则，只看 git diff）= "控制属于系统"；真正的任务编排交给 AI Agent 平台（WorkBuddy / Claude 等），sofagent 不做编排 = "智能属于模型"。模型负责理解任务、拆解步骤；代码负责验收结果、守住边界。这不是我们发明的理论——行业正在收敛到同一个判断：模型是变化最快的变量，真正应该保持稳定的是受控执行引擎。
...
行业把 Agent 竞争核心收敛到「受治理的自主性（Governed Autonomy）」——sofagent 既有的「受控自主性 / 受控执行引擎」判断与此同义。**Governed Autonomy 给出 4 级能力成熟度模型**，可作定位叙事的外部框架参照：
...
>
> 📖 来源：学习笔记《Claude Code 强模型时代提示词革命》2026-07-30。
...
**SFT 与 RAG 的适用边界（概念对齐 · 远期参考）**：后训练不是万能药，要先分清问题属于哪一层——**prompt / RAG 解决「模型知道什么」（知识新增）**，把企业知识库挂在检索层，不动模型参数；**SFT 解决「模型本身是什么样」（行为固化）**，把企业的风格、流程、判断习惯训进权重。一句话：**知识新增用 RAG，行为固化用 SFT**。这帮 sofagent 厘清两条产品线的定位——知识层（knowledge/ + 本体结构 + RAG 检索）负责「让 Agent 知道企业的业务」，微调层（v3.x QLoRA）负责「让模型养成企业的做事方式」。两者正交，不互相替代。> 来源：学习笔记《SFT 适用边界》2026-07-30（当前 sofagent 尚未实现 SFT，仅作概念准备）。
...
- 产品形态从「一套 Harness」升级为「后训练模型自动部署引擎」，交付物是**部署在企业侧的定制模型（基于企业自有/通用基座后训练，非 sofagent 自制大模型）**，使用者是企业客户而非 sofagent 自身；
- **后训练模型的自动化部署**能力由 Harness + 进化引擎泛化而来（从「自己进化」到「帮企业进化」）。
...
**能力底座（一底座·三引擎）**：约束底座 + 三引擎（审计·回溯·进化）形成治理闭环。FORGE LOOP 流水线是项目内部自迭代工具。完整角色划分与触发方式见 [HANDBOOK §心智模型](./HANDBOOK.md#心智模型能力底座与生命周期)，架构设计见 [ARCHITECTURE §治理架构](./ARCHITECTURE.md#治理架构一底座三引擎)。以下仅补充哲学视角：

**生命周期（激活链，v1.2.5+）**：在能力底座之上，企业 AI 从"诊断完交付一堆文档"走向"自运转"——诊断 → 激活 → 编排 → 执行 → 进化。能力底座回答"怎么保证做对"，生命周期回答"让什么跑起来"；引擎为生命周期提供能力（审计引擎在 EXECUTE 每步把关，进化引擎在 SUSTAIN 吃 think.md 回写），生命周期让引擎有活干。这不是并列的两套东西，是同一产品的**能力 × 流程**两个维度。详见 [激活链设计文档](./guides/fde-activation-chain.md)。
...
> **部署不是终点**——系统必须学会"说话"。sofagent 的进化引擎不只是优化规则，还定期生成价值证明报告（审计守护周报、知识库增长月报、无 FDE 对照季报），让客户持续感知到 FDE 部署的底座在产生价值。FDE 的成功悖论是：系统跑得越稳，客户感知越弱（详见 [FDE/GUIDE.md §5.9 离场](../FDE/GUIDE.md#59-离场五大能力)）。持续存在感是设计需求，不是营销策略。
...
> **原则**：架构复杂度应该来自已经观察到的真实需求，而不是来自对「高级 Agent」的想象。优先建设 Harness 解决工具、状态、权限、恢复和可观测性；当单次结果经常接近正确却需要测试和修订才能稳定交付时，再增加 Loop；只有任务出现明确分支、并行工作、多个专家、人工审批和恢复路径时，才值得引入 Graph。
...
| 图形界面/仪表盘 | LUI-first——语言就是界面（v1.2.3 已落地的只读终端 Dashboard 见 §六，不是交互式 GUI）|
