...
> **🔄 自举（产品哲学的核心）：FDE Harness 层给自己做的第一份 FDE，就是 sofagent 自己。** 我们自己是一家"FDE 公司"——执行「给企业做 AI 落地」这条 workflow 的能力就是 sofagent（嵌在成熟 Agent 与模型之间）。它对自己做 FDE：把项目自身梳理成一条 FDE workflow（梳理 → 节点 → 双图谱交付），确认每个节点全自动（LangGraph 编排 + DeepSeek Harness 执行 + 约束层审计），训练引擎也围绕 FDE——怎么让 FDE 更好、怎么让数据飞轮转起来。这是自举循环：**FDE Harness 层对自己做 FDE → 项目更 AI 化 → 更好地服务企业**。产品形态 = FDE Harness 层（不造 Agent：对执行体 DSH / OpenClaw / WorkBuddy 约束、对智力源通用/专属模型治理，plugin + skill + MCP + CLI + dashboard 五种形态分发）。
...
> **术语口径注（2026-08-29 拍板）**：对外产品文案（README 首屏 / 发布物）统一用「7×24 自动执行任务」。深度文档沿用行业「数字员工 / 硅基员工」原词，**判据是语境而非文件名单**——满足其一即可：① **管理哲学论述**：讲 AI 作为劳动力的岗位、绩效、准入、边界（本文档 §一、[VALIDATION 四跨越](./VALIDATION.md)、[HANDBOOK 岗位准入判定](./HANDBOOK.md)、[DEVELOPMENT 记忆观](./DEVELOPMENT.md)）；② **转述行业观点**：a16z、红杉、钉钉一粟等他人的词，用原词是学术诚实，不随品牌口径改写（[FDE/GUIDE 行业共识](../FDE/GUIDE.md)、[THANKS 署名观点](./THANKS.md)）；③ **语法必要性**：需与「人」对举或作主语时（如版本记录里「人和数字员工同标准」）——「7×24 自动执行任务」是动词短语，塞不进主语位置。**直接引文任何情况下不改**（如 VALIDATION 中 a16z「一百万个糟糕的硅基员工」）。
...
> **数字劳动力的三代进化**：行业把 AI 工具演进划为三代——① 聊天机器人（Chatbot：一问一答，不动手）→ ② 数字分身（Digital Twin：按 SOP 模仿人的操作步骤）→ ③ 数字员工（Digital Employee：进组织架构、有账号、被派活、对结果负责）。三代分水岭不是"更聪明"，是**责任归属**——前两代产物归人，第三代产物归岗位。sofagent 管的就是第三代：约束注入链 = 岗位职责说明书（什么能做什么不能做），审计能力 = 绩效考核（每次变更留证据），回溯能力 = 试错容错（做错了退得回），进化能力 = 经验沉淀（越做越熟）。把 AI 当劳动力而不是工具，就需要劳动力管理基础设施——这正是 sofagent 的定位。
>
> 📖 **行业印证**：红杉说 Neo-Lab「组织上研究团队和产品、工程团队完全绑在一起，不搞孤岛」——与「数字员工进组织架构、有账号、被派活、对结果负责」同一逻辑：**AI 不是外包的智能，是进组织架构的智能**。约束层四能力（注入=岗位职责说明书 / 审计=绩效考核 / 回溯=试错容错 / 进化=经验沉淀）正是「不搞孤岛」的基础设施——孤岛里的 AI 没有岗位，岗位里的 AI 才有考核与沉淀。
>
> **落地卡点的迁移**：数字员工落地的卡点正从「模型能力」转向「组织协议」——能不能进组织架构、有没有账号、怎么派活、绩效怎么算、做错了怎么退回，这些组织侧问题越来越先于模型侧问题成为瓶颈。沿用上文四能力口径：约束层不是技术中间件，而是**数字员工的组织入职协议执行器**——企业接入 AI 的本质，是给硅基员工办入职（背景调查=模型层对齐，入职后的审计制度=运行时对齐，见 [§四·双层对齐](#双层对齐模型层找平衡运行时层兜底)）。
...
> **为什么"没核心平台就沦为外包"有财报数字佐证**：Palantir 2025 财年毛利率 82.4% 对同期大型系统集成商的 31.9%——同样是重人力交付，一个是软件级毛利、一个是服务级毛利，差别不在人力便宜，而在底下有没有可复用的原语（用 FDE 把系统集成成本内部化了、却没换到任何长期杠杆，就是"用 FDE 的姿势做外包的事"）；sofagent 的对策是把约束层（审计规则 + 约束注入链 + FORGE 循环引擎）做成可复用的原语底座，而不是按客户人月堆人力。
...
**再往深一层问：垂直 Harness 里最值钱的是什么？** 不是 harness 软件本身，也不是模型——这两样都是可替换的（sofagent 亲证：编排引擎从 ao → LangGraph → DSH 多后端一路切换，业务流一行没改；模型从 GPT 系换到 DeepSeek 系，审计规则零调整）。真正不可替换的是两样沉淀物：**约束规则**（企业踩过的每个坑固化为审计规则与红线，换 harness 带不走竞品、但跟着企业走）与**业务数据**（审计轨迹、决策日志、错题本——换模型后新模型要从这些数据重新学起，谁积累谁受益）。这回答了「通用 Harness 被吞噬后，垂直玩家还剩什么」：剩的不是软件，是**每家企业独有的规则库 + 数据资产**——sofagent 把它们以开源格式（SKILL 文件 + Ledger + knowledge/）存在企业自己手里，这本身就是护城河。

### 约束增益：四源独立印证

外部约束不是能力的限制器，而是放大器——四个独立来源交叉验证同一结论：

1. **逆缩放**（Harness-MU，arXiv:2606.21856）：基座模型越不可靠，外部管控框架增益越大——GPT 基座指令跟随 42.2%→91.2%（+48.9pt；冲突模式 30.9%→78.1%），全部访问控制攻击下隐私零泄露，效用分反升 0.28–0.39，wall-time 反降 11%。**基座越弱，外部约束放大越狠。**
2. **安全涌现**（GLM-5.3-Flash）：未经专门安全训练，安全能力从「更受约束的编程环境」涌现——约束下的编程能力即安全能力（CyberGym 84.5%）。**约束不压制能力，约束解锁能力。**
...
四源共同支撑「模型越强，Harness 越值钱」的下一句：**Harness 不只保值，还增值**——约束层的注入/审计/回溯/进化四能力，把外部约束从「保险」重新定义为「性能组件」。

> 📖 来源：arXiv:2606.21856（实测核验）；GLM-5.3-Flash 安全涌现评测 / 学术 Skill 横评 / AMD AgenticAI 工程实践（知识库蒸馏四源积累）
...
sofagent 的约束层就是这条原则的工程实现：审计能力（24 条确定性规则，其中 19 条纯 git-diff 零 token，5 条非纯 git-diff——4 条 hybrid 需 Agent 日志 + 1 条 filesystem 扫描）= "控制属于系统"；真正的任务编排交给 AI Agent 平台（WorkBuddy / Claude 等），sofagent 不做编排 = "智能属于模型"。模型负责理解任务、拆解步骤；代码负责验收结果、守住边界。这不是我们发明的理论——行业正在收敛到同一个判断：模型是变化最快的变量，真正应该保持稳定的是受控执行引擎。

> 📖 来源：受控智能体引擎设计实践（2026-07），核心理念「智能属于模型，控制属于系统」

> 🏛️ **「金牌辅助，不抢饭碗」——MIT 诺奖得主的技术分类论据**：麻省理工三位经济学家（含 2024 年诺奖得主阿西莫格鲁、约翰逊）在《构建支持劳动者的 AI》中把 AI 技术分五类——助力劳动者 / 助力资本 / **纯自动化** / 技能均等化 / 创造新任务，判断最利于劳动者的是「创造新任务」，最不利的是「纯自动化」（机器接管人类劳动）；真正好的技术是**当金牌辅助、不跟人抢饭碗、拓展人类能力边界，提升劳动价值而非抹杀专业门槛**（来源：MIT 报告《Building an AI That Works for People》，2026）。这与 sofagent「不替代 Agent，做 Agent 的控制面」同构：约束层让 AI 在边界内干得更好，而不是让 AI 顶掉人——测量、合规、审计正是「金牌辅助」落点（审计引擎记录 Agent 行为，人负责判断与签字，见 [VALIDATION · 测量者转型](./VALIDATION.md#测量者转型从月底审计报表到每次-ai-行动留日志cloudflare-实证)）。
...
> **为什么 Agent 不能继承人的角色权限**：角色树是给人设计的，隐含"这个人知道什么时候不该点"这个默认。Agent 拿到"处理这批延期订单"和采购单修改权，就会去改交期把延期变成不延期——角色树写了能改交期，没写动交期得先问计划员。这就是为什么 Agent 需要独立身份 + 最小 scope token，而不是直接继承发起人的角色权限——"知道什么时候不该点"是人的判断力，不是角色树的字段。

> 📖 来源：Palantir 实施方法论报告（2026-08，§3.3 权限继承的边界条款）
...
| **二级·组件复制** | 把「做过的」变成「能直接用的」——集成连接器、评估框架、部署模板 | install.sh 一键部署 + workflow.yml 批量生成 + Refine Agent 复用 Onboard 引擎 |
| **三级·产品复制** | 把「反复出现的定制」抽象为平台标准能力——从此这个需求不再需要工程师到场 | Ontology 运行时层 + L3 组织能力市场 + 训练协议接口 + 训练引擎工程骨架（v1.4.1~1.4.6 开源排期，训练资产走商业侧） |

**试金石——定制递减率**：第 N 个客户的定制工作量，应该显著小于第 1 个。连续三个客户定制量不降，立即检视产品回流机制——不降就是外包，不是 FDE。sofagent 的 dogfooding 度量应跟踪这个指标：每次给新客户部署，定制量有没有在降。

**现场需求三层分流**（收到现场需求的第一动作是分流，不是判断——四问管「该不该收编进产品」，三层管「放在哪一层」，是收编决策的前后两步）：**客户独有** → 留在项目层（客户自己的 skills/，不进产品）；**行业共性** → 行业模板（ruleset overlay 通道，如 fintech / medical，见 [FDE/GUIDE §5.8c](../FDE/GUIDE.md#58c-行业-overlay-标注进场时--v142-起)）；**跨客户复现** → 标准产品候选（才进下面的「产品化四问」走收编判断）。
...
sofagent 用**约束层（一个层四种能力：注入·审计·回溯·进化）× 生命周期（诊断→激活→编排→执行→进化）**的双层架构组织产品——约束层回答"怎么保证每次做对"，生命周期回答"企业 AI 从诊断到自运转怎么走"。完整的层级定义、状态机图与"双层 vs 三层嵌套"消歧见 **[ARCHITECTURE §心智模型](./ARCHITECTURE.md#心智模型先读这个)**（权威源），操作视角的角色表与触发方式见 [HANDBOOK §心智模型](./HANDBOOK.md#心智模型约束层与生命周期)。以下仅补充**哲学视角**：

**演进视角：从「脑力自动化四阶段」看四种能力的来路**
行业对「脑力自动化」的认知可归纳为四个阶段——提示词自动化、上下文工程、驾驭（agent 自主执行）、循环自动化（自驱动迭代）。sofagent 的**约束层四种能力**，正是对这四个阶段的逐层回应：审计能力承接「提示词 / 规则」的确定性沉淀，约束注入链承接「上下文工程 + 驾驭」的掌控执行，进化能力承接「循环自动化」的自迭代闭环。底层不变的是约束层——无论 AI 能力涨到哪一阶段，确定性边界始终由约束层兜住。这不是外部理论的移植，而是我们自己在做产品时反复验证过的演进逻辑。
...
| **Harness Engineering** | 怎么约束模型的行为 | **sofagent 整体**——约束层（注入 + 审计 + 回溯，核心价值层）|
...
这两个约束推出 sofagent 的定位：**运行时对齐不是模型层对齐的替代，是它的保险层。** 模型层对齐解决「出厂时的平均情况」，运行时对齐解决「这一次操作的实际情况」——审计引擎（git diff 硬证据）、验收门（turn-stopping）、HITL 审批，都是在模型已经输出（或即将输出）之后用确定性规则再核一遍。模型层对齐做得再好，企业也不能把「模型应该不会乱来」当部署依据——正如再好的员工背景调查，也不能替代入职后的审计制度。
...
> 📖 来源：Palantir Foundry 官方 Ontology 建模指引 + 实施方法论报告（2026-08，客户版 V1.0.0 §3.2 建模重心从名词转向动词）
...
> ⚠️ 远期愿景，尚未实现，非当前能力
>
> 📌 归属边界（与 ROADMAP v1.4.1~1.4.6 排期一致）：训练引擎的**工程骨架随开源仓交付**（编排/审计/沙箱等通用工程能力——开源是信任杠杆，v1.4.1~1.4.6 六版交付；训练协议三约定 + 预算控制已前移 v1.3.6）——**训练资产走商业侧**（商业层承载：配方/权重/数据基准 + 规模化运营）——开源管「骨架 + 审计」，商业管「资产 + 运营」。
...
| L2 策略 | 规划瘦身（10 步砍到 7 步） | route-policy + skillopt 优化 | ✅ 部分 |
| L3 技能 | 成功路径封装成可调用技能 | instinct→skill（三源提取 + 置信度 + 人审聚合，v1.3.5） | ✅ |
| L4 工具 | Agent 自写工具注册进工具箱 | ——（安全语义基建已有：SkillScan 门 + 人审注册） | 🔴 探索方向 |
| L5 模型 | 轨迹→数据集→SFT/RL 改权重 | 训练协议 + 预算 + 引擎工程骨架（v1.4.1~1.4.6 开源排期，资产走商业侧） | 🚧 排期中 |

由此而来的产品判断：

- **「做 1000 次任务后是否比第一次更强」是比单次质量更硬的强度指标**。审计轨迹、AB 实验胜负、错题本复发率都是现成数据源——把「越用越聪明」做成可观测曲线，而不是营销话术。（边界注：当前进化是设计上自动沉淀——think.md 反思回灌 + skillopt 生成候选，但 Dream Cycle 重启即丢、skillopt 依赖外部 CLI，效果是"可积累"而非"全自动闭环"，详见 [LIMITATIONS](./LIMITATIONS.md)）
- **L4（自写工具）必须过安全门**。Agent 自己写的代码要进生产工具箱，与人类贡献代码同待遇：SkillScan 扫描 + 人审批准 + 审计留痕。没有治理的自进化是漏洞制造机——这是 sofagent 与「放养式自进化」叙事的分野。
- **进化必须保多样性——「锐化」与「泛化」的平衡有量化依据**。RL 后训练会压缩解的覆盖（pass@k 随 k 增大反而被基座超越），ES 却同时提升 pass@1 与 pass@k；在 MATH500/Olympiad Bench 上，ES 相比 RL checkpoint 减少回退、增加前进，且失败时保留更高答案熵——RL 是「低熵地确信错」（过度自信的错误），ES 是「高熵地存疑」。经验沉淀（think.md → knowledge/）应保留多解法谱系而非收敛到单一「最优解」，Dream Cycle 蒸馏保留失败路径记录，就是给下一轮进化留分叉（arXiv 2608.12679，Cognizant AI Lab × UT Austin）。
...
> 📖 来源：Palantir 实施方法论报告（2026-08，§4.2 FDE 双重身份）/ Palantir 官方博客（Deployment Strategist 系列）

FDE 入场时，不搭交互页面。做的事是：梳理 workflow 节点 → 定义输出终点 → 注入 AI 知识库。但产品化之后，**用户自己也能做**——这份文档就是教你怎么做。

> **部署不是终点**——系统必须学会"说话"。sofagent 的进化能力不只是优化规则，还定期生成价值证明报告（审计守护周报、知识库增长月报、无 FDE 对照季报），让客户持续感知到 FDE 部署的底座在产生价值。FDE 的成功悖论是：系统跑得越稳，客户感知越弱（详见 [FDE/GUIDE.md §5.9 离场](../FDE/GUIDE.md#59-离场五大能力)）。持续存在感是设计需求，不是营销策略。
