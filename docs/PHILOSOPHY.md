...
> **「核心智能绝不能外包」——黄仁勋（2025.7, with Harrison Chase）**：企业建立在专属领域的知识产权之上——这里边的智力本质上就是一种智能，直接定义了企业是谁。通用基础
...
> 📖 来源：数据搬砖日记《大模型能自动构建本体吗：AI 可发现候选但不能替业务定义现实》（2026-09）；老李说数《企业本体落地系列④：业务部门深度参与——三类角色四环节与标签争议裁决》（2026-09）；虎嗅《企业该买模型还是养上下文：专有模型四件套拆解》（2026-09）——三源跨域互证（链接见知识库同名笔记 frontmatter）技能（编程、写作）可以依托公共模型，但定义企业独特性的专属智能必须完全掌握在自己手中。「我无法想象当我需要提升企业自身智能水平的时候，还要去向第三方求助——我必须在企业内部直接完成这件事。」 sofagent 的 MIT 开源 + 本地部署 + 数据不出本机，正是这一原则的工程落地。
