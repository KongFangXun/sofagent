...
| L5 模型 | 轨迹→数据集→SFT/RL 改权重 | 训练协议 + 预算 + 约束层工程骨架（v1.4.1~1.4.6 开源排期，资产走商业侧） | 🚧 排期中 |
