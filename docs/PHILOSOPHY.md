...
> **术语口径注**：对外产品文案（README 首屏 / 发布物）统一用「7×24 自动执行任务」。深度文档沿用行业「数字员工 / 硅基员工」原词，**判据是语境而非文件名单**——满足其一即可：① **管理哲学论述**：讲 AI 作为劳动力的岗位、绩效、准入、边界（本文档 §一、[VALIDATION 四跨越](./VALIDATION.md)、[HANDBOOK 岗位准入判定](./HANDBOOK.md)、[DEVELOPMENT 记忆观](./DEVELOPMENT.md)）；② **转述行业观点**：a16z 等行业机构的词，用原词是学术诚实，不随品牌口径改写（[FDE/GUIDE 行业共识](../FDE/GUIDE.md)、[THANKS 署名观点](./THANKS.md)）；③ **语法必要性**：需与「人」对举或作主语时（如版本记录里「人和数字员工同标准」）——「7×24 自动执行任务」是动词短语，塞不进主语位置。**直接引文任何情况下不改**（如 VALIDATION 中 a16z「一百万个糟糕的硅基员工」）。
