...
约束注入链是骨架里的钢筋，审计能力是质检——开发者才需要往下看。审计能力 25 条规则按实现方式分 20 条纯 git-diff（零 token、不调 LLM）+ 4 条混合需 Agent 日志 + 1 条文件系统扫描（按启用方式则为 17 默认 + 8 扩展，SSOT 见 [SECURITY](../SECURITY.md)）；**约束层（一个层五种能力：注入·审计·回溯·沉淀·进化）× 生命周期（诊断→激活→编排→执行→进化）**双层架构覆盖从治理到自运转。FORGE 自迭代工具链是项目内部开发工具（**FORGE = 项目内部自迭代工具链，非产品能力**；本文档中 FORGE 的后续出现均指此，见 [WIKI 术语表](./WIKI.md)）。本文档以下九章讲的就是这套底层实现（约束层）的设计哲学。
...
> ⛔ **交付服务边界（显式声明）**：本仓开源 **FDEing 方法论与判据 schema**，**不提供、不承诺、不代理人力交付服务**——「派 FDE 进场替企业梳理」属交付服务生意，归宿在服务型产品线（FDE Agent for enterprise Forward Deployment 的服务面），与本仓是**方法论供给**关系而非同一交付主体。本仓文档一律以方法论自述（「FDEing 指导交付者……」），不写「我们去企业交付」的服务口吻。仓内 `fde_*` 系列 MCP 工具是**方法论的软件辅助**（产出判据与数据，属本仓），不是服务本身。**判据 schema 唯一 SSOT**：判据格式定义只住在本仓（rules schema + 正负样例 schema）——任何外部仓（含服务线）消费判据一律引用本仓 schema，禁止另写一份；FDE 方法论文件可以引用判据、不得定义判据（有门禁断言）。
...
> 💡 **为什么 sofagent 是「系统型」不是「薄壳型」（行业产品生死线判断）**：行业把 AI 产品分为「薄壳型」与「系统型」——薄壳站在**完成型任务**上（翻译/总结/识图，有终点），赚「比用户更会用模型」的信息差，模型一升级门槛被抹平、壳被穿透；系统型站在**开拓型指标**上（更多客户/更高复购/更高转化，没有终点），模型每一代升级都是把结果往上推的工具。sofagent 的审计模块 20/25 条规则是纯 git-diff 确定性判定、零 token、不依赖模型版本——模型升级不仅不穿透审计，反而让「AI 干更多活 → 需要审计的变更更多」，这正是系统型的结构：护城河在「定义完成 + 验收 + 留痕」的闭环，不在模型调用技巧本身（同源见下文「市场实证——垂直 Harness 才是护城河」）。
...
> **为什么「没核心平台就沦为外包」有财报数字佐证**：Palantir 2025 财年毛利率 82.4% 对同期大型系统集成商的 31.9%——同样是重人力交付，一个是软件级毛利、一个是服务级毛利，差别不在人力便宜，而在底下有没有可复用的原语（用 FDEing 把系统集成成本内部化了、却没换到任何长期杠杆，就是「用 FDEing 的姿势做外包的事」）；sofagent 的对策是把约束层（审计规则 + 约束注入链 + FORGE 循环机制）做成可复用的原语底座，而不是按客户人月堆人力。
...
sofagent 的约束层就是这条原则的工程实现：审计能力（25 条确定性规则，其中 20 条纯 git-diff 零 token，5 条非纯 git-diff——4 条 hybrid 需 Agent 日志 + 1 条 filesystem 扫描）= 「控制属于系统」；真正的任务编排交给 AI Agent 平台（WorkBuddy / Claude 等），sofagent 不做编排 = 「智能属于模型」。模型负责理解任务、拆解步骤；代码负责验收结果、守住边界。这不是我们发明的理论——行业正在收敛到同一个判断：模型是变化最快的变量，真正应该保持稳定的是受控执行模块。
...
> **「核心智能绝不能外包」——黄仁勋（2025.7, with Harrison Chase）**：企业建立在专属领域的知识产权之上——这里边的智力本质上就是一种智能，直接定义了企业是谁。通用基础，技能（编程、写作）可以依托公共模型，但定义企业独特性的专属智能必须完全掌握在自己手中。「我无法想象当我需要提升企业自身智能水平的时候，还要去向第三方求助——我必须在企业内部直接完成这件事。」

sofagent 的 MIT 开源 + 本地部署 + 数据不出本机，正是这一原则的工程落地。

**定义权的两个侧面：系统侧定义「成功」，业务侧定义「语义」**。约束层的既有判断是成功标准必须握在系统侧（成功的定义权不外包给模型）；行业本体实践补上另一个侧面——语义规则的最终定义权必须留在业务侧。四类问题不能交给模型：什么该建模、两个名称是否同一对象、哪条规则当前有效、后果谁承担——模型可承担候选发现、形式化草稿、结构检查、测试辅助，定义权不可外包。业务侧落地形态是参与边界表：业务定义对象属性、确认流程规则、验收数据质量；IT 只做架构/映射/性能，不定义业务规则、不判断数据口径、不决定考核指标；争议经治理委员会裁决（会前有材料、会中批准/驳回、会后闭环）。与「智能与控制分离」互为表里：控制面管「何时算做对」，业务面管「什么是对」——两层定义权都不在模型手里。

> 📖 来源：数据搬砖日记《大模型能自动构建本体吗：AI 可发现候选但不能替业务定义现实》（微信公众号，2026-09-16）；老李说数《企业本体落地系列④：业务部门深度参与——三类角色四环节与标签争议裁决》（微信公众号，2026-09-18）；虎嗅《企业该买模型还是养上下文：专有模型四件套拆解》（微信公众号，2026-09-16）——三源跨域互证（公众号原文无公开 URL，存档见知识库同名笔记）
...
两个方向的反证说明这不是拼盘：**只有 FDE 没有 Harness** = 传统咨询——交付一堆文档，人走判断蒸发，没有 7×24 执行，「企业上了 AI 但没人用」的原始问题没被解决；**只有 Harness 没有 FDE** = 空壳引擎——25 条规则没有业务判据可审，编排没有 workflow 可跑，daemon 没有节点可巡检。两者互为存在前提。

> **从 FDE 到 FDEing——把 Engineer 岗位变成 Engineering 能力**：两个相位的合成效果，是 Forward Deployed Engineer 从一个岗位变成 Forward Deployed Engineering 一种能力（**FDEing 即 Forward Deployed Engineering 的缩写**——名词指这种能力，动词指把 FDE 从人力工作变成这种能力）——人的判断冻结进交付物，人离场后判断作为能力持续运转（7×24 执行、审计、进化时写回）。岗位随人走，能力随交付物留。**动词化：FDE 是名词，FDEing 是动词——万物皆可 FDEing**，任何业务对象、流程、节点都能被 FDEing 一遍「梳理 → 判定 → 交付 → 养护」（不限于软件：硬件节点、机器人运动过程同样是 workflow，差别在执行器、不在治理形态）。**FDEing 同时是一种思维方式**：做任何事之前先想三件——workflow 怎么搭 / 其中的 AI 节点是什么 / 怎么让 AI 更好地帮你实现。这是「软件生息」的语言学表达，也是下文「三级杠杆」要抵达的终点——定制递减到零时，企业得到的不是一名工程师的服务，而是这项工程能力本身。
...
| L4 | Accountable System（可问责系统） | 全流程留证据、可解释决策、责任可追溯 | **sofagent 目标态**：append-only 审计（25 条：A1-A11 + A14-A24 + E1/E2/E4）+ 控制平面治理 + 责任可追溯 |
...
**确定性与概率性分离**是这一原则的工程落地方式：刚性安全边界（审计、权限、数据一致性）由确定性引擎保障，LLM 仅在意图理解、参数组装等环节发挥自主性。sofagent 的 20/25 条纯 git-diff 规则，正是因为这个原则——不看 Agent 说什么，看 diff 里实际改了什么。
...
| ④ 模型精调 | 知识库 → QLoRA 训练语料 | `sofagent-model` CLI | 🟠 排期中（[v1.7.0](./changelog/v1.7/v1.7.0.md) 本地 LoRA 档 + [v1.8.0](./changelog/v1.8/v1.8.0.md) 权重分发） |
...
**路线已前移（原 v3.x/v4.x 时间线作废，2026-09-26 对齐）**：本地推理与权重部署已排进判定底座施工期——[v1.7.0](./changelog/v1.7/v1.7.0.md) 本地 LoRA 档为交付形态下限、[v1.8.0](./changelog/v1.8/v1.8.0.md) 权重分发通路与自带件形态成立、离线介质两档交付（A 安装器档 / B 便携运行时档）同期落地。完整技术骨架见 [ROADMAP · 分层模型架构（v3.x 远景概述）](./ROADMAP.md#分层模型架构v3x-远景概述)。
...
**离线介质的身份语义（重定位 · 2026-09-26）**：U 盘退役「物理身份载体/完整离线节点」旧叙事，重定位为 **DSH 节点 + 判定件的交付介质**（两档：安装器档 / 便携运行时档，见 [v1.8.0](./changelog/v1.8/v1.8.0.md)）——介质承载的是「被治理的判定能力」，治理身份仍在企业侧的审计链与权限体系，不随介质转移。
