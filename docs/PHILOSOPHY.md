...
FDE Agent 不替代大厂 Agent，而是建在它们之上——用 River 比喻说就是：堤坝 + 自来水厂 + 管网 + 水龙头（详见 [README §①](../README.md)）。FDE 进场四阶段：梳理→挖掘→交付→离场。离场后 AI 节点自己跑。
...
**一条河的比喻**（详见 [README §①](../README.md) / [FDE §9.6](../FDE/FDE.md)）：大厂造河（LLM + Agent 平台），我们做河的治理层——堤坝（约束层）+ 自来水厂（沙箱安全）+ 管网（Workflow）+ 水龙头（Sub Agent）。这把「做平台/中间件、不做外包/服务」具象化：我们不做河，我们做河的治理。
