...
> **SSOT 口径**：`FORGE/playbook/acceptance-test.sh` 头部「场景数」声明 = 真实 `scenario` 调用行数（非编号最大值、非运行时执行数）。当前值 305（最大场景号 S374，S1-S344 间有历史空洞号；S373/S374 为 v1.4.5 反哺闭环与 L4 自进化链路级场景，S265 归并入 S264 对销）。
>
