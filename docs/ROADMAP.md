...
> **v1.4.0 已于 2026-08-23 交付**——Web 工作明细页（四视角工作记录）+ 图谱栏（FDE 双 graph + MCP 工具视图 + skill 加载链）+ 成本审计（超支告警 + `cost_query` MCP + COST DecisionKind）+ DSH 插件家族（`cordis-plugin-sofagent-*` 9 款，DSH inventory 全可见 + Cursor/Claude hook 拦截）+ OpenClaw 插件家族（4 款 code-plugin）+ Dashboard HTML 产品化 + 联邦查询跨设备 E2E（S320 fork 10 断言 + S322 独立进程 4 场景）+ MLflow 接线 + Agentic Browser（61→66 tools）+ 工具角色分层 + 瘦描述（默认全量 66 · `SOFAGENT_MCP_ROLES` 显式收窄专职面）+ bash 3.2 真实环境验证 + 审计溯源字段回填。详见 [v1.4.0 开发日志](./changelog/v1.4/v1.4.0.md)。
>
...
| 版本 | 核心交付 |
|------|------|
| **v1.4.0** | 📊 Web 工作明细页 + 🗺️ 图谱栏（FDE 双 graph + MCP 工具视图 + skill 加载链）+ 💰 成本审计（超支告警 + `cost_query` MCP + COST DecisionKind）+ 🔌 DSH 插件家族（`cordis-plugin-sofagent-*` 9 款 · inventory 全可见 + Cursor/Claude hook 拦截）+ 🦞 OpenClaw 插件家族（4 款 code-plugin）+ 🏠 Dashboard HTML 产品化 + 📡 远程 API 通道（契约）+ 🔗 MLflow 接线 + 🌐 Agentic Browser（66 tools）+ 🔀 工具角色分层 + 瘦描述（默认全量 66 · `SOFAGENT_MCP_ROLES` 显式收窄专职面）+ 🔄 联邦查询跨设备 E2E（S320 fork 10 断言 + S322 独立进程 4 场景）+ 🐚 bash 3.2 验证 + 🔍 审计溯源字段回填 · 测试 2903→2978 全量（+74：引擎 +30（含工具分层 +18）· DSH 插件 +27 · OpenClaw 插件 +17；workspace 口径 2934/13 包）· MCP 61→66 |
