...
> v1.4.9 · 2026-09-17（UTC，✅ 已发版）· 设备接入与数据承接（多设备 Harness 中间层 + router 伴生数据面）· MCP 95→104 tools · 测试 4429→4805 · acceptance 337→352。此前版本详见 [CHANGELOG](../CHANGELOG.md) 与各版开发日志。
...
| 版本 | 状态 | 核心交付 | 日志 |
|------|:--:|------|:--:|
| **v1.5.0** | 📋 规划中（已开发完成·待发版） | **🛡️ 治理模块 · 可见性与本体成熟**：治理 KPI 面板（治理 tab 六卡 + 数据集审阅 + lineage 合规报告 + 周报）· 本体数据双时态 + 三层渐进加载 · Ontology Validation Engine · 跨层证据对账 trace_reconcile · FDE 陪跑期 · 存量清扫 + @sofagent/inject 更名 · DSH 插件事件接线。*T8/T9 管线接线已顺延 v1.5.1 第六章* | [日志](./changelog/v1.5/v1.5.0.md) |
| **v1.5.1** | 📋 规划中 | **⚡ 编排模块 · 事件驱动**：事件驱动触发（`on:` 声明式订阅 + 死信重放）· 理解债务应对 · AI 异常处理总线 · G12 设备 OTA · 任务下发通道二期 · T8/T9 生产管线接线（自 v1.5.0 顺延）· 审计输入双通道 | [日志](./changelog/v1.5/v1.5.1.md) |
| **v1.5.2** | 📋 规划中 | **🔍 审计模块 · 开放与治理面**：audit 数据对外（只读查询 + 事件订阅）· should-run 判定链 · doctor 修复闭环 · 约束导出与证据链外部可验（独立验签工具）· 双规则引擎统一 · 审计规则自测 schema · A24 交付物落点规则 · 审计结论失效语义 | [日志](./changelog/v1.5/v1.5.2.md) |
| **v1.5.3** | 📋 规划中 | **⚡ 执行模块 · 路由与验证**：模型路由层（云端规划/本地执行 + routeReason 可解释）· 凭证隔离 Vault · 多实例自验证 · 全节点执行状态机 · tool search 按需加载 · CLI 单入口收敛（13 bin → 域路由） | [日志](./changelog/v1.5/v1.5.3.md) |
