...
> v1.3.1 · 2026-08-09（UTC）· 🛡️ 运行时审计最小闭环 + 激活链 Phase 4 收尾 + 决策审计

产品定位详见 [设计哲学](./PHILOSOPHY.md) 和 [README](../README.md)。

## 现在在哪：v1.3.1（已发布 · 2026-08-09）
...
| **v1.4.1** | 📋 规划中 | **🚀 训练引擎（把「后训练」变成可交付能力 · AIR 支撑 · 训练相关内容统一从此开始）**：① **双栈架构决策**——Node 控制面 + Python 执行面（verl/TRL/NeMo RL 均 Python；个人 Mac 阶段 0 用 @mlx-node/trl 纯 Node 验证）② **train-job 编排层**（train_submit / 监控 / 取消 / checkpoint 续跑，状态机 + 幂等）③ **企业数据 → 训练集管道**（CSV/Excel 异构接入 + instruction/偏好对构建 + 质量闸门 + 训练入口脱敏）④ **训练中 eval 闭环**（复用 v1.3.1 Benchmark，阈值外部化——机制开源/阈值闭源）⑤ **train_job 审计**（训练本身可审计可回滚 + 数据源 hash 溯源）⑥ **训练沙箱 + 设备打包**（扩展 v1.3.7，数据不出边界 + 离线可训 + E4 前置）⑦ **训练需求推导方法论**（`train analyze`：workflow 节点 → 训练目标/数据需求/评估标准/QLoRA 配置）⑧ **训练语料导出三件套**（规则 + GUIDE 方法论 + 样本四源 [decision-log/llm-calls/evaluation-log/runtime-audit] + Trace 轨迹 + 通用脱敏管线 + HMAC 签名 + 合规红线——含 human-fde 人工基准，从 v1.3.2/v1.3.6 归集）⑨ **C13 本地权重部署链路**（权重目录规范 + 本地加载 + 版本回滚，从 v1.3.6 归集） | [日志](./changelog/v1.4/v1.4.1.md) |
...
| **v1.3.9** | **🛠️ AST 规则引擎 + meta-harness + 📊 工作明细数据层** | `sofagent-ruleset-ast` 参考实现 + 多 harness 统一编排 + AI 工作明细（`worklog_query` + 落盘 `worklog.json`，**补节点实际耗时采集，AIR C8 输入**，终端 ASCII 可选）（详见 开发日志 ./changelog/v1.3/v1.3.9.md）|
| **v1.4.0** | **📊 Web 工作明细页（版本号重新启用）** | `dashboard.html` 加「工作明细」区块，读 `worklog.json`（v1.3.9 落盘）（详见 开发日志 ./changelog/v1.4/v1.4.0.md）|
| **v1.4.1** | **🚀 训练引擎（AIR 支撑 · 训练相关内容统一从此开始）** | 双栈决策（Node 控制面 + Python 执行面，Mac 阶段 0 用 @mlx-node/trl）+ train-job 编排 + 数据→训练集管道 + 训练中 eval 闭环 + train_job 审计 + 训练沙箱/设备打包 + 训练需求推导 + 语料导出三件套（从 v1.3.x 归集）+ C13 本地权重部署（从 v1.3.6 归集）（详见 开发日志 ./changelog/v1.4/v1.4.1.md）|
