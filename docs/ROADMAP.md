...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 [v1.5.4](./changelog/v1.5/v1.5.4.md) 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed + **判据质量 lint**（硬项 fail-closed：正负样例矛盾 / 证据不可复现；软项只提示不阻断：可校准性 / 判据重复度）+ 理念升级驱动的文档重构机制立规（触发条件 / 分层口径 / 版本关系纪律）+ **判定底座边界声明**（做马鞍不做马场——四判据 + 边界外清单 + 边界内六项落点）。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本。**形态归属**：主干 3 章（三原语类型化 / 24 条规则判据重述 / 判定底座边界声明）+ 非功能 1 章（文档重构机制立规） | [日志](./changelog/v1.6/v1.6.0.md) |
...
| **v1.6.2** | 📋 规划中 | **🧪 判定底座 · 阶段一收口 · 编码器基座与零样本基线**：**encoder-only** 基座接入（非生成路线）+ 类型化判定头（choice/score/noul 分头共享编码器）+ 多问并行（一次编码多头出答案）+ 判定不占本地主模型槽位（行为锁）+ 零新增运行时组件 + 零样本评测器（逐判据准确率 + **失败模式分类**）+ 校准现状探测（分桶可靠图 + ECE）+ **中文零样本基线**（中英分列落数不合并 / 判据集中文版语义等价非机翻 / 失败模式按语言分报）+ **基线如实落数不修饰**。**产物性质**：这是基线不是成果——阶段一累计产物为「判据+数据+基线」三件内部资产，对外口径零变化。**形态归属**：主干 2 章（编码器与判定头接线 / 零样本基线落数） | [日志](./changelog/v1.6/v1.6.2.md) |
...
| **v1.7.1** | 📋 规划中 | **📐 判定底座 · 阶段二 · 校准（可用性硬门槛）**：分桶温度拟合（粒度可配）+ 三原语分别校准（`score` 档**保序**）+ 校准集隔离（独立于训练集，按轨迹/租户划分）+ 校准参数独立制品（与权重解耦）+ 三态对照（零样本/训练后/校准后）+ **ECE 达阈为验收硬线**（未达阈本版不通过）+ **校准判据纪律从设计承诺落地为门禁**（决策路径上未标注校准依据的阈值 → 断言失败）+ 校准漂移监测（只提示不阻断）+ 既有置信度普查登记 + **弃权语义独立化**（弃权为一等输出 / 0.5 是真值概率非「中等可信」 / 三源分别留痕 / 弃权率可观测）+ **校准上线前置门**（ECE 未达阈时弃权默认开启且不可关闭；新增判据域弃权带默认最宽）。**形态归属**：主干 3 章（校准实现 / 校准判据纪律落地 / 弃权语义与校准上线前置门） | [日志](./changelog/v1.7/v1.7.1.md) |
...
| **v1.8.1** | 📋 规划中 | **🖥️ 判定底座 · 阶段三 · 一体机推理接入**：**并行约束解码**（同状态全部问句一次前向出答案，KV cache 广播 + 单次前向填充全部字段）+ **类型约束解码**（解码层保证类型正确，与 schema 层双重保证）+ 独立问句并行/有依赖问句串行（依赖显式声明，不自动推断）+ 判定独立槽位通道（不与主模型抢槽）+ 离线可用（不联络外部服务）+ 延迟/吞吐/并发劣化曲线落数 + **路由判定档（`decision-model`）从插槽接通为可达**（不进自动矩阵 / 绝不落云端生成分支 / 可观测）。**形态归属**：主干 2 章（并行约束解码 / 路由判定档接通） | [日志](./changelog/v1.8/v1.8.1.md) |
