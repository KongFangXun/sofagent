...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 [v1.5.4](./changelog/v1.5/v1.5.4.md) 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed + **判据质量 lint**（硬项 fail-closed：正负样例矛盾 / 证据不可复现 / **概率来源不可追溯**；软项只提示不阻断：可校准性 / 判据重复度）+ 理念升级驱动的文档重构机制立规（触发条件 / 分层口径 / 版本关系纪律）+ **判定底座边界声明**（做马鞍不做马场——四判据 + 边界外清单 + 边界内六项落点）。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本。**形态归属**：主干 3 章（三原语类型化 / 24 条规则判据重述 / 判定底座边界声明）+ 非功能 1 章（文档重构机制立规） | [日志](./changelog/v1.6/v1.6.0.md) |
...
| **v1.6.2** | 📋 规划中 | **🧪 判定底座 · 阶段一收口 · 编码器基座与零样本基线**：**encoder-only** 基座接入（非生成路线）+ 类型化判定头（choice/score/noul 分头共享编码器）+ 多问并行（一次编码多头出答案）+ 判定不占本地主模型槽位（行为锁）+ 零新增运行时组件 + 零样本评测器（逐判据准确率 + **失败模式分类**）+ 校准现状探测（分桶可靠图 + ECE）+ **中文零样本基线**（中英分列落数不合并 / 判据集中文版语义等价非机翻 / 失败模式按语言分报）+ **分布内外分列**（演示任务族 vs 从未训练的任务类型，落差约 24 个百分点的量级须显式落数）+ **基线如实落数不修饰**。**产物性质**：这是基线不是成果——阶段一累计产物为「判据+数据+基线」三件内部资产，对外口径零变化。**形态归属**：主干 2 章（编码器与判定头接线 / 零样本基线落数） | [日志](./changelog/v1.6/v1.6.2.md) |
...
| **v1.8.0** | 📋 规划中 | **🔌 判定底座 · 阶段三（接线与分发）· DecisionChannel 真实实现**：以阶段一/二产物作 `DecisionChannel` **默认实现**（填 [v1.5.6](./changelog/v1.5/v1.5.6.md) 留空）+ 契约可插拔测试（换实现不动契约）+ **L1 判定层从「恒返回未就位」切换为真实可用**（**保留未就位兜底路径**——不留空窗、不静默放行）+ 云端 provider 默认关闭（opt-in 留痕）+ 失败语义 fail-closed（失败退 L0）+ 判定结果落 decision-log（含概率+四态+routeReason）+ 判定不占主模型槽位（行为锁）+ **判定面输入契约**（state 构造唯一入口 / 最小可判定前缀 / 定序去噪 / 协议结构保留 / 超限不静默截断 / 输入与留痕同源）。**形态归属**：主干 2 章（DecisionChannel 实现落地 / 判定面输入契约） | [日志](./changelog/v1.8/v1.8.0.md) |
| **v1.8.1** | 📋 规划中 | **🖥️ 判定底座 · 阶段三 · 一体机推理接入**：**并行约束解码**（同状态全部问句一次前向出答案，KV cache 广播 + 单次前向填充全部字段）+ **类型约束解码**（解码层保证类型正确，与 schema 层双重保证）+ 独立问句并行/有依赖问句串行（依赖显式声明，不自动推断）+ 判定独立槽位通道（不与主模型抢槽）+ 离线可用（不联络外部服务）+ **实现路线不绑定**（契约由 DecisionChannel 保证，并行约束解码 / 离散扩散去噪 / 零微调均属实现面，契约测试以接口为准）+ **超词表上界的选项集分块**（分块打分 + 末轮归一 + 「以上都不是」槽，候选集不静默截断）+ 延迟/吞吐/并发劣化曲线落数 + **路由判定档（`decision-model`）从插槽接通为可达**（不进自动矩阵 / 绝不落云端生成分支 / 可观测）。**形态归属**：主干 2 章（并行约束解码 / 路由判定档接通） | [日志](./changelog/v1.8/v1.8.1.md) |
