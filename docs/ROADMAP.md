...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 [v1.5.4](./changelog/v1.5/v1.5.4.md) 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本 | [日志](./changelog/v1.6/v1.6.0.md) |
| **v1.6.1** | 📋 规划中 | **🧱 判定底座 · 阶段一 · 数据集三元组与三源采集**：`(state, questions, answers)` 三元组格式 + **概率目标表达**（分布/区间/真值概率——硬标签训不出校准，格式层预留）+ `label.source` 三档（`deterministic`/`model-judge`/`human`，对齐 [v1.5.14](./changelog/v1.5/v1.5.14.md) 域验证器口径）+ history/HITL/PR 三源采集器 + 来源真实性门（未建链/合成来源 fail-closed 拒收，防自证污染）+ 租户隔离 + 去重与**跨集泄漏防护**（按轨迹粒度切分）+ 采集可观测 | [日志](./changelog/v1.6/v1.6.1.md) |
| **v1.6.2** | 📋 规划中 | **🧪 判定底座 · 阶段一收口 · 编码器基座与零样本基线**：**encoder-only** 基座接入（非生成路线）+ 类型化判定头（choice/score/noul 分头共享编码器）+ 多问并行（一次编码多头出答案）+ 判定不占本地主模型槽位（行为锁）+ 零新增运行时组件 + 零样本评测器（逐判据准确率 + **失败模式分类**）+ 校准现状探测（分桶可靠图 + ECE）+ **基线如实落数不修饰**。**产物性质**：这是基线不是成果——阶段一累计产物为「判据+数据+基线」三件内部资产，对外口径零变化 | [日志](./changelog/v1.6/v1.6.2.md) |
| **v1.7.0** | 📋 规划中 | **🎯 判定底座 · 阶段二（模型与校准）· RLCD 训练复现**：严格适当评分规则（`Noul` 用 log score / `Choice` 用 log score + spherical / `Score` 用 RPS 保序）+ 策略梯度训练回路（**无人类偏好模型、无外部教师模型**——奖励全来自评分规则）+ 消费概率目标而非硬标签 + `label.source` 档位加权 + 默认云端全参数微调（LoRA 档仅作客户要求本地训练备选，有效性须实测）+ TrainChannel 提交（不写云厂商适配器）+ 训练可复现清单 | [日志](./changelog/v1.7/v1.7.0.md) |
| **v1.7.1** | 📋 规划中 | **📐 判定底座 · 阶段二 · 校准（可用性硬门槛）**：分桶温度拟合（粒度可配）+ 三原语分别校准（`score` 档**保序**）+ 校准集隔离（独立于训练集，按轨迹/租户划分）+ 校准参数独立制品（与权重解耦）+ 三态对照（零样本/训练后/校准后）+ **ECE 达阈为验收硬线**（未达阈本版不通过）+ **校准判据纪律从设计承诺落地为门禁**（决策路径上未标注校准依据的阈值 → 断言失败）+ 校准漂移监测（只提示不阻断）+ 既有置信度普查登记 | [日志](./changelog/v1.7/v1.7.1.md) |
| **v1.7.2** | 📋 规划中 | **🧮 判定底座 · 阶段二收口 · 三原语概率面与基线对照**：三原语完整输出面（choice 分布+熵 / score 区间+序 / noul 概率+**四态映射**）+ 建议动作**不具执法力**（执行权归既有规则与审计层）+ **证据面不扩大**（只提高已获证据利用率，不新增采集）+ 输出直落 decision-log + 三态对照落数 + **判定质量与概率可信度分列报告**（禁合并单一分数）+ 失败模式复扫（已解决/部分改善/未改善）+ 阶段二收口判据（双达阈才启动阶段三） | [日志](./changelog/v1.7/v1.7.2.md) |
| **v1.8.0** | 📋 规划中 | **🔌 判定底座 · 阶段三（接线与分发）· DecisionChannel 真实实现**：以阶段一/二产物作 `DecisionChannel` **默认实现**（填 [v1.5.6](./changelog/v1.5/v1.5.6.md) 留空）+ 契约可插拔测试（换实现不动契约）+ **L1 判定层从「恒返回未就位」切换为真实可用**（**保留未就位兜底路径**——不留空窗、不静默放行）+ 云端 provider 默认关闭（opt-in 留痕）+ 失败语义 fail-closed（失败退 L0）+ 判定结果落 decision-log（含概率+四态+routeReason）+ 判定不占主模型槽位（行为锁） | [日志](./changelog/v1.8/v1.8.0.md) |
| **v1.8.1** | 📋 规划中 | **🖥️ 判定底座 · 阶段三 · 一体机推理接入**：**并行约束解码**（同状态全部问句一次前向出答案，KV cache 广播 + 单次前向填充全部字段）+ **类型约束解码**（解码层保证类型正确，与 schema 层双重保证）+ 独立问句并行/有依赖问句串行（依赖显式声明，不自动推断）+ 判定独立槽位通道（不与主模型抢槽）+ 离线可用（不联络外部服务）+ 延迟/吞吐/并发劣化曲线落数 + **路由判定档（`decision-model`）从插槽接通为可达**（不进自动矩阵 / 绝不落云端生成分支 / 可观测） | [日志](./changelog/v1.8/v1.8.1.md) |
| **v1.8.2** | 📋 规划中 | **📦 判定底座 · 阶段三收口 · 权重分发通路与自带件形态成立（含叙事章）**：`weights-manifest` 补 `remote` 字段（**纯增量**，既有五环零破坏）+ 下载件（断点续传/重试/进度，接续 v1.4.2 排期）+ **sha256 篡改检测 fail-closed**（不匹配拒绝启用并留痕）+ 来源白名单（防供应链投毒）+ 多版本并存与回滚（复用既有 `model_switch`）+ **三层分发物**（代码包 / 规则资产 / 模型制品，权重版本与代码版本松耦合——契约由 `DecisionChannel` 保证非版本号）· **自带件形态成立**（实现+离线推理+分发通路三要素齐备，本版**首次**具备对外叙事资格；仍不得宣称准确率——证据链属 v1.9.1） | [日志](./changelog/v1.8/v1.8.2.md) |
| **v1.9.0** | 📋 规划中 | **🧩 判定底座 · 阶段四（消费与收口）· 既有判定面收敛与假概率正名**：三原语既有实现补概率（`fde_classify` 补全候选分布 / `evaluate`+`quality-judge` 补可信区间 / `shouldAllow` 并列输出概率而**布尔执行语义零改动**）+ 判定源统一路由到判定底座（**降级路径保留**，底座不可用时行为与今日一致）+ 24–25 条审计规则判定面**零改动** + **假概率正名**（实测普查：引擎既有置信度数值无一是校准概率——硬编码常数正名为**启发式权重**、枚举映射数值保留**序数等级**语义；**全仓 `probability` 字段零命中**）+ 数值携带语义标记（概率/权重/序数）+ **不静默改行为**。**边界**：本版产出是「诚实」而非「更强」 | [日志](./changelog/v1.9/v1.9.0.md) |
| **v1.9.1** | 📋 规划中 | **📊 判定底座 · 阶段四 · 判据面扩展与四层性能标准（含叙事章）**：判据面从审计规则扩至 **workflow / 本体数据 / 路由判定**三域 + 各域校准状态显式声明（未校准域输出**不进决策路径**）+ 未验证域（开放式/主观/长链推理）显式声明不宣称能力 + **四层性能标准**（判据级准确率 / 校准级 ECE / **分流级三段门控真实错误率** / 端到端延迟吞吐）**分层落数分列报告**（禁压成单一分数）+ **第四方对照**（高能力本地档 / 大参数云端档 / 云端快档 / 同类开源判定件，同判据集同评测器，四维并列，第三方数字标注口径）。**纪律**：不比通用 benchmark | [日志](./changelog/v1.9/v1.9.1.md) |
| **v1.9.2** | 📋 规划中 | **🏁 判定底座 · 施工期收口与全站口径对齐（含叙事章）**：四阶段收口判据逐项核对（达阈/未达阈如实标注）+ 两条分支（全部达阈 → 进 V2.0；任一未达 → **施工期内增补细版**，不带着未达阈项进 V2.0）+ 全站口径对齐（术语统一 / 判定面定位 / 边界如实 / 制品架构表述）· **版本弹性机制落盘**：阶段内可增补细版（`vX.Y.3+`）不挤后续阶段；新工作面**插入新阶段**（如 `v1.10.x`）保持原编号不重排；**V2.0 时点不锁死**（由「四阶段收口达阈」决定，不由版本号决定）。**不做**：产品身份切换（属 V2.0，本版只铺平口径） | [日志](./changelog/v1.9/v1.9.2.md) |
