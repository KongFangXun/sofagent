...
> **v1.3.9 已于 2026-08-21 交付**——官方 AST 规则引擎（sofagent-ruleset-ast 含 ASI01/ASI04）+ meta-harness 多 harness 统一编排（DSH 形态对齐）+ AI 工作明细数据层（worklog + worklog_query MCP）+ API 分级 @public/@internal（1449 符号 + CI 门禁）+ FORGE driver 切 DSH（显式后端选择 + CLI 桥接 + bash 全权限）+ MLflow agent 评估 + Agentic Browser + 跨平台适配器（Cursor/Codex/Gemini）+ tools/ 物理分子目录 + ATTRIBUTION 归因引擎 + Dream Sandbox 沙盒审计 + >5MB diff 缝隙修复 + FORGE driver 进程守护（daemon + watcher）。详见 [v1.3.9 开发日志](./changelog/v1.3/v1.3.9.md)。
>
...
| 版本 | 核心交付 |
|------|------|
| **v1.3.9** | 🔍 官方 AST 规则引擎（sofagent-ruleset-ast 含 ASI01/ASI04 · 8+2 规则）+ 🧩 meta-harness 多 harness 统一编排（DSH 形态对齐）+ 📊 AI 工作明细数据层（worklog + worklog_query MCP）+ 🔬 API 分级 @public/@internal（1449 符号 + CI 门禁）+ ⚙️ FORGE driver 切 DSH（显式后端选择 + CLI 桥接 + bash 全权限）+ 📈 MLflow agent 评估（13 指标 + LLM-as-Judge）+ 🌐 Agentic Browser（4 工具 + 视觉降级）+ 🧭 跨平台适配器（Cursor/Codex/Gemini CLI）+ 🗂️ tools/ 物理分子目录 + 🏷️ ATTRIBUTION 归因引擎 + 🏖️ Dream Sandbox 沙盒审计 + 🩹 >5MB diff 缝隙修复（spill 落盘）+ 🔄 FORGE driver 进程守护（daemon + watcher）· 阶段五~八全流程（15 acceptance 场景补齐 + release-gate 三跑 PASS） |
...
| **插件市场企业管控（Codex Harness 启发 · 2026-08-22 新增）** | Codex 的 marketplace.json 兼容 `.claude-plugin` / `.cursor-plugin` 多格式 + 企业 allowlist/restricted 策略（`MarketplaceAllowedSource`：Git URL / GitHostPattern / Local 三类来源白名单 + `allow_managed_hooks_only` 托管 hook 独裁模式）——sofagent 的 ClawHub/SkillHub 分发目前无企业级「谁能装、从哪装」管控；评估把插件来源白名单 + 托管 hook 模式引入企业部署（install.sh 企业侧）。**候选版本：v1.4.x 尾段**（插件家族交付后自然延伸） |
| **规则即测试：match/not_match 内嵌单元测试（Codex execpolicy 启发 · 2026-08-22 新增）** | Codex execpolicy 的 `prefix_rule(pattern, decision, justification, match, not_match)`——规则自带示例即单元测试（加载时校验），`decision` 三态 allow/prompt/forbidden + `justification` 人类可读理由。sofagent 审计规则（A1-A23）目前纯代码判定，借鉴点：① 规则加 match/not_match 示例（进 acceptance 自动回归）② justification 字段（拦截时给人类可读理由，替代现在的规则名硬提示）。**候选版本：v1.4.x** |
| **审计留痕格式：raw trace + reduced state 双层（Codex rollout-trace 启发 · 2026-08-22 新增）** | Codex 的 rollout-trace 分「raw bundle（原始事件流，writer 热路径只写不读）+ reduced state（语义规约缓存，供 replay/viewer 消费）」两层——与 sofagent「Ledger 原始不可变 + Views 派生」同构，但 Codex 把「raw 写入器」和「语义回放器」物理拆成两个 crate（热路径零负担）。借鉴点：审计历史（history.jsonl）的读取端拆出独立规约层，避免「写多读少」的单一文件性能瓶颈。**候选版本：v1.4.x**（与 SQLite 迁移评估同批） |
| **进程硬化四平台（Codex process-hardening 启发 · 2026-08-22 新增）** | `pre_main_hardening()` 在 main 前执行：禁 core dump / 禁 ptrace attach（Linux+macOS）/ 清 LD_PRELOAD+DYLD_* 危险环境变量，Linux/Android/macOS/BSD/Windows 全平台覆盖。sofagent 的 daemon 常驻进程未做此类硬化——评估在 daemon 启动段补（禁 core dump + 清 LD_PRELOAD），低成本高安全收益。**候选版本：v1.4.x** |
| **脱敏清单对照（Codex secrets sanitizer 启发 · 2026-08-22 新增）** | Codex `redact_secrets` 用 4 组正则（Bearer / sk- / AKIA / api_key|token|secret|password 赋值），sofagent A2 密钥规则 + sanitize() 管道功能重叠——对照补漏：`SECRET_ASSIGNMENT_REGEX`（`api_key = "..."` 通用形态）若 sofagent 未覆盖则补 A2 检测面。**候选版本：v1.4.x**（A2 增强） |
