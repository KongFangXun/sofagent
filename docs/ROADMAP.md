...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 v1.5.2 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本 | [日志](./changelog/v1.6/v1.6.0.md) |
| **v1.6.1** | 📋 规划中 | **🧱 判定底座 · 阶段一 · 数据集三元组与三源采集**：`(state, questions, answers)` 三元组格式 + **概率目标表达**（分布/区间/真值概率——硬标签训不出校准，格式层预留）+ `label.source` 三档（`deterministic`/`model-judge`/`human`，对齐 v1.5.5 域验证器口径）+ history/HITL/PR 三源采集器 + 来源真实性门（未建链/合成来源 fail-closed 拒收，防自证污染）+ 租户隔离 + 去重与**跨集泄漏防护**（按轨迹粒度切分）+ 采集可观测 | [日志](./changelog/v1.6/v1.6.1.md) |
...
| **v1.8.0** | 📋 规划中 | **🔌 判定底座 · 阶段三（接线与分发）· DecisionChannel 真实实现**：以阶段一/二产物作 `DecisionChannel` **默认实现**（填 v1.5.3 留空）+ 契约可插拔测试（换实现不动契约）+ **L1 判定层从「恒返回未就位」切换为真实可用**（**保留未就位兜底路径**——不留空窗、不静默放行）+ 云端 provider 默认关闭（opt-in 留痕）+ 失败语义 fail-closed（失败退 L0）+ 判定结果落 decision-log（含概率+四态+routeReason）+ 判定不占主模型槽位（行为锁） | [日志](./changelog/v1.8/v1.8.0.md) |
