...
| **v1.6.1** | 📋 规划中 | **🧱 判定底座 · 阶段一 · 数据集三元组与三源采集**：`(state, questions, answers)` 三元组格式 + **概率目标表达**（分布/区间/真值概率——硬标签训不出校准，格式层预留）+ `label.source` 三档（`deterministic`/`model-judge`/`human`，对齐 [v1.5.14](./changelog/v1.5/v1.5.14.md) 域验证器口径）+ history/HITL/PR 三源采集器 + 来源真实性门（未建链/合成来源 fail-closed 拒收，防自证污染）+ 租户隔离 + 去重与**跨集泄漏防护**（按轨迹粒度切分）+ 采集可观测 + **八类困难样本采集点**（弃权记录 / 上下文取舍 / 路由对比 / 措辞不变性 / OOD / 漂移探测 / 对抗 / 对齐分歧）+ **四道采集质量门**（来源真实性 / 跨集泄漏 / 标签噪声对账 / 校准标注）+ **主动学习四类选样**（不确定 / 分歧 / 代表 / 漂移优先；中文优先与人审优先纪律）。**形态归属**：主干 3 章（数据集三元组格式 / 三源采集器与八类补充采集点 / 采集质量门与主动学习选样）+ 通道成分 1（采集与落盘的执行动作） | [日志](./changelog/v1.6/v1.6.1.md) |
...
| **v1.7.0** | 📋 规划中 | **🎯 判定底座 · 阶段二（模型与校准）· RLCD 训练复现**：严格适当评分规则（`Noul` 用 log score / `Choice` 用 log score + spherical / `Score` 用 RPS 保序）+ 策略梯度训练回路（**无人类偏好模型、无外部教师模型**——奖励全来自评分规则）+ 消费概率目标而非硬标签 + `label.source` 档位加权 + 默认云端全参数微调（LoRA 档仅作客户要求本地训练备选，有效性须实测）+ TrainChannel 提交（不写云厂商适配器）+ 训练可复现清单。**形态归属**：主干 1 章（训练目标与评分规则） | [日志](./changelog/v1.7/v1.7.0.md) |
| **v1.7.1** | 📋 规划中 | **📐 判定底座 · 阶段二 · 校准（可用性硬门槛）**：分桶温度拟合（粒度可配）+ 三原语分别校准（`score` 档**保序**）+ 校准集隔离（独立于训练集，按轨迹/租户划分）+ 校准参数独立制品（与权重解耦）+ 三态对照（零样本/训练后/校准后）+ **ECE 达阈为验收硬线**（未达阈本版不通过）+ **校准判据纪律从设计承诺落地为门禁**（决策路径上未标注校准依据的阈值 → 断言失败）+ 校准漂移监测（只提示不阻断）+ 既有置信度普查登记 + **弃权语义独立化**（弃权为一等输出 / 0.5 是真值概率非「中等可信」 / 三源分别留痕 / 弃权率可观测）+ **弃权之外的稳定化重读**（逐问句看熵、超阈带新噪声重读取平均；默认关闭且校准须同口径重测）+ **校准上线前置门**（ECE 未达阈时弃权默认开启且不可关闭；新增判据域弃权带默认最宽）。**形态归属**：主干 3 章（校准实现 / 校准判据纪律落地 / 弃权语义与校准上线前置门） | [日志](./changelog/v1.7/v1.7.1.md) |
...
| **v1.8.1** | 📋 规划中 | **🖥️ 判定底座 · 阶段三 · 一体机推理接入**：**并行约束解码**（同状态全部问句一次前向出答案，KV cache 广播 + 单次前向填充全部字段）+ **类型约束解码**（解码层保证类型正确，与 schema 层双重保证）+ 独立问句并行/有依赖问句串行（依赖显式声明，不自动推断）+ 判定独立槽位通道（不与主模型抢槽）+ 离线可用（不联络外部服务）+ **实现路线不绑定**（契约由 DecisionChannel 保证，并行约束解码 / 离散扩散去噪 / 零微调均属实现面，契约测试以接口为准）+ **超词表上界的选项集分块**（分块打分 + 末轮归一 + 「以上都不是」槽，候选集不静默截断）+ **职责频率分层**（延迟即职责边界，判定永不进控制回路与安全反射；无 GPU 的 CPU 档为最低可用档）+ 延迟/吞吐/并发劣化曲线落数 + **路由判定档（`decision-model`）从插槽接通为可达**（不进自动矩阵 / 绝不落云端生成分支 / 可观测）。**形态归属**：主干 2 章（并行约束解码 / 路由判定档接通） | [日志](./changelog/v1.8/v1.8.1.md) |
...
| **v1.9.1** | 📋 规划中 | **📊 判定底座 · 阶段四 · 判据面扩展与四层性能标准（含叙事章）**：判据面从审计规则扩至 **workflow / 本体数据 / 路由判定**三域 + 各域校准状态显式声明（未校准域输出**不进决策路径**）+ 未验证域（开放式/主观/长链推理）显式声明不宣称能力 + **四层性能标准**（判据级准确率 / 校准级 ECE / **分流级三段门控真实错误率** / 端到端延迟吞吐）**分层落数分列报告**（禁压成单一分数）+ **第四方对照**（高能力本地档 / 大参数云端档 / 云端快档 / 同类开源判定件，同判据集同评测器，四维并列，第三方数字标注口径）+ **静默失败监控**（四探针：输出分布漂移 / 概率分布异常 / 弃权率突变 / 输入长度-延迟相关——超阈告警不阻断但必须可见）+ **判定使用度量**（采纳率 / ASK / DENY / 被人工推翻率——「无人消费」与「被推翻」必须可区分）+ **压缩留痕完整性**（压缩前的判据输入引用可还原，摘要不能替代）+ **公开独立实测存档**（独立机构实测商业判定件：777 次判断 < 0.7 s、成本约四分之一美分、**准确率比前沿模型低一档**；开源复刻作者在一体机同款机型上的对照：扩散路线 198/201 vs 商业件 191/201——两条评测集均系对方自建，只并列不排名）+ **分布内外分列与不利结论照录** + **同集可比对**（唯一同集同口径的一对公开数字：某公开评测严格公共子集 343 对上商业件 86.9% vs 开源复刻 76.7%）+ **可复算产物**（每个数字附脚本与结果文件，或标注单次运行口径）+ **判定调用密度**（每任务判定次数）。**纪律**：不比通用 benchmark。**形态归属**：主干 4 章（判据面扩展 / 四层性能标准 / 第四方对照 / 静默失败监控与判定使用度量） | [日志](./changelog/v1.9/v1.9.1.md) |
