...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 [v1.5.4](./changelog/v1.5/v1.5.4.md) 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed + 理念升级驱动的文档重构机制立规（触发条件 / 分层口径 / 版本关系纪律）。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本。**形态归属**：主干 2 章（三原语类型化 / 24 条规则判据重述）+ 非功能 1 章（文档重构机制立规） | [日志](./changelog/v1.6/v1.6.0.md) |
| **v1.6.1** | 📋 规划中 | **🧱 判定底座 · 阶段一 · 数据集三元组与三源采集**：`(state, questions, answers)` 三元组格式 + **概率目标表达**（分布/区间/真值概率——硬标签训不出校准，格式层预留）+ `label.source` 三档（`deterministic`/`model-judge`/`human`，对齐 [v1.5.14](./changelog/v1.5/v1.5.14.md) 域验证器口径）+ history/HITL/PR 三源采集器 + 来源真实性门（未建链/合成来源 fail-closed 拒收，防自证污染）+ 租户隔离 + 去重与**跨集泄漏防护**（按轨迹粒度切分）+ 采集可观测。**形态归属**：主干 2 章（数据集三元组格式 / 三源采集器）+ 通道成分 1（采集与落盘的执行动作） | [日志](./changelog/v1.6/v1.6.1.md) |
| **v1.6.2** | 📋 规划中 | **🧪 判定底座 · 阶段一收口 · 编码器基座与零样本基线**：**encoder-only** 基座接入（非生成路线）+ 类型化判定头（choice/score/noul 分头共享编码器）+ 多问并行（一次编码多头出答案）+ 判定不占本地主模型槽位（行为锁）+ 零新增运行时组件 + 零样本评测器（逐判据准确率 + **失败模式分类**）+ 校准现状探测（分桶可靠图 + ECE）+ **基线如实落数不修饰**。**产物性质**：这是基线不是成果——阶段一累计产物为「判据+数据+基线」三件内部资产，对外口径零变化。**形态归属**：主干 2 章（编码器与判定头接线 / 零样本基线落数） | [日志](./changelog/v1.6/v1.6.2.md) |
...
| **v1.7.1** | 📋 规划中 | **📐 判定底座 · 阶段二 · 校准（可用性硬门槛）**：分桶温度拟合（粒度可配）+ 三原语分别校准（`score` 档**保序**）+ 校准集隔离（独立于训练集，按轨迹/租户划分）+ 校准参数独立制品（与权重解耦）+ 三态对照（零样本/训练后/校准后）+ **ECE 达阈为验收硬线**（未达阈本版不通过）+ **校准判据纪律从设计承诺落地为门禁**（决策路径上未标注校准依据的阈值 → 断言失败）+ 校准漂移监测（只提示不阻断）+ 既有置信度普查登记。**形态归属**：主干 2 章（校准实现 / 校准判据纪律落地） | [日志](./changelog/v1.7/v1.7.1.md) |
...
| **v1.8.0** | 📋 规划中 | **🔌 判定底座 · 阶段三（接线与分发）· DecisionChannel 真实实现**：以阶段一/二产物作 `DecisionChannel` **默认实现**（填 [v1.5.6](./changelog/v1.5/v1.5.6.md) 留空）+ 契约可插拔测试（换实现不动契约）+ **L1 判定层从「恒返回未就位」切换为真实可用**（**保留未就位兜底路径**——不留空窗、不静默放行）+ 云端 provider 默认关闭（opt-in 留痕）+ 失败语义 fail-closed（失败退 L0）+ 判定结果落 decision-log（含概率+四态+routeReason）+ 判定不占主模型槽位（行为锁）。**形态归属**：主干 1 章（DecisionChannel 实现落地） | [日志](./changelog/v1.8/v1.8.0.md) |
...
| **v1.9.1** | 📋 规划中 | **📊 判定底座 · 阶段四 · 判据面扩展与四层性能标准（含叙事章）**：判据面从审计规则扩至 **workflow / 本体数据 / 路由判定**三域 + 各域校准状态显式声明（未校准域输出**不进决策路径**）+ 未验证域（开放式/主观/长链推理）显式声明不宣称能力 + **四层性能标准**（判据级准确率 / 校准级 ECE / **分流级三段门控真实错误率** / 端到端延迟吞吐）**分层落数分列报告**（禁压成单一分数）+ **第四方对照**（高能力本地档 / 大参数云端档 / 云端快档 / 同类开源判定件，同判据集同评测器，四维并列，第三方数字标注口径）。**纪律**：不比通用 benchmark。**形态归属**：主干 3 章（判据面扩展 / 四层性能标准 / 第四方对照） | [日志](./changelog/v1.9/v1.9.1.md) |
