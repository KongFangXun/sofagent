...
| **v1.4.7** | 📋 规划中 | **🔌 商业平台接口版**：G2 能力缺口查询 · G4 绩效数据导出 · G6 节点可见性元数据 · G7 多租户数据路径 v0 · workflow 烧进 USB · 审计留痕双层 · 静态加密全量接线 · G8 首部署 cron job 包（**本版最高优先级**）· **G13 PR 生命周期接口**（MCP `pr_submit`/`pr_review`/`pr_merge`——PR 提交→审阅→合并/拒绝动作面写接口，PR 状态机 + merge_criteria 联动 + 拒绝留痕进 decision-log 训练信号，2026-09-01 收编缺口 A）· **G7 补数字员工组织身份字段**（identity-store/workflow 元数据加 `orgId`——字段与策略分离，G4 绩效查询按 org 过滤防跨租户泄漏，2026-09-01 拍板提前自 v2.x）。子项定义见 [v1.4.7 规划文件](./changelog/v1.4/v1.4.7.md)；商业侧规划独立于本仓库维护 | [日志](./changelog/v1.4/v1.4.7.md) |
