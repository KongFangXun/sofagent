...
| **v1.3.9** | **🛠️ AST 规则引擎 + meta-harness + 📊 工作明细数据层** | `sofagent-ruleset-ast` 参考实现 + 多 harness 统一编排 + AI 工作明细（`worklog_query` + 落盘 `worklog.json`，**补节点实际耗时采集，商业模型层 C8 输入**，终端 ASCII 可选）（约束导出已并入 v1.3.6）（详见 开发日志 ./changelog/v1.3/v1.3.9.md）|
