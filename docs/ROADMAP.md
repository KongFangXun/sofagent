...
| **v1.6.0** | 📋 规划中 | **🧭 判定底座 · 阶段一（判据与数据地基）· 判据 schema 定稿**：三原语类型化（`Choice` 带全候选分布 / `Score` 带区间 / `Noul` 带真值概率）+ 问句集限定（类型越界拒绝）+ 多问并行形态预留 + 24–25 条审计规则逐条重述为「判据 + 证据」（复用 [v1.5.4](./changelog/v1.5/v1.5.4.md) 正负样例格式，零格式分叉）+ 判据 schema 加载校验 fail-closed + 理念升级驱动的文档重构机制立规（触发条件 / 分层口径 / 版本关系纪律）。**边界**：不训练、不接线、不出权重；decision-log / HMAC 链 schema 零改动。**前置门**：判据不可用则不启动后续版本 | [日志](./changelog/v1.6/v1.6.0.md) |
