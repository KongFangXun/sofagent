...
| **Cordis 内嵌迁移：证据注入保持主路径（release-gate run-04~22 决策 · 2026-08-24 拍板，同日实证修正）** | DSH worker 从 CLI 桥接切 Cordis 内嵌（库内集成 + sofagent 工具注入），worker 工具面回归——**决策：precheck 证据注入（driver 预执行注入 userMessage）保持为主路径**（一致性 / 可审计性 / token 成本可控），工具自读（read_file 等）作为补充能力。**实施时机：2026-08-24 实测修正——rc.2 现在就能做**（此前误判「等正式版」）：路径 = `boot()` + `loadProfile()` 替换裸 `new Context()` + 注入 `cmdlineArgs`/`appExit` 两服务 + 驱动契约适配 rc.2 实际 API（`agentLoop.createAgent` 替代旧契约 deliver/followup——`ctx.get('agents')` 是 AgentRegistry 形态无 deliver）。详见 [FORGE/lessons/driver.md](./../FORGE/lessons/driver.md) DSH Cordis 内嵌段。关联：release-gate 判断层 run-04~22 教训（DSH CLI 桥接无自定义工具面 → 证据必须 driver 注入） |
