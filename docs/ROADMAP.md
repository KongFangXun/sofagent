...
- [现在在哪：v1.5.0（已发版）· 下一版 v1.5.1（已排期）](#现在在哪v150已发版-下一版-v151已排期)
- [迭代历程](#迭代历程)
- [未来去哪](#未来去哪)
- [版本规划](#版本规划)
- [行业印证](#行业印证)
...
## 现在在哪：v1.5.0（已发版）· 下一版 v1.5.1（已排期）

> **v1.5.0 开发完成、文档收尾完成，已于 2026-09-19 发布**——治理 KPI 面板 + 本体数据双时态 + Validation Engine + 跨层证据对账 + FDE 陪跑期 + 存量清扫 + DSH 插件事件接线 · 测试 4805→4903 · acceptance 352→357。详见 [v1.5.0 开发日志](./changelog/v1.5/v1.5.0.md)。
...
| **v1.5.0** | 🔨 开发完成（✅ 已发版（2026-09-19）） | **🛡️ 治理模块 · 可见性与本体成熟**：治理 KPI 面板（Dashboard 治理 tab 六卡 + 数据集审阅 + lineage 合规报告 + 周报）· 本体数据双时态 · 本体渐进加载三层 · Ontology Validation Engine · 决策因果链消费 · FDE 陪跑期补全 · 存量清扫收尾（退役 API 移除 + 别名下线 + audit CLI 三 shim 移除，breaking）· 跨层证据对账 · 命名债务清扫（`@sofagent/harness` → `@sofagent/inject`，breaking）· DSH 插件事件接线。**形态归属**：主干 5 章（治理面板 / 本体双时态 / Validation Engine[**不可拔**] / FDE 陪跑期 / 跨层对账）+ 通道成分 1 + 非功能 3 章 + 插件 1 章。详见 [开发日志](./changelog/v1.5/v1.5.0.md)。 | [日志](./changelog/v1.5/v1.5.0.md) |
| **v1.5.1** | 📋 规划中 | **⚡ 编排模块 · 事件驱动**：业务节点事件驱动触发（三类事件源 + `on:` 订阅 + 死信重放）· 理解债务应对 · AI 异常处理总线（自 v2.x 前移）· G12 设备 OTA 远程升级（自 v2.x 前移）`[通道]` · 任务下发通道二期（推送/实时）`[通道]` · `sofagent demo` 五分钟戏剧弧`[通道]` · 审计输入双通道 · T8/T9 生产管线接线（自 v1.5.0 顺延）。**形态归属**：主干 5 章 + 通道 3 章 + 通道成分 1。*原「进化模块实证收口」已整版前移 [v1.4.5](./changelog/v1.4/v1.4.5.md)*。详见 [开发日志](./changelog/v1.5/v1.5.1.md)。 | [日志](./changelog/v1.5/v1.5.1.md) |
| **v1.5.2** | 📋 规划中 | **🔍 审计模块 · 开放与治理面**：MCP audit 数据对外（`audit_query` 只读 + 事件订阅推送）· 运行时 should-run 判定链 · doctor 修复闭环（含 `@sofagent/evolve` 旧包退役收尾，breaking）· 约束导出与证据链外部可验（`ruleset_export` + 零依赖验签工具）· 双规则引擎统一 · 审计规则自测 schema · A24 交付物落点审计规则（+1 成 25）· 审计结论失效语义。**形态归属**：主干 7 章 + 通道成分 1 + 非功能 1 章；四章已后移 [v1.5.4](./changelog/v1.5/v1.5.4.md)。详见 [开发日志](./changelog/v1.5/v1.5.2.md)。 | [日志](./changelog/v1.5/v1.5.2.md) |
