...
| 版本 | 核心交付 |
|------|------|
| **v1.4.3** | **🚀 后训模块 · 运行与需求 + 审计聚合指标**：① 训练监控与 GPU 队列（train_status/train_list + 显存预算排队不 OOM + webhook 三态推送）② 失败诊断 train_diagnose（七类故障 + 修复建议）③ 训练沙箱 + 设备打包（进程隔离/无外网——客户机房离线可训 + U 盘交付）④ 需求推导 + RL 配方模板库（train analyze 四步引导 + grpo/dapo/cispo 配方）⑤ 后训练 workflow 模板（七节点 DAG + 三 HITL）⑥ DSH 执行深化步一~三收口（流式面重建/分级切/usage 计量）⑦ 审计聚合指标（sofagent-audit --stats——约束层价值变成可汇报的治理 KPI）⑧ 训练反作弊基线（reward hacking 四形态双防线默认化）⑨ onboarding 断层走查 ⑩ fresh-eyes 四轮 bugfix 批 ⑪ 存量清扫五件（含 checkHistoryChainIntegrity 退役公告）⑫ 存量升级审查八项（迁 v1.4.4 第七章）⑬ doctor 补 Ontology 完整性检查 · 测试 3349→3619 · acceptance 276→294 场景（S345-S361）· MCP 76→79 |
| **v1.4.2** | **🚀 后训模块 · 数据与评估 + FDE Harness 层**：① 企业数据→训练集管道（CSV/Excel/DB/API 多源 + instruction/偏好对构建 + 质量闸门 + 脱敏）② 训练集版本管理（dataset_version）③ 训练中 eval 闭环（阈值外部化）④ 训练环境管理（train env init + train doctor）⑤ dry-run 与算力外推预检（ScaleRL 缩放律小 run 外推大 run 成本）⑥ 训练报告（客户可读交付物）⑦ FDE 引擎化六件（fde_interview/classify/quantify/derive/distill/deploy——方法论→可执行模块）⑧ IM 桥远程指挥（dsh-im 扫码接入）⑨ DSH 执行深化（步零三债 + 重复率熔断；步一~三移交 v1.4.3 承接）· 测试 3202→3349 · acceptance 271→276 · MCP 67→76 |
...
| **v1.4.0** | 📊 Web 工作明细页 + 图谱栏（FDE 双图谱 + MCP 工具视图 + skill 加载链）+ 成本审计（超支告警 + cost_query MCP）+ DSH 插件家族 9 款 + OpenClaw 插件家族 4 款 + Dashboard HTML 产品化 + 联邦查询跨设备 E2E + MLflow 接线 + Agentic Browser + 工具角色分层 + 瘦描述 + DSH 默认启用 + MCP 自动配置 + bash 3.2 验证 + 审计溯源字段回填 · 测试 2903→2981 · MCP 61→66 |
...
| 版本 | 状态 | 核心交付 | 日志 |
|------|:--:|------|:--:|
| **v1.4.4** | **🚀 后训模块 · 信号与部署闭环**：① 训练语料导出三件套（规则「24+3」+ GUIDE 方法论 + 样本五源 + Trace 轨迹 + 脱敏管线 + HMAC 签名 + 合规红线，含规则→reward/verifier 映射）② 企业专属模型本地权重部署链路（目录规范 + 本地加载 + 版本回滚）③ 训练产物→模型注册自动衔接（train done + eval pass → model_register）④ 多基座对比训练（train compare + ROI 排序）⑤ 决策因果链与先例检索（causedBy 因果边 + traceDecisionChain 链式回溯 + findSimilarDecisions 先例匹配）⑥ CI 供应链加固（8 workflow SHA 固定 + pin 对账 + 本地面 fail-closed）⑦ 存量升级审查（十六项：本版消化十项 + 六项分发 v1.4.8~v1.5.1）⑧ spec-first 硬禁令（check-spec-first 门禁 + SKILL 铁律区 + doctor 覆盖率展示）⑨ 六轮审查 17 项修复批（2 P0 + 6 P1 + 9 P2 全量收编） | [日志](./changelog/v1.4/v1.4.4.md) |
