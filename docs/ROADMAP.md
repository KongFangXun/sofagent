...
| **v1.2.9** | **🔧 FORGE Driver 短任务化 + ⏸️ Checkpoint/Resume + 🏠 PM2 守护 + 🔗 激活链 Phase 3 后半** | ① **🔧 短任务化**——fresh-eyes-loop 的 a-check/b-check 从"1 个 worker 跑 12 视角"拆成"12 个独立 worker 各跑 1 视角"（5-8 次工具调用/视角），保留发散性、消除长循环不收敛。release-gate 的 acceptance/f-fix 同理分片 ② **⏸️ Checkpoint/Resume**——worker 级断点续跑，driver 被杀后精确续跑未完成 worker（v1.2.8 设计的落地+增强）③ **🏠 PM2 守护进程**——driver 脱离 WorkBuddy session 生命周期，本地 PM2 管理崩溃自动重启+日志持久化+开机自启 ④ **🔗 激活链 Phase 3 后半**——HITL 中断处理 + 每节点执行后自动审计 + 异常处理 ⑤ **工程债务**——mcp-server.ts 1899 行拆分（详见 [开发日志](./changelog/v1.2/v1.2.9.md)）|
...
| **v1.3.4** | 📋 规划中 | **L3 组织能力市场（发布→发现→调用→评价）**：Skill / Agent / 流程打包发布（market/ 目录）+ 目录检索（复用 search_knowledge）+ 调用挂载（复用 registry）+ 评分聚合（评分 × 调用量加权自然选择）+ 全程审计。高频高价值能力自然胜出。⚠️ **边界**：企业内部能力共享，不含商业交易/第三方入驻/收益分成（跨企业商业市场属平台层）。原 v1.2.5 拆入，依赖 v1.3.3 L2 协议（详见 开发日志 ./changelog/v1.3/v1.3.4.md）|
...
| **v2.0** | 📋 规划中 | **引擎层接口外化**：引擎层四样核心能力从"内聚实现"升级为"对外标准接口"——① **约束导出通道**（审计规则从 TS 代码导出为机器可读标准格式，供外部系统消费）② **Workflow 标准格式 + 运行容器**（支持从外部提交标准格式 workflow，引擎层加载并执行，不再只接受代码内手写 graph）③ **Ontology 标准 Schema + 注册接口**（ontology 消费能力已有，补齐公开 schema 文档 + 外部注入接口）④ **审计规则插件化**（24 条规则从硬编码改为可外部注册的插件机制）。前置依赖：v1.3.1 Ontology 本体结构落地 + v1.4.0 沙箱底座就绪 |
...
| **自带净水设备的水龙头（v3.x+ 远景）** | Subagent 支持挂载外部精调小模型（引擎层提供路由与加载插槽，精调能力属模型层非开源范围），零投喂、本地推理、离线可用 |
