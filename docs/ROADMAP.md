...
| **v1.4.7** | 📋 规划中 | **🔌 商业平台接口版**：G2 能力缺口查询 · G4 绩效数据导出（orgId 过滤防跨租户）· G6 节点可见性元数据 · G7 多租户数据路径 v0 · workflow 烧进 USB · 审计留痕双层 · 静态加密全量接线 · **G8 首部署 cron job 包（本版最高优先级）** · G13 PR 生命周期接口（含拒绝留痕进 decision-log）· **G1 workflow 模板导出/导入（🔶 候选占位 · 待定，非定案）** · audit 专职面文档化 + tool 描述四原则化（机制 v1.4.4 已交付，本版补对外门面）；商业侧规划独立于本仓库维护 | [日志](./changelog/v1.4/v1.4.7.md) |
