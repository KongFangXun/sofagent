...
> sofagent v0.98（© v0.8 原始设计，v0.98 更新 rules.md→fde.md 引用）
> sofagent 的第一个 session 外触发器——让治理从「Agent 自觉」升级为「外部进程强制」。
> 循环工程 = 触发器 + 文件结构 + 工具 + 代码库治理。sofagent 只有后三样的雏形，
> 触发器完全空白。daemon 填这个坑。
>
> v0.98 · 更新于 2026-06-30（原始设计：v0.8-draft, 2026-06-20）
...
加载链补丁：监控 think.md 和 fde.md 变更 → Agent 启动时生成提醒文件 → Agent 看到提醒后自觉 Read。
...
| **事件触发** | 文件监控（think.md / fde.md 变更）| 跨 session 经验不丢失——Agent 启动前 daemon 已经把最新的教训准备好 |
...
softagent 的最大瓶颈：**加载链靠 Agent 自觉。** 跨 session 的经验（think.md）和规则（fde.md）能不能被读到，完全看 Agent 在下一个 session 是否愿意读。v0.61 验证失败已经证实：「文字写"必读"」挡不住 Agent 跳步。
...
| 做 | 不做 |
|---|------|
| 文件监控：watch think.md + fde.md 变更 | daemon 不参与 Agent 对话 |
...
daemon_loop() {
    ...
    local last_fde_hash=""     # 上次 fde.md 的 shasum
    local loop_count=0
...
inject_reminder() {
    ...
    local fde_updated
    fde_updated=$(jq -r ".fde_updated // false" "${DAEMON_JSON}" 2>/dev/null)
...
"
    ...
    if [ "$fde_updated" = "true" ]; then
        contract+="⚙️ **fde.md 已更新**\n\n"
    fi
    ...
    # ── 在 SKILL.md 加载链中联动 ──
    # 约定：reminder.md 放入加载链第 2.5 层，在 think.md 之后、fde.md 之前
