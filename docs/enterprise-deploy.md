...
| 数据加密 | ⚠️ 明文 | 等 v0.7x 的加密方案 |
| 权限控制 | ✅ 700 | install.sh 自动设置 |
| 数据保留策略 | ⚠️ 手动 | 等 v0.7x 的 --purge 命令 |
| 审计日志 | ⚠️ task/logs | 当前靠 task/logs 追溯，独立审计日志待 v0.7x |
| 外部 API 调用 | ✅ 可关闭 | 离线模式跳过 ClawHub |
| 配置文件修改 | ✅ 可控 | --no-config-inject 跳过 |

## 待办（v0.7x 规划）

以下能力已在 Roadmap，v0.55 暂不支持：
- task/logs 脱敏（写入前扫描 API Key/密码/手机号）
- 数据保留策略（--purge --before 命令）
- 独立审计日志
- think.md gpg 加密自动化
