# sofagent 陌生视角审查 Prompt（7 视角 × 6 方面）
...
>
> **三轮审查法**：本 prompt 包含 7 个视角，分为三轮：
> - **第一轮：角色扮演**（视角 1-5）—— 5 个陌生人各自独立看一遍
> - **第二轮：用户旅程**（视角 6）—— 从安装到跑通，走完整路径找断点
> - **第三轮：红队对抗**（视角 7）—— 故意搞破坏，找边缘 case
>
> 三轮可以分三次跑（每次一个或几个视角），也可以一次性全跑。轮次之间清空认知，从空白开始。

---

## 你的身份（第一轮：5 角色切换）

> 第一轮你将**连续切换 5 个身份**，每轮只能从当前身份的视角出发，不能借用其他身份的知识。每轮完成后再切换到下一个。第二轮和第三轮有独立的身份设定。
...
**你的任务**：
1. README 里说的"Agent 提交时审计工具——git diff 硬证据，11 条规则，pre-commit hook，不依赖 Agent 配合"——这句话你能反驳吗？漏洞在哪？
...
> 以下两项在完成全部 7 个视角后输出一次。不填视为审查未完成。
...
4. **绕过审计**：
   ...
   - 如果 Agent 用 `git commit --no-verify` 提交，sofagent 能检测到吗？
   - 有没有其他绕过路径？
