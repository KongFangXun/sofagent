...
> **核心原则**：这是发布后的独立审查（fresh-eyes 复核）——以未参与本版本开发的独立审查者视角重新看。不设固定检查项、不指定具体文件、不跑 grep、不预设"之前已修好 X"——凭第一印象和直觉判断，专门抓"作者自己看不出的、显而易见的问题"。
>
...
> | 轮次 | 维度 | 角色 | 方法 |
> |------|------|------|------|
> | 1 多角色 | 一 陌生人 / 二 企业IT / 三 竞品 / 四 npm / 五 开源 | 5 个独立审查者各自独立看 | 凭直觉 |
...
>
> 📌 **术语说明**：本 prompt 的「维度 N」「轮次」是**内部 QA 审查结构标签**（审查维度 / 轮次），与 `regression-checklist.md` 维度编号同源，属正常 QA 术语；本文档是发布后独立审查（fresh-eyes 复核）工具，并非对外扮演「陌生视角 / 外部视角」的文案，与术语铁律所禁角色扮演话术无关。
...
>
> **身份**：sofagent 发布后独立审查。当前版本已通过发版前审查（pre-push-check / acceptance-test / OpenClaw / 回归清单全绿）。这轮以独立审查者身份重新审视——不看作者意图，只凭交付物本身判断"这东西站不站得住"。**v1.1.9 叙事转向后**：审查对象的产品身份是 **FDE Agent**（对外用户看到的唯一入口），底层是 **sofagent 引擎**（Harness 中间件 = 审计 + 编排 + 约束 + 回溯）。审查时验证两层叙事是否一致——对外 FDE Agent + 底层 sofagent 引擎，有没有该说"FDE Agent"却说了"sofagent 引擎"的地方（反之亦然）。直觉比 grep 更有价值。发现的问题不阻塞本版本，进下版本修复。
>
...
10. **FDE Agent 统一产品身份感知（v1.1.4 新增，v1.1.9 叙事转向更新）**：你在 GitHub 看到 `FDE/` 和 `LOOP/` 两个**大写**目录——3 秒内能搞清楚它们和 sofagent 的关系吗？**v1.1.9 叙事转向后**：对外用户看到的应该是一个统一的 **FDE Agent** 产品（底层是 sofagent 引擎），而不是"sofagent + FDE + LOOP 三个并列产品"。验证：README §① 的"FDE Agent 是什么"段落是否让你形成"这是一个产品"的印象？还是仍像旧叙事那样让你觉得有"三个并列产品"？三产品关系（§③）有没有明确说明 FDE Agent 是对外的统一身份？大写目录（`FDE/`、`LOOP/`）是部署/产品入口，小写目录（`sofagent/`、`docs/`）是核心代码——这个命名约定文档里有写明吗？**特别检查**：**双层身份一致性**——所有文档中"FDE Agent"（对外产品身份）与"sofagent 引擎"（底层引擎）两层是否一致？有没有该说"FDE Agent"却说了"sofagent 引擎"的地方？
11. **README 首屏信息架构评审（v1.1.6 新增）**：前面几个任务都是"读 README 某一段判断一件事"，这一项专门审查 README 作为整体文档的**编排质量**——结构、密度、对比表诚实度、首屏比喻一致性。任务 6（文档瘦身）看的是"行数够不够短"，任务 11 看的是"排得够不够好"。具体检查：
    ...
    - **信息密度盲区**：有没有段落读完什么信息也没拿到？有没有同一个意思在首屏、中段、结尾说了三遍？有没有 details 折叠块外面和里面信息重复？**特别检查**：「21 条规则（5 类）」分组表和「完整规则表（折叠）」——分组表已经列了每条规则的一句话描述，折叠表又把同样规则再列一遍加判定逻辑——这两张表是否可以合并，还是分组表真的只给"概览"、折叠表给"细节"？
    ...
    - **历史教训**：v1.1.5 及之前 README 首屏 3 个比喻打架（"接上 AI" / "高速公路" / "全生命周期"），结构上"设计哲学"在"怎么装"之前，读者 3 屏还搞不清这东西是干什么的。v1.1.6 重构后统一为"行车记录仪+安全带"一个比喻，对比表前置，467→280 行减 40%。v1.1.9 再次重构为四段式用户视角（① FDE Agent 是什么 → ② 装上就能用 → ③ 企业落地 → ④ 引擎架构），比喻统一为 River（堤坝+自来水厂+管网+水龙头），从技术视角转为用户视角。这类"整体文档编排"问题单看任何一段发现不了，必须把 README 当一个整体来读才能暴露。
...
> 你做完了前面八轮审查，确认了文档和代码的一致性。但还有一个维度是前八轮都没覆盖的：**用户能不能感知到 FDE Agent 的存在？** v1.1.9 叙事转向后，用户面对的产品身份是 FDE Agent（底层是 sofagent 引擎）——最大的挑战是让用户感知到**底层引擎在保护他们**：所有审计、约束、回溯都在正常工作，但用户看到的只是 Agent 的对话输出，看不到"是谁让这一切安全发生的"。
