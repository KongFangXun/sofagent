...
| 4 | 越界改动 | 修 bug 时混改无关文件 | ⚠️ A3 WARN | ✅ 允许（提醒） | A3 最小变更 |
| 5 | 配置删除 | 删除 tsconfig.json | ⚠️ A4 WARN | ✅ 允许（提醒） | A4 配置保护 |

> **设计说明**：A1/A2 为 FAIL（exit 2）→ 阻断提交；A3/A4 为 WARN（exit 1）→ 允许提交但提醒。A3 依赖任务描述匹配，CI 场景下 commit message 可能不精确，误判风险高；A4 删配置可能是正常重构，提醒比阻断更合理。
...
# 3. 逐个跑场景（手动模拟 Agent 文件改动 + git commit 触发 pre-commit hook）
...
git add src/utils.ts README.md && git commit -m "refactor utils"                        # 场景 4 → A3 WARN 允许

git rm tsconfig.json && git commit -m "remove config"                                   # 场景 5 → A4 WARN 允许

# 4. 验证结果
# PASS 场景 → git commit 成功
# FAIL 场景（A1/A2）→ pre-commit hook 拦截，hook exit 1 / sofagent-audit exit 2，修复建议显示
# WARN 场景（A3/A4）→ commit 成功 + 警告输出
...
- [ ] pre-commit hook 正确触发
- [ ] 审计输出 banner 可视化正确（通过/警告/违规三场景）
- [ ] 违规场景显示修复建议
- [ ] PASS 场景 commit 成功
- [ ] FAIL 场景 commit 被拦截（hook exit 1，sofagent-audit exit 2）
- [ ] WARN 场景 commit 允许 + 警告输出
- [ ] 审计历史正确写入 history.jsonl

## 更新时机

- sofagent 审计规则变更（新增/修改规则）
- pre-commit hook 模板变更
