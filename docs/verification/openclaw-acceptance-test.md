...
### 场景 57：LOOP 独立产品（目录结构 + install 契约 + FLOWHUB 隔离）

```bash
# v1.1.4 最大架构调整：LOOP 从 sofagent 包内迁出到根目录 LOOP/，成为独立产品
# 大写目录=独立产品（LOOP/FDE/FLOWHUB），小写=代码文件夹

LOOP_DIR="$SOFAGENT_DIR/LOOP"
FLOWHUB_DIR="$SOFAGENT_DIR/FLOWHUB"
...
# 2. FLOWHUB/ 目录（社区模板市场，与 LOOP 内部编排隔离）
for f in README.md CATALOG.md; do
  [ -f "$FLOWHUB_DIR/$f" ] && echo "FLOWHUB/$f ✅" || echo "FLOWHUB/$f ❌"
done
