...
> v1.1.4 暴露：`tools/acceptance-test.sh`（50 场景）和 `docs/verification/openclaw-acceptance-test.md`（51 场景）对本版本新增功能**零覆盖**——A18 垃圾文件检测、LOOP 独立产品（LOOP/ 目录 / loop-install.sh / loop-workflow.sh / 模板市场）、USB federation、工具注入（ENGINEER_TOOLS / REVIEWER_TOOLS / maxTurns / checkDangerousCommand / recordLoopAuditHistory / warn-accumulator）全部无场景。两份验收测试场景数远落后于代码实现。
...
# 例：sofagent-audit --daemon（v1.1.4 废弃）、work模板市场/（v1.1.4 更名为 模板市场/）
grep -rn "sofagent-audit --daemon\|work模板市场/" tools/acceptance-test.sh docs/verification/openclaw-acceptance-test.md
