...
- **真实代码库 + 真实 PR 当考题**：研报用「已合并 PR + 原 PR 测试用例」当评分标准，规避公开 benchmark 泄漏导致的刷分。对应 sofagent `regression-checklist.md`（265 维）+ `acceptance-test.sh`（115 场景）——用真实修复场景与历史 case 当验收，而非玩具 benchmark。
