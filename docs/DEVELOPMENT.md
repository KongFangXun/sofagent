...
> 💬 **开发铁律**：sofagent 不建编辑器类交互界面。只读 Dashboard 面板（如 `dashboard.html`）例外——它是状态可视化，不做双向编辑。核心能力通过 MCP 协议暴露。Agent 首次连接时主动推送 `list_capabilities`。开发任何新功能前，先回答三个问题：（1）用户怎么通过对话发现这个能力？（2）结果推到哪？（3）用户怎么知道这个结果是 sofagent 做的，不是模型做的？——任何面向用户的输出必须带 `[sofagent]` 签名标注来源。详见 [设计哲学](./PHILOSOPHY.md)。MCP 完整 resource 清单见 [MCP 使用指南](./guides/mcp-usage.md)。
...
sofagent **能力底座（一底座·三引擎）** 各有分工。**审计引擎**只看 git diff（提交时），不依赖 Agent 配合。**编排引擎**在 Workflow 梳理时生成节点定义，之后 Sub Agent 自加载约束执行。两种调用路径：支持 Hook 的平台节点走内部 API，其他节点走 CLI。两者通过 think.md 交汇——审计引擎基于 diff 硬证据自动生成反思，编排引擎读取优化策略。
...
### 轨迹优化闭环：Trajectory Store + LLM-Judge 蒸馏（2026-07 行业参考 blog 研读）
...
### 记忆 ≠ RAG：分层状态管理（2026-07 行业参考 blog 研读）
...
> 激活链 Phase 1-3（v1.2.5-v1.2.8）已实现。Phase 4 收尾（v1.3.0 SUSTAIN）规划中。以下为给贡献者的扩展指南。
