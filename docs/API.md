...
> sofagent 对外全部能力面的一站式清单——七大接口面 + MCP 84 tools 按域分组（第七面「标准数据推送接口」v1.4.6 交付）。工具清单由 `engine/mcp/src/tool-registry.ts` 生成（scripts/check 门禁对账，文档与代码永不漂移）。
>
...
| 7 | **标准数据推送接口** | 数据推送 API（约定 schema · 分拣闸 + 合规闸双闸） | 企业凭证 | 企业存储/业务系统推送训练语料与知识数据（v1.4.6 交付） |
