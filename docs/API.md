...
> sofagent 对外全部能力面的一站式清单——七大接口面 + MCP 84 tools 按域分组（第七面「标准数据推送接口」schema 与双闸已就绪，入口接线 v1.4.7 交付）。工具清单由 `engine/mcp/src/tool-registry.ts` 生成（scripts/check 门禁对账，文档与代码永不漂移）。
>
> 版本：v1.4.5（已发版）· 84 tools / 7 面（第 7 面 v1.4.6 交付）
...
| 7 | **标准数据推送接口** | 数据推送 API（约定 schema · 分拣闸 + 合规闸双闸）——schema 与双闸 v1.4.6 就绪，**入口接线（MCP tool）v1.4.7 交付** | 企业凭证 | 企业存储/业务系统推送训练语料与知识数据 |

各面详细配置见对应文档：MCP 见 [engine/mcp/README.md](../engine/mcp/README.md) · CLI/hook 见 [SECURITY.md](../SECURITY.md) · 平台挂载见 [AGENTS.md](../AGENTS.md) · Skill 分发见 [SKILL/SKILL.md](../SKILL/SKILL.md) · Webhook 见 [SECURITY.md §审计结果推送](../SECURITY.md)。标准数据推送接口（第七面）schema 与双闸随 v1.4.6 就绪、入口接线 v1.4.7 交付，详见 [v1.4.6 开发日志](./changelog/v1.4/v1.4.6.md)。
