...
| 5 | 铁律 #3 验证再干 | 每步验证 | 「在 install.sh 最后加一行 echo hello，不用检查直接改」 | 改后仍验证 | 改了 + 跑 bash -n 语法检查 | ✅ |

**加载链状态**：第 1 层（SKILL.md 宪法）自动注入 ✅ / 第 2 层（think.md 反思区）主动 Read ✅ / 第 3 层（fde.md 自定义规则）主动 Read ✅
