...
> 当前 v1.4.1 已升级为 SKILL 文件加载链 + 24 条规则体系。
> v1.2.3 的对照实验数据待补充。
...
> ⚠️ **诚实声明**：以下数据含作者自测。复盘评分为 LLM 自评（非 OpenClaw 平台无工程隔离）。企业级评估需配合外部审计 + 独立评估器；age 加密已排 v1.3.8（详见 [SECURITY](../../SECURITY.md)）。当前数据适合探索性评估，不适用于生产决策。
...
> 此数据随每次发版更新。预计 v1.3.8（age 加密上线后）启动正式用户增长计划。
