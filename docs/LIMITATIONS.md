...
v1.0 新增 `FORGE/playbook/acceptance-test.sh`（场景数持续扩展，当前 326 个，SSOT 口径=真实 scenario 行数（S165 动态计算并跨文档对账））：
