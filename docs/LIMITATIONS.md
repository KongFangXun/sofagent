...
当前审计核心 786 个、全 workspace 2515 个测试（实测见 `tools/test-count.sh`，flaky 复跑机制内置，以脚本判定为准，与 pre-push-check 一致），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
v1.0 新增 `FORGE/playbook/acceptance-test.sh`（场景数持续扩展，当前 222 个，SSOT 见脚本头部声明）：

- **CI 已覆盖**：单元测试审计核心 786 个、全 workspace 2515 个测试（全绿，详见上方「测试覆盖范围」节，实测见 `tools/test-count.sh`，与 pre-push-check 一致）、sofagent-core verify 约 44-48 项（动态）
- **发版前手动覆盖**：acceptance-test.sh 222 场景（含子断言，CLI 端到端，步骤 2.3）、OpenClaw 验收 63 场景（Agent 端到端，步骤 2.5）
