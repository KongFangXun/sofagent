...
当前审计核心 878 个、全 workspace 3168 个测试（v1.4.1 交付批次 2946→3168 +222：train 模块 orchestrator 215 + daemon 7；实测见 `tools/check/test-count.sh`，flaky 复跑机制内置，以脚本判定为准，与 pre-push-check 一致），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试审计核心 878 个、全 workspace 3168 个测试（v1.4.1 开发批次 2946→3168 +222：train 模块 orchestrator 215 + daemon 7；全绿，详见上方「测试覆盖范围」节，实测见 `tools/check/test-count.sh`，与 pre-push-check 一致）、sofagent-core verify 约 44-48 项（动态）
