...
当前审计核心 587 个、全 workspace 1527 个测试（共 1527 个，全绿；含 v1.2.4 新增 64 个知识进化测试 + v1.2.5 审计加固测试 + v1.2.7 新增 slash/mailbox/enterprise-graph/support-bundle 等 57 个测试。实测见 `tools/test-count.sh`，与 pre-push-check 一致），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试审计核心 587 个、全 workspace 1527 个测试（共 1527 个，全绿；含 v1.2.4 新增 64 个知识进化测试 + v1.2.5 审计加固测试 + v1.2.7 新增 57 个测试。函数级，实测见 `tools/test-count.sh`，与 pre-push-check 一致）、sofagent-core verify 约 44-48 项（动态）
- **发版前手动覆盖**：acceptance-test.sh 132 场景（含子断言，CLI 端到端，步骤 2.3）、OpenClaw 验收 63 场景（Agent 端到端，步骤 2.5）
