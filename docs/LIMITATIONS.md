...
**这是定位差异而非缺陷**：前置授权（mandate）属「先批后干」，与本项目的**事后审计**定位不同。补齐事前环登记在[路线图探索方向](./ROADMAP.md#探索方向)，动手条件是真实企业客户提出先批后干诉求——在此之前它保持**已知缺口**状态，不假装覆盖。

**降级探测手段**：门控健康度四联红旗（ASK 率趋零 + 获批率恒 100% + 审批时延 1–2 秒 + 人均审批量异常）能在没有完整审批链时**部分**探测「审批形同虚设」——分开看每项都有良性解释，合起来看没有。这是**代理证据，不等于审批链审计**。
...
当前审计核心 1226 个、全 workspace 4906 个测试（口径：13 包 workspace；逐批沿革账已迁出，见 [v1.4.9 开发日志 · 附录](./changelog/v1.4/v1.4.9.md#附录测试与场景账沿革)），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试审计核心 1226 个、全 workspace 4906 个测试（口径见本文件「测试覆盖范围」节）、sofagent-core verify 约 44-48 项（动态）
- **发版前手动覆盖**：acceptance-test.sh 358 场景（含子断言，CLI 端到端；阶段五步骤一脚本层直跑）、OpenClaw 验收 63 场景（Agent 端到端）
