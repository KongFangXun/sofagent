...
| 7 | **FDE 交付物激活断裂带（v1.2.5+ 解决中）**——FDE 诊断交付的 ontology + workflow.yml + skills/ 是静态文件，企业 IT 拿到不知道怎么跑起来。激活链 Phase 1-3（ACTIVATE+ORCHESTRATE+EXECUTE 前半）已于 v1.2.5-v1.2.8 交付，Phase 3 后半（v1.2.9）已交付，Phase 4（v1.3.0 SUSTAIN）开发中。 | [十二、FDE 交付物激活断裂带（v1.2.5+ 解决中）](#十二fde-交付物激活断裂带v125-解决中) |
...
当前审计核心 689 个、全 workspace 1684 个测试（全绿，实测见 `tools/test-count.sh`，与 pre-push-check 一致），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试审计核心 689 个、全 workspace 1684 个测试（全绿，详见上方「测试覆盖范围」节，实测见 `tools/test-count.sh`，与 pre-push-check 一致）、sofagent-core verify 约 44-48 项（动态）
...
> ✅ 已于 v1.1.9 修复：Workflow Hub 模板整体迁出 MIT scope，相关 CLI（`sofagent hub deploy`）与模板源已移至外部商业仓，不在开源仓库维护。
