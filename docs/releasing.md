...
2.8 陌生视角审查 → 用 ~/Workbuddy/sofagent-fresh-eyes-review.md（7 视角）从全新角度审查，发现回归检查覆盖不到的新问题
...
- [ ] **回归检查**（`~/Workbuddy/sofagent-regression-checklist.md`）：106 维度逐项核对，全 PASS。发现回退 → 修复后重跑
- [ ] **陌生视角审查**（`~/Workbuddy/sofagent-fresh-eyes-review.md`）：7 视角审查，记录发现的新问题。P0/P1 必须修复后才能发版，P2 可记录到 changelog 后续修复
