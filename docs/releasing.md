...
| 10.5 | 回归检查：用 137 维度回归清单逐项核对 | 审核者 | 全 PASS。发现回退→修复后重跑。新问题→追加检查项（从 138 开始编号） |
| 10.6 | 陌生视角审查：用 [陌生视角审查 Prompt](../../sofagent-fresh-eyes-review.md) 跑 7 视角 × 6 方面——与回归检查互补，回归检查看已知问题是否回退，陌生视角看是否有盲区 | 审核者 | 7 视角全部输出报告。发现的问题分类：P0→立即修、P1→下一次、P2→记录。审查体系更新建议→更新回归清单 + 陌生视角 prompt |
...
| 18 | **审查体系维护**：审视审查 prompt 是否需要更新——① 有没有通用检查点要加到回归清单（编号从 138 递增）？② 新问题类别有没有要加到陌生视角审查的？③ 陌生视角 prompt 本身有没有过时的角色或问题？ |
