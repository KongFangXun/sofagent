...
商业模型层.md §3.4 明说：帮企业训练「不是泛泛训个模型，而是**根据 workflow 中每个节点需要的能力，精确定义这个节点需要什么样的专属模型**」。本版把这条推导做成引导式工具 + 场景模板库——训练引擎区别于训练工具的分水岭。
...
| **商业模型层 模型层** | ① 训练沙箱 = C13 联合训练「数据不出边界」的实现底座 ② 需求推导 + 模板库 = 商业模型层.md §3.4「基于 workflow 定义训练需求」的开源规则驱动版（商业模型层 模型驱动版在其上）③ 设备打包 = E4 设备封装前置 ④ **后训练 workflow = 训练引擎作为 FDE Agent 核心功能点的载体（企业一句话发起后训练的完整链路）** |
