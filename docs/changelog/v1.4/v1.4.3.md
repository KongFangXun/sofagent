...
> 📋 **2026-08-29 拍板迁移**：升级审查八项（eval 三套统一/加载链 token 预算/webhook 事件源/daemon 巡检归组/IM 桥/瘦描述复核/证据分级/本体混合检索）已整体迁入 [v1.4.4 第七章](./v1.4.4.md#七存量升级审查落本版十五项--2026-08-29-用户拍板收编)（SSOT）——eval 统一与语料导出同版更顺，76 tools 复核并入 v1.4.4 发版检查。本版第十一章存量清扫（删除型）不受影响，仍在 v1.4.3 执行。
...
- [ ] 坏样本三件（缺 `---` / YAML 语法错 / relations 拼写错）逐一 WARN 且病因文案准确
- [ ] 正常 entities/ 目录零误报
- [ ] doctor 输出含跳过对账（合并引擎跳过数与检查报告数一致）
- [ ] `npm test` 全绿
