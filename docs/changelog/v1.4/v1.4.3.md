...
| 组件 | 能力 |
|------|------|
| **train_status MCP tool** | 查训练进度：status / step / loss / reward 曲线 / 日志尾部——MCP 客户端（WorkBuddy/FlowHub）可查 |
...
AIR.md §3.4 明说：帮企业训练「不是泛泛训个模型，而是**根据 workflow 中每个节点需要的能力，精确定义这个节点需要什么样的专属模型**」。本版把这条推导做成引导式工具 + 场景模板库——训练引擎区别于训练工具的分水岭。
...
| **SUSTAIN 运转** | ⑦ 部署上线 + 持续后训练 | model_register / E2 | 强制人审晋升 |
...
| 后续版本 | 依赖 v1.4.3 的什么 |
|---------|-------------------|
| **v1.4.4 信号与部署** | 沙箱承接训练进程；需求推导产出的配置喂给语料导出（训练目标标注）；后训练 workflow 的 pt-deploy 节点衔接 C13 权重部署 |
| **AIR 模型层** | ① 训练沙箱 = C13 联合训练「数据不出边界」的实现底座 ② 需求推导 + 模板库 = AIR.md §3.4「基于 workflow 定义训练需求」的开源规则驱动版（AIR 模型驱动版在其上）③ 设备打包 = E4 设备封装前置 ④ **后训练 workflow = 训练引擎作为 FDE Agent 核心功能点的载体（企业一句话发起后训练的完整链路）** |
