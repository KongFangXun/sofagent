...
> 📌 **版本身份说明**：阶段五 regression 报「v1.4.2（npm/SSOT/tag 三处一致）」与本日志「v1.4.3」并存属**发版流程正常时序**——版本 bump 在发版 SOP 阶段十一执行，闸门运行时仓库三处必然仍为上一已发版本，非数据冲突。勘误人：run-02 零信任复验（2026-08-31），闭环 run-02 V-P0-1。
...
| F-10（findings） | 非 git 目录装 hook 静默跳过 | 已修（else 分支 warn） | install.sh:486-492 条件不满足即静默的旧形态已由批次四复验在位确认；裸 pre-commit 非支持形态（上表定谳）记录不修 |
