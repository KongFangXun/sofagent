...
**版本拆分（2026-08-10 拍板）**：训练引擎拆四版——**v1.4.1 地基**（本版：协议+编排+审计+预算+隔离+指纹+签名）→ **v1.4.2 数据与评估**（数据管道/训练集版本/eval 闭环/环境管理/dry-run/训练报告）→ **v1.4.3 运行与需求**（监控+GPU 队列/失败诊断/沙箱+设备打包/需求推导+模板库）→ **v1.4.4 信号与部署闭环**（语料导出三件套/企业专属模型权重部署/产物注册衔接/多基座对比）。
...
>
> 🔗 **schema 兼容承诺**（2026-08-10 声明）：job.json schema 向后兼容**最近 3 个版本**（如 v1.4.6 v2 schema 的引擎可跑 v1.4.3 v1 schema 的 job）。不兼容的 breaking change 走 **migrate 子命令**（`train migrate --from v1 --to v2`）+ 人审确认，不做静默升级。早期版本 schema 变动小，migrate 可后置；声明先行让企业有预期。
...
> 📋 **2026-08-10 补充（训练引擎完善）**：训练是**算力密集型长任务**——企业交付要能设预算上限（时间 / 估算算力成本），超预算自动暂停 + 人审。对齐「成本透明」产品原则 + 模型网关的预算控制模式。
...
> 📋 **2026-08-10 补充（企业 IT 头号关切）**：训练引擎服务多个企业时，A 企业的训练数据 / 权重 / 审计记录 / 成本，必须保证不串到 B 企业。v1.3.0 做了「运行时审计日志按 git 仓库隔离」——训练引擎的隔离维度需要同样清晰。这是企业 IT 视角的头号问题（「财务 Agent 和人事 Agent 用同一个底座，数据会串吗」同样适用于训练），不补就是定时炸弹。
...
## 六、训练可复现指纹（train-fingerprint · 完整输入冻结）
...
| 攻击面 | 覆盖措施 | 归属版本 |
|--------|---------|---------|
| **job.json 路径注入** | zod schema 校验 + 路径白名单（只允许 `data/train/` 下）+ 命令注入字符过滤 | v1.4.1 ① 协议 |
...
| **权重防篡改** | SHA-256 + HMAC + 加载校验（v1.4.1 ⑦） | v1.4.1 ⑦ |
| **跨企业数据泄漏** | enterpriseId 全链路隔离 + 跨企业访问阻断（v1.4.1 ⑤） | v1.4.1 ⑤ |
...
| 后续版本 | 依赖 v1.4.1 的什么 |
|---------|-------------------|
| **v1.4.2 数据与评估** | 数据管道产出的训练集喂给 train-job（协议① job.json）；eval 闭环消费 train-job 完成事件；数据管道产出走 enterpriseId 隔离 |
...
| **模型层（商业侧）** | ① 双栈协议 = 「提供训练管线」交付模式的地基 ② train_job 审计 = 训练本身可审计可回滚 ③ 预算控制 = 成本透明（定价前提）④ @mlx-node/trl = 个人 Mac 阶段 0 纯 Node 验证路径（reward → 收敛）⑤ **多企业隔离 = 服务多客户的信任底线** ⑥ **可复现指纹 = A/B 对比训练的声称基础** ⑦ **产物签名 = 审计 DNA 延伸到权重文件** |
