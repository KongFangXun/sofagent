...
>
> 🔗 **2026-08-12 前移说明（B2 决策）**：原 v1.4.1 的「训练协议三约定」+「训练预算控制」**前移至 v1.3.6**（理由：协议是双栈契约不依赖完整训练引擎，提前定稿让 C13 PoC 客户更早接入；预算控制是横切能力，与协议同版避免割裂）。本版保留双栈架构决策作为背景，**编排/审计/隔离/指纹/签名/恢复/安全七大块仍在本版**。
