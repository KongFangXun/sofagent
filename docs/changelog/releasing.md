...
> **十二阶段**：审查（fresh-eyes-loop 找齐上版本 bug）→开发（先修上版本 bug，再做新功能）→基础自测（bump + build + test + shellcheck）→fresh-eyes-loop 质量循环 + 代码审核 + 验收测试→审查体系合并更新（来源提取→四份文档升级→防膨胀瘦身）→release-gate-loop 发版闸门→审查体系最终确认→文档收尾→工具脚本健康检查→确认关口→发布（含设备端安装）→发布后（含独立审查）。
...
> 🔴 **Prompt 模板铁律**：本文档中所有「一键复制 prompt」模板（阶段四步骤 1、阶段六监控 session），AI 输出时**必须把所有占位符替换为当前版本的实际值**再呈现——版本号、项目路径、日期等全部填好，用户复制后可直接粘贴到新 session 执行，不需要再手动替换任何内容。
...
| 2 | P1 工程欠债 | 工程师 | 应该修 |
| 3 | P2 改进 | 工程师 | 不阻塞发布 |
| 4 | 审查体系更新 | 工程师 | 随修复同步更新：① 回归清单追加检查项（编号递增）② 发布后审查文档（`fresh-eyes-review.md`）补充新盲区维度/任务。**不要等到阶段五和阶段八才做——开发时记忆最新，随修随记** |
...
> **v1.2.7 流程重构**：原阶段三拆为两个独立阶段——阶段三（纯机械自测）+ 阶段四（fresh-eyes-loop + 代码审核 + 验收测试）。阶段三只跑脚本验证（bump → build → test → shellcheck → diff），全部通过后进入阶段四做独立审查。
...
> 阶段三纯机械自测全绿后，本阶段做独立审查 + 人工审核 + 验收测试更新。三件事有依赖关系：先跑 fresh-eyes-loop 找 bug 修完，再拿 changelog 逐项核对，最后基于确认后的功能清单更新 acceptance-test。

### 步骤

| # | 步骤 | 谁做 | 验证方式 |
|:--:|------|:--:|------|
| 1 | **🔴 运行 fresh-eyes-loop（开发后独立质量循环）**：在**全新 session** 中启动 Node driver——`node FORGE/src/fresh-eyes-driver.mjs --target <本版本号> --max-rounds 10`。driver 用 LangGraph createReactAgent 编排 A（审查模型）双盲并行审查 + B（工程模型）修复 + A 验证，连续 2 轮 findings 无 P0/P1 即停（机制详见 `FORGE/SKILL/fresh-eyes-loop/SKILL.md`）。**Session 监控协议**：启动 driver 后按 SKILL.md「Session 监控协议」**在 session 内持续轮询** `<runDir>/status.json`——每 120 秒读一次，phase 变化时一句话汇报，phase 不变就继续轮询（session 一直在转，不 sleep 空转）。🔴 修复提交本地、不 push（发版步骤才统一推） | driver 跑完，`status.json` 显示 phase=completed 且无未推送提交被误 push |

> **📋 一键复制 prompt（阶段四步骤 1 专用）**：执行到这一步时，**当前 session 的 AI 必须把下面的模板中所有占位符替换为实际值**（版本号、项目路径等），**输出填好的完整 prompt** 到对话里（不要创建任何文件），用户复制后直接粘贴到新 session 的第一条消息即可执行：
...
| 4 | FAIL 项修复 | 当前 session（切回开发者角色） | build + test 全绿 |
| 5 | **🔴 代码冻结回归验证**：所有代码变更（fresh-eyes-loop 自动修复 + 步骤 4 人工 FAIL 修复）完成后，重新跑 `npm run build` + `npm test` 确认全绿——fresh-eyes-loop 的 B 模型修改了代码，必须验证不引入回归。build + test 在当前 session 直接跑（2-3 分钟出结果），不需要开新 session | 当前 session | build exit 0 + test 全绿 |
...
> 🔴 **v1.1.3 教训**：每版本发版后，验收测试文件自身的功能也会过时——**场景数落后于代码实现、新增功能零覆盖**。步骤 6 在 fresh-eyes-loop 之后执行，确保补的场景基于已审核确认的功能清单。

---

## 🔴 阶段五：审查体系合并更新（四份审查文档 + 防膨胀瘦身）

> **核心原则**：阶段五的首要任务不是自审行数格式，而是**从本轮所有产出物中提取新发现，系统性地升级四份审查文档**。审查体系如果不能跟上每轮新功能和新问题的节奏，就永远是上一个版本的审查水平——新功能的检查面永远是盲区。

> **四份文档，四套逻辑**：`regression-checklist.md`（精确清单，加法）· `acceptance-test.sh`（自动化验证，加法）· `check-version.sh`（结构性门禁，加法）· `fresh-eyes-review.md`（留白式直觉审查，校准非加法）。

所有 P0/P1/P2 开发修复完毕、自测和代码审核全部通过后，执行以下步骤：

| # | 步骤 | 谁做 | 验证方式 |
|:--:|------|:--:|------|
| 1 | **审查内容来源提取**：逐一过以下 5 个来源，产出 A/B/C 三类清单（对话中列出，不落盘）：<br>**来源①** fresh-eyes-loop 审查报告（阶段一/四）——每个 P0/P1 finding 对应的问题模式<br>**来源②** BugFix 交付清单（阶段二）——每个修复项是否已有 regression-checklist 覆盖<br>**来源③** 新功能交付清单（阶段二）——每个新交付引入的新代码路径/API/行为契约/对外声称<br>**来源④** fresh-eyes 复审报告（如有）——预料外盲区<br>**来源⑤** CHANGELOG 开发日志——交叉验证交付列表每项都有对应的审查内容提取<br><br>**三类清单**：<br>**A 类：新功能审查面**——新功能/新代码路径/API/行为契约在四份文档中有没有检查面？→ regression-checklist + acceptance-test + check-version.sh<br>**B 类：Bug 防回归**——下次怎么防止回退？→ regression-checklist + acceptance-test<br>**C 类：预料外盲区**——凭直觉发现、无法精确化的系统性问题 → fresh-eyes-review<br><br>**🔴 关键检查 1：新功能审查面覆盖率**——打开 changelog 交付列表，逐条问"这个交付引入的新代码路径/API/行为契约，在 regression-checklist 里有对应的检查维度吗？在 acceptance-test 里有对应的验证场景吗？新增的对外声称数字，check-version.sh 能对账吗？"有遗漏 → 标记，必须在步骤 2 中补上<br><br>**🔴 关键检查 2：问题模式系统性提取（v1.3.1 补强——不只是功能覆盖，还要从 bug 中提取模式）**——手里有本轮所有审查报告（fresh-eyes findings / release-gate verdict / 16 视角人工审查），**逐份过一遍，每个真实发现（非误报）问三个问题**：<br>① 这个问题属于什么**模式**？（如"同一属性多处声明漂移""检查命令依赖具体措辞""架构迁移后检查命令没跟上"）<br>② 这个模式在四份文档里有对应的检查面吗？→ 没有就是新维度/场景/视角<br>③ 如果是"审查文档自身的检查命令过期"（如维度 15 grep 已不存在的措辞、维度 70 查已迁移的路径）→ 这是**元模式**，需修旧检查命令 + 加维度 92 式的元检查<br>**禁止只做功能覆盖检查不做模式提取**——功能覆盖只防"新功能没测"，模式提取防"同样的坑踩第二次"。v1.3.1 教训：只做功能覆盖（14 项交付全有场景），漏了从 run-10/13 的 12 个真实发现提取模式，被用户提醒后才补 | 当前 session | A/B/C 三类清单产出 + 问题模式清单产出；changelog 每个交付项都有审查覆盖（零遗漏）+ 本轮每个真实发现都有模式归类（已覆盖 or 新增） |
...
| 3 | **逐项验证**：① 每条新增回归维度跑一遍命令确认可执行 ② 确认 fresh-eyes-review 新维度与回归维度互相印证、无矛盾 ③ **新功能审查面覆盖率确认**——步骤 1 标记的遗漏项在步骤 2 中是否全部补齐？逐条 `grep` 确认 changelog 每个交付的关键词在 regression-checklist 或 acceptance-test 中至少出现一次 ④ **问题模式提取闭环确认（v1.3.1 补强）**——步骤 1 的"问题模式清单"中每个标记为"需新增"的模式，在步骤 2 中是否都落到了对应文档（regression-checklist / fresh-eyes-review / acceptance-test）？ | 当前 session | 所有新增维度可执行 + 两份文档互相印证 + 新功能审查面覆盖率 100% + 问题模式提取零遗漏 |
...
> 🔴 **防膨胀检查**：两份验证文件历史上严重膨胀过（回归清单曾达 288 维度 3686 行，验收脚本曾达 3207 行）。每版本做一轮瘦身——既然每版都做，单次瘦身量小、负担可控。

**防膨胀轻量自检（每版本，步骤 3 之后立即跑）**
...
**fresh-eyes-review 校准（每版本）**

> 🔴 **核心原则**：fresh-eyes-review 和 regression-checklist 的更新逻辑根本不同。regression-checklist 是精确清单（加法），fresh-eyes-review 是留白式直觉审查（校准）。过去曾把两者混为一谈，导致 fresh-eyes-review 从"凭直觉发现问题"膨胀成"第二个 regression-checklist"（826 行），后重写为 274 行才修复。fresh-eyes-review 只做校准，不加检查项。

步骤 1 提取的 **C 类清单**（预料外盲区）走以下决策树：

```
预料外发现
    │
    ├─ 是可精确描述的具体问题模式？
    │   ├─ 可自动化验证？ → ✅ regression-checklist + acceptance-test（不动 fresh-eyes-review）
    │   └─ 不可自动化验证？ → ✅ regression-checklist（不动 fresh-eyes-review）
    │
    ├─ 是"凭直觉嗅到、但无法写成精确检查项"的系统性盲区？
    │   └─ 走三选一升级（见下）
    │
    └─ 是偶发问题、无规律？
...
**三选一升级**（当发现属于"直觉可感但无法精确化"时）：

| 动作 | 什么时候用 | 怎么做 | ⚠️ 禁忌 |
|------|-----------|--------|---------|
| **A. 新增视角** | 属于全新审查角度，现有视角都没覆盖 | 新增一个视角（角色+心态+举例） | ❌ 不要写成检查清单。✅ 写成"你是一个 X，你会注意到……" |
| **B. 校准现有视角** | 属于现有视角的覆盖范围，但敏感度不足 | 在举例区补一条，或微调心态描述 | ❌ 举例区不是必查清单。✅ 保持"举例，不是清单"的措辞 |
| **C. 更新历史教训** | 是反复出现的系统性问题模式 | 在末尾「历史教训」补一条 | ❌ 不要写成检查项。✅ 写成"过去在 X 出过问题，保持敏感" |

| # | 步骤 | 谁做 | 验证方式 |
|:--:|------|:--:|------|
| 7 | **fresh-eyes-review 校准**：① 回顾步骤 1 的 C 类清单，逐一走决策树 ② 需要升级的走三选一（A/B/C）③ 跑风格守护自检（见下）④ 确认可自动化验证的发现已同步到 acceptance-test | 当前 session | `git diff` 显示有更新（或确认无需更新）；风格守护自检全 PASS |

**风格守护自检**（每次更新 fresh-eyes-review 后必跑）：
...
## 🔴 阶段六：release-gate-loop 发版闸门（开新 session，一步到位）

**操作模式**：阶段五完成后，**开一个新 session**，给它一段 prompt（见下方模板），它一次性完成 acceptance 预跑 → driver 启动 → 轮询监控 → 最终汇报。

> 🔴 **阶段六必须 PASS 后才能进阶段七~八**：阶段六 FAIL 会回阶段五修复（regression-checklist / fresh-eyes-review / 源码都可能动），此时阶段七八的审查体系确认和开发日志定稿会被推翻。**禁止提前做阶段七八**——等 verdict=PASS 再进。
...
| 2 | **🔴 确认 acceptance test 的审查维度已同步**（v1.1.4 教训）：`regression-checklist.md` 维度 8「acceptance-test 健壮性」+ 维度 61「新功能零覆盖禁止」+ `fresh-eyes-review.md`「附：历史教训」5 方向（数字漂移 / 搬迁残留 / 叙事不一致 / 静默失效 / 假绿假阳性）——三处配合本版本 acceptance-test.sh 新增场景构成完整验证链。**⚠️ 场景号随版本变化**，以当前实际场景数为准 | 对照三份文件确认：regression 维度 → acceptance 场景 → fresh-eyes 5 方向形成闭环 |
...
核心两步：① `./tools/bump-version.sh <旧> <新>` ② `./tools/check-version.sh`（确认 64/64 全绿）。`npm run build` 和 `npm install --package-lock-only` 不在此处——build 需等阶段六 release-gate-loop + 阶段四 fresh-eyes-loop 所有代码修复完成后才执行（确保 dist 产物含全部修复 + 正确版本号），package-lock 在阶段十一发版前最后一步同步。
...
> 如果本版本涉及 CLI 命令迁移（旧命令改名、上帝包子命令拆到新包二进制），阶段三跳过的 shellcheck（步骤 5）在**此处补跑**——文档收尾已完成，所有引用已更新，跑出来才是真实结果。acceptance-test 不在此处补跑——它已挪到阶段六由 release-gate-loop driver 执行。
...
| 3 | 确认开发日志「发布检查清单」已全部 `[x]`（应在阶段八定稿时完成，此处只复核） | 打勾动作在阶段八，不在确认关口 |
...
> ⚠️ `npm install -g .` 从本地源码安装，不会走 npm registry。**发版时的 `npm install -g /audit@latest`（阶段十一·执行发布 Step 3 验证）仍然必须做**——那是验证发布的 npm 包。这里是发布前的自用安装。
...
| **ClawHub 版本号预发冲突（v1.2.6）** | `clawhub skill publish --version 1.2.6` 报 `Version 1.2.6 already exists` | dev 分支 session 提前预发了 1.2.6（甚至误发到 1.3.0），同版本号不可覆盖（铁律）。正式发版时版本号被占。教训：dev 分支不要预发 Skill；若必须预发用 `--dry-run` 或独立 slug。已占用时跳过 ClawHub，等下一正式版本自然覆盖 |
...
| 5 | **SOP 自我进化**（FDE 提议 → 作者确认）：FDE 发版后自动跑一轮，生成 releasing.md 更新建议（diff 格式），作者确认后 apply。检查项：<br>① 本版本发布过程中遇到的流程漏洞 → 直接吸收进对应阶段，标注版本号<br>② 检查本 SOP 中的数字是否过期（维度数、检查项数、doctor 项数等）<br>③ 本版本新增的工具/脚本是否已纳入对应阶段（如 pre-push-check.sh、check-docs.sh）<br>④ 如果 FDE 未发现需更新项，输出"无需更新"报告——零变更也是有效结果 |
...
5. **跑 `./tools/check-dev-prompt.sh ~/Desktop/vX.Y-dev-prompt.md`——有 ❌ → 修正 prompt → 重跑 check → 循环直到零 ❌**（📋 待新建不算错误）；脚本零 ❌ 后，**再按 `FORGE/playbook/dev-prompt-checklist.md` 逐条自查**（查签名准确性/注册点/已完成区等脚本拦不住的软错误），两项都过才算定稿（完整循环体见阶段十二步骤 7）
...
| 五 | 审查体系合并更新（来源提取→四份文档升级→防膨胀瘦身） | 当前 session | 否 | 步骤 1 来源提取（A/B/C 三类清单）→ 步骤 2 四份文档分发 → 步骤 3 覆盖率确认 → 步骤 4-6 防膨胀瘦身 → 步骤 7 fresh-eyes-review 校准 |
...
| 十二 | 发布后（含独立审查） | 作者 | 是（步骤 8 开新 session 读 `fresh-eyes-review.md` 做独立审查——零上下文，与下版阶段一 loop 目的不同） | npm 验证 + CI 全绿检查（步骤 3）+ 流程漏洞吸收 + SOP 自我进化 + 生成下一版 prompt（步骤 6）→ 发布后独立审查（步骤 8）→ 自动进入下版本阶段一 |
...
| v1.2.3 | **npm 发包漏 rules 包**：Step 3 验证循环只列 11 个包（漏 `rules`），导致 rules 滞留 1.2.1、其余 11 包 1.2.3 却报「全部一致」= 虚假绿色（v1.1.3 教训的重演）。修法：验证/发布清单必须与 `engine/` 全部非私有包一致，新增包同步追加；阶段十二步骤 5 数字核查时发现并补发 rules@1.2.3 | 阶段十一 |
...
| v1.2.5 | publish/验证清单漏 load-chain（engine/hooks/ 子目录非一级目录），远端滞留 1.2.1 两个版本不被检出——v1.2.3 漏 rules 虚假绿色教训的重演；包清单必须穷举 engine/ 下全部非私有包，含 hooks/ 子目录 | 阶段十一 |
| v1.2.5 | verify CI 三连修：① npm/cli#4828 缺 rollup-linux-x64-gnu（npm ci 后显式装）② 根 build 链拓扑序错误（skillopt 被 orchestrator 依赖却排在其后，本地 dist 残留掩盖、CI 全新环境暴露）③ Node 18 无全局 crypto（@langchain/core uuid rng.ts 依赖 globalThis.crypto，Node 19+ 才 flag-free）→ verify node 18→22 对齐 pr-check | 阶段十一 |
| v1.2.5 | CI 失败诊断方法：先看真实 ReferenceError/AssertionError，别凭步骤顺序猜测根因——本次一度误诊为 SOFAGENT_DATA 数据目录污染，提取全日志才发现真错误是 crypto is not defined | 阶段十二 |
| v1.2.5 | 设备端安装 EISDIR：全局目录有指向本地源码的旧 symlink（npm link 残留）时 `npm install -g` rename 失败——先删 symlink + `.audit-*` 残留再重装（ENOTEMPTY v1.0.9 的变体，多一步删 symlink） | 阶段十一 |
