...
>
> 🔴 **阶段十不可跨（v1.4.9 实锤）**：阶段九 npm publish 完成后最容易直接跳进阶段十一——分发（Skill 双平台 / DSH plugin 家族 / OpenClaw plugin 家族 / Marketplace 核查）是用户实际安装的通道，跳过 = 新版只存在于 npm，所有生态渠道停在旧版。步骤一发布后验证里的「分发渠道对账」段是事后报警面，进度追踪本行打勾是事前防线——**跨阶段十进十一 = 发布没走完**。
