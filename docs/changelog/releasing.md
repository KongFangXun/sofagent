...
- [x] 六 · release-gate-loop 发版闸门（新 session · 必须 PASS 才继续）→ [06-release-gate.md](./releasing/06-release-gate.md) · **v1.3.5：run-01 假 PASS 根治（driver 收敛判定改保守）→ run-07 四失败项 → run-08 手工裁决 PASS（86 维 0 FAIL）** · **v1.3.7：run-01/04 两轮 loop FAIL 零信任复验全部改判检查器侧债（dim106 SSOT 漏跟/dim116 awk 转义炸/跳号误报/coverage worker 偶发）已修 → 主 session 手工裁决 PASS：regression 89/89 全绿（driver 同款语义亲跑）+ coverage 12 关键词矩阵全命中 + acceptance 303/303 EXIT=0**
- [x] 七 · 审查体系最终确认 → [07-final-confirm.md](./releasing/07-final-confirm.md) · **v1.3.5：三文档闭环核对（86 维 ↔ 214 场景 ↔ 5 方向）+ acceptance 线上调撤销改瘦身回 2500** · **v1.3.7：CRS 5 FAIL 全修（87→89 维/scenario 计数口径 [a-z]? 对齐/自校验警戒线同步）→ CRS=0；89 维↔226 场景↔16 视角闭环确认**
