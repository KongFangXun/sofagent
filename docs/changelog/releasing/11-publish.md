...
- **Title**：`vX.Y.Z — {emoji 交付名} + {emoji 交付名} + ...`（常规）/ `🎉 vX.Y.Z — ...`（里程碑仅限 vX.Y.0，前缀不在尾部）
  - **每个交付名前必须带语义 emoji**（与 changelog note 内该交付的 emoji 一致——同名同 emoji）——2026-08-15 统一规则，v1.3.0~v1.3.4 五版已对齐此标准
  - 交付名挑 top 3-6 个核心交付（不是全部罗列）；尾部不加装饰后缀（🎉/🔧 等，历史漂移已清理）
- **Body 结构**（三段固定顺序，不可调）：
