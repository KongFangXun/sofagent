...
| 5 | **ROADMAP 六步**：①「现在在哪」叙事 ② 开发日志链接 ③ **迭代历程表补本版行** ④ 版本号 ⑤ 发版日期 ⑥ **已交付版本移出版本规划表 + 里程碑表**——版本规划只留 📋 规划中版本，已交付版本（✅）的唯一权威入口是迭代历程表 + CHANGELOG，不残留在规划表里。⚠️ **未来版本的 changelog 引用一律用纯文本，不用 markdown 链接**（指向不存在的文件 = 死链）。已发版的才用链接 | ROADMAP 更新 |
