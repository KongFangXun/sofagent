...
| 5 | **ROADMAP 六步**：①「现在在哪」叙事 ② 开发日志链接 ③ **迭代历程表补本版行** ④ 版本号 ⑤ 发版日期 ⑥ **已交付版本移出版本规划表 + 里程碑表 + 探索方向段**——三个段的职能边界：探索方向（想到了没排期）→ 版本规划表（已排期 📋）→ 迭代历程表（已交付 ✅），条目**单向流动**互不重复。⚠️ **探索方向同步清理规则**：已交付的整条移除（不残留「已交付」标注）；已排进版本规划表的整条移除（不残留「见版本规划表」标注）；「v1.3.x 后期」等模糊版本号要落具体版本（写进对应 changelog）或保留探索（去版本号）。⚠️ **未来版本的 changelog 引用一律用纯文本，不用 markdown 链接**（指向不存在的文件 = 死链）。已发版的才用链接 | ROADMAP 更新 |
