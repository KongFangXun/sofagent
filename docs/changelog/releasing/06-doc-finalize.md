...
> **结构 SSOT = 上一版实际发布物**（`gh release view v上一版 --json body` 动态拉取）——本节范本快照仅供追溯，**每版发布前必须重新拉取上一版对照**，禁止按本节文字直接生成（范本段与实际发布物的结构差异曾在发布前才暴露）。已知结构基线（v1.4.4）：定位句 + TL;DR (EN) + 五 H2（⚡ Install/Upgrade → 🔨 核心变更 → ✅ 质量验证 → ⚠️ 破坏性变更 → 🔗 深入了解）+ 尾链。曾出现发布物漏质量验证表与标题主题短语被作者退回——范本对照不可省。
