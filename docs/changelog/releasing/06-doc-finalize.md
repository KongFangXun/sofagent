...
> 🔴 **铁律：发布时禁止把 changelog 内嵌段直接复制当 GitHub body**——本段是格式规范源头，GitHub body 由 [09-publish.md 5.0 三道工序](./09-publish.md) 生成；两处同源同构，但生成动作在阶段九，此段只定义标准。
...
| 动作 | 细节 |
|------|------|
| 标题 | `## 现在在哪：vX.Y.Z（已交付）`——**版本号 = 当前 SSOT 版本**（check-version [11/14] 校验标题前两段 = SSOT，跨大版本时（v1.3.9→v1.4.0）写「下一版号」会撞 FAIL）。下一版方向写进叙事段，不写标题 |
