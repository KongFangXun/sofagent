...
> 🔴 **执行时逐格勾选本表「完成」列**（v1.3.6 教训：步骤 8「对话讲解」曾被漏做仍继续推进——非命令行动作没有勾选就无防漏）。全部勾完本阶段才算闭环。
...
| 7 | [ ] | **开发 Prompt 校验循环**（详见下方——脚本 + checklist 五条自查两步都要过） | prompt 定稿 |
| 8 | [ ] | **下版本内容对话讲解**（详见下方——三问讲完 + 负责人确认优先级，缺任一不算完） | 项目负责人理解下版本方向 |
...
## 发版耗时记录（v1.3.6 起追踪 · 步骤 10 自迭代的输入）
