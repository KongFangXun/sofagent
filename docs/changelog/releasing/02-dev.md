...
| D7 | acceptance 场景对账：本版每个新功能交付都有对应 acceptance 新场景（`bash tools/check/check-test-count.sh --scenarios-only` 过 + 场景数增长与新功能数匹配）——场景「顺延」= 行为锁欠账，交付时欠的账要在本阶段收口 |
...
| 改了什么 | 必须同步更新 |
|---------|------------|
| **新功能交付（任意包）** | **acceptance-test.sh 新场景行为锁**——每个新功能交付当场补对应场景，**禁止「顺延到下版」**（顺延 = 行为锁欠账：新功能上线时无行为锁，回归只能靠单测）+ 对应包内单测 |
...
| **接线收口类交付**（devlog 章出现「接线/消费/挂链」字样） | **check-unwired-exports SYMBOLS 监控表同步登记**——交付涉及的 @public 导出（符号:定义文件）必须进表，不登记 = 交付未完成（接线后门禁在册防回退） |
