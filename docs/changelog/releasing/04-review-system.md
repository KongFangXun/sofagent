...
| 六 | | **核心文档体系审查（大版本选做，小版本跳过）**：major/minor 发版或多文档大批量改动后，按 [playbook/doc-review-prompt.md](../../../playbook/doc-review-prompt.md) 复制 prompt 到新 session 跑独立人读审查（含 dashboard.html 专项）；报告落桌面 → 零信任复核 → 修复批 → 收敛判定 GREEN 后放行。轮次策略与已知问题类型检查表见该 playbook（v1.5.0 四轮实跑沉淀）。**与 fresh-eyes 的分工**：fresh-eyes 管代码审查，doc-review 管文档人读体验——两者互补不互替 | 审查报告 + 复核记录 + 收敛 GREEN |
