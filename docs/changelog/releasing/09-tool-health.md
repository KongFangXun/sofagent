...
| # | 步骤 | 验证方式 |
|:--:|------|------|
| 一 | **🔴 跑工具健康门禁**：`bash tools/check-tool-health.sh`——六项自动检查：审查文档路径活性（含 CI glob 前缀引用验证）/ 孤儿配置排查 / bump↔check 结构对照 / hook 头版本标记 vs SSOT / CI workflows 引用有效性 / **set -u 新变量初始化守卫**（`VAR="${VAR}..."` 首次赋值即自引用且无前置初始化 = 炸弹——v1.3.7 test-count.sh FLAKY_PKGS 实案，含 `local`/`declare`/`do` 后缀形态） | 脚本全绿（RC=0）；FAIL 按输出逐条修复 |
...
| 四 | **🔴 `npm run build` 重建 dist 产物**（源码改了 dist 没改 = CLI 版本号不对） | `node engine/audit/dist/index.js --help` 显示正确版本 |
| 五 | **跨文档锚点校验** | `node tools/check-anchors.mjs` 全绿 |
| 六 | **🔴 hook 端到端实测**（见下方脚本） | 拦截 exit 2 + 放行 exit 0 |
| 七 | **shell 变量定界守卫**（`tools/*.sh` 中 `$VAR` 后紧跟 CJK 全角标点 → set -u 崩溃） | `bash tools/check-cjk-var.sh` 全绿 |
