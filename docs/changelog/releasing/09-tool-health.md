...
| 三 | **三脚本对照检查**：① pre-push-check.sh 的检查项数量是否和 CHANGELOG/ROADMAP 声明的一致？② bump-version.sh --dry-run 必须验证为纯只读（跑完后 `git diff --stat` 零改动）。（check↔bump 结构对照与 check-version 分母已由步骤一 脚本覆盖） | 跑脚本对照 |
