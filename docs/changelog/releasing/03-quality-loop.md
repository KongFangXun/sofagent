...
>
> **执行方式（自动收敛模式，单 session 制）**：步骤二 driver 及其修复兜底**走新 session**（用户手动开新 WorkBuddy 窗口），主 session 用下方「执行 session Prompt 模板」生成 prompt 直接在对话中输出（不落盘文件），用户复制粘贴到新窗口执行。执行 session 独立跑完整循环：driver 多轮审查修复（单盲四角色流水线：A 审 12 视角 → B 修 → C 验 → D 复核，REOPEN 打回消耗修复批额度）→ 跑完仍有 P0/P1 残留则 session 按「修复批协议」分诊修复 → 复绿 commit → 重跑 driver → 直到「连续 2 轮无 P0/P1」（driver 内建停止条件）或命中停手条件（见模板）。主 session 不代跑 loop——只负责复验收编与停手接手分诊；**四角色分离在自动收敛下依然成立**（发现=A worker / 修复=b-fix 或执行 session / 验收=c-verify / 裁决=d-review + 主 session 收编，见「修复分工与角色分离」专节的角色映射表）。
>
...
> driver 兜底（单盲四角色，A 侧 12 视角 worker）只用于「待取证」项多或大版本变更。
> 对话式多轮审查（主会话按 playbook 视角人肉多轮）与两者产出等价，可互换——见 01-review.md 审查分层第三层。取用配置参照 playbook 分层表：常规发版 1-12 必跑，动态面 17-19（工具/门禁/hook/引擎改动版建议追加）、文档治理 13-14（文档大改版建议追加）、通读 15-16、发现面 22（门面改动）按版型选层；深度专项 20-21 留季度体检。
...
| 步骤 | 完成判据（全满足才可打勾） |
|---|---|
| 一 草稿 | `~/Desktop/fresh-eyes-draft-vX.Y.Z.md` 存在 **且** 含全部 16 个视角节（`grep -c "^## 视角" = 16`——草稿工具静态子集 1-16；动态面 17-19 由 driver/人工兜底取证）。若走了降级：产物为 `.prompt.md` 时 = **未完成**，必须粘贴执行出正式草稿后才算。若走对话式多轮形态（01-review.md 第三层）：产物为桌面 `vX.Y.Z-bugfix-prompt.md`（含问题总表 + 逐项修复方案 + 验证命令）**且**修复批已收编 commit——两者满足其一即可 |
| 二 driver（若应跑） | runDir 内有 `verdict`/`findings` 产物**文件**——仅 status.json 不算（dry-run 空转也是 completed 状态）。未应跑时：汇报须显式写「步骤二未跑，理由：待取证 N≤3」；**环境异常豁免**（判据命中但 driver 无法启动/无法收敛）须写「步骤二环境豁免：<故障形态> + 补偿措施」并经主 session 复验认可——两种豁免都须留痕，禁止静默跳过 |
...
**🔴 步骤二环境异常豁免协议（判据命中但 driver 跑不了时）**：

草稿判据命中应跑 driver（待取证>3 或 P0 需独立复验），但环境故障使 driver 无法执行（LLM 端点 300s 大输入超时 / DSH 依赖漂移 / 系统性 worker 全灭）时，按下述**降级执行链**补偿，禁止裸跳过：

1. **豁免资格前置**：driver 至少真实启动尝试 2 次（含 `--resume` 续跑与平价时段重试各一），全部失败才具备豁免资格——启动尝试与失败形态记入汇报
2. **补偿措施（按优先级）**：
   - a. **步骤一草稿升格**：草稿已产出的（含降级路径产出的正式草稿），由主 session 对草稿 P0/P1 项**零信任复验**（逐项 grep 实证，不采信草稿结论）后，以草稿审查+复验替代 driver 盲审——草稿降级路径（`.prompt.md` 交独立 subagent 执行）本身已保证「审查者≠收编者」角色分离
   - b. **修复批走独立 session**：残留修复必须落独立 session（或 subagent）执行，主 session 零信任复验收编——角色分离不断
   - c. **阶段五闸门兜底**：阶段五 release-gate driver 的 regression/coverage worker 会重扫全维度，driver 缺位的盲区由闸门兜底——豁免汇报须注明「依赖阶段五兜底」
3. **留痕铁律**：汇报写「步骤二环境豁免：<故障形态> + 补偿措施 a/b/c」；LEDGER 无需补行（fresh-eyes run 未启动不产生 run 记录），但 changelog devlog 的阶段三章节须注明豁免与补偿形态——**未来读者必须能看出这版没跑 driver 及其原因**
...
> **自动收敛语义**：driver 内建「审查→修复→验证」多轮循环与「连续 2 轮无 P0/P1」停止条件；执行 session 的增量职责 = driver 跑完仍有 P0/P1 残留时按「修复批协议」接手修复并重跑 driver（外层上限 2 次修复批）——角色分离不破坏（见下方角色映射表）。
...
| 独立验收 | c-verify（C 验收者，逐条实测不采信自报）+ d-review（D 复核者，P0/P1 裁决 CONFIRM/DOWNGRADE/REOPEN） | C/D 与 A/B 零信息通路——A 不再兼任验证（原告兼法官问题消除） |
| 复验收编 | **主 session 零信任复验后收编** | 执行 session 修复批自跑的验证 ≠ 复验收编；主 session 打勾前逐项 grep 实证（见「步骤完成判据」） |

---

## 修复分工与角色分离

> **为什么**：曾出现 loop 结构性不收敛后主 session 停轮**直接代做修复 + 自己复验 + 自己收编**——发现问题（worker）、修复、复验角色重合，「谁来检查检查者」缺位，被迫事后开新 session 补审。固化为以下分工（loop 内已落地四角色流水线，session 层守住最后的复验收编位）：

| 角色 | 承担者 | 职责边界 |
|------|--------|---------|
| **发现问题** | loop 的 A worker（独立子会话，12 视角单盲） | 审查 + findings 产出，不修 |
| **修复执行** | **独立 session**（用户开新窗口，或工程师寇豆码）；loop 内为 b-fix worker | 消费 result.md / b-fix 修复规格（summary-batch-*.md），机械应用到主仓库并 commit |
| **独立验收** | loop 内 c-verify（C）/ d-review（D 复核裁决） | C 逐条实测验收、D 对抗复核 P0/P1——与发现/修复零信息通路 |
...
**driver 天然进程独立**（daemon detached，不依赖任何 session 存活）——「新 session」要求的本质不是进程隔离而是角色隔离：审查者 ≠ 修复者 ≠ 复验者。loop 在哪个 session 启动均可，收口时守住角色分离即可。

**自动收敛模式下的角色映射**（单 session 制不破坏角色分离——分离的对象是「行为」不是「session 实例」）：执行 session 同时承担「跑 loop」与「兜底修复批」，但它相对四角色始终只占「修复执行」位——发现永远来自 A worker（独立子进程，单盲 12 视角），验收永远归 c-verify/d-review（独立验收/复核），复验收编永远归主 session（打勾前逐项 grep 实证）。危险组合是「同一行为主体既发现又修复」或「既修复又复验收编」——自动收敛模板的红线与停手条件就是防这两个组合的。
