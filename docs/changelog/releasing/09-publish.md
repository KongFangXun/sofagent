...
## 授权边界两段式（自动收敛在本阶段的适用边界）

阶段九的步骤分两类，授权语义不同（🔴 与对外动作铁律对齐：流程顺序 ≠ 自动授权）：

| 类别 | 覆盖步骤 | 授权语义 |
|------|---------|---------|
| **准备类**（可逆/只读） | 前置（lock file + 内部依赖一致性）/ 步骤一（本地狗粮）/ 步骤二（发布前检查）/ 步骤三（push 前置检查）/ 步骤四的 push 前检查与 CI 轮询等待 | 可连续执行；红项按 [`auto-converge-protocol.md`](./auto-converge-protocol.md) 修复批协议自行修复复绿（修产品不修测试红线同款适用），修复后重跑该检查至 EXIT=0 |
| **发布类**（对外可见，不可逆） | 步骤四的 `git push` 本体 / 步骤五（安装入口 bump commit）/ 步骤六（git tag + push tag）/ 步骤七（gh release）/ 步骤八（npm publish） | **每个动作前显式请示**，停下等作者明确指令才执行；报错暂停请示，不自行重试对外动作 |

**判别口径**：该步骤失败后是否需要撤回「已对外可见的状态」——需要 = 发布类；只是本地修复重跑 = 准备类。步骤四横跨两类：push 前检查属准备类，`git push` 命令本体属发布类——执行到 push 本体前必须停下请示。与 [08-confirm「AI 代执行边界」](./08-confirm.md) 一致：授权 AI 跑命令 ≠ 授权 AI 管凭证，npm 凭证始终在人的控制下。
...
> **本节是 GitHub Release body 的强制规范——所有版本必须遵守，不可漂移。质量表 7 项钉死，禁止增减（曾连续多版各缺不同项的漂移）。**

- **Title**：`vX.Y.Z — {emoji 主题短语} + {emoji 主题短语}`（常规，≤3 个短语）/ `🎉 vX.Y.Z — ...`（里程碑仅限 vX.Y.0，🎉 前缀在版本号前，不在短语尾部——N11）
  - **主题短语 ≠ 交付名清单**——title 讲主线故事（如「🏪 组织能力市场（L3 五环）」），不逐项罗列交付名（那是 note 新功能段的事）。**title 与 note 新功能禁止逐字重复**
  - 每个主题短语带语义 emoji；尾部不加装饰后缀
- **Body 结构**（定位句 + 五段，固定顺序不可调）：
  0. **首行定位句**——`{emoji 主题短语呼应 title}——{一句人话}`，如「🏪 协作成果像内部应用商店一样流转——Skill/Agent/流程可发布、可评分、可退役，第三方 Skill 先过安全门。」，**≤220 字符**（N9，超线拆 H3 承载）。**禁止用旧 title 清单复读**（曾出现 body 首行挂着交付名清单、title 改了 body 没跟）。与 title/H1 分工：title 点主题（名词短语）/ H1 讲故事（动词句）/ 定位句说人话（给用户的价值）
  0b. **定位句紧随英文 TL;DR**——`**TL;DR (EN):** {one-line English summary}`：GitHub Explore 抓取 release 首段、marketplace/awesome 清单引流海外用户——纯中文首段等于对非中文读者隐身。一句话覆盖「这版是什么 + 关键数字」，≤280 字符。**禁用嵌套代码围栏**（Install 块若用 ``` 围栏会截断外层结构，统一用 4 空格缩进块）
  0c. `## ⚡ Install / Upgrade` 速查块——三行命令直接可抄：新装（install.sh）/ 升级（`npx @sofagent/audit@vX.Y.Z --doctor`）/ GitHub Action（`uses: KongFangXun/sofagent@vX.Y.Z`）。用户不该翻 HANDBOOK 找升级命令
  1. `## 🔨 核心变更`——功能按重要性降序，安全修复优先于文档修复。每个变更点用 `-` 列表，一句话说清楚做了什么（不写「为什么」——那在开发日志里）。**重点小节补文档深链**（如 FDE 引擎段链 GUIDE/WIKI）
  2. `## ✅ 质量验证`——**固定 7 项表格**（见下方模板），结果列每项带 ✅，缺一项即视为体例不合规
  3. **破坏性变更前置加粗 + 迁移命令**——`## ⚠️ 破坏性变更` 段内，每条破坏性变更不只说现象，**必须附「如遇 X，执行 Y」的解法命令**（曾出现只说改了什么、用户撞墙后要去翻文档找解法）
  4. `📖 [详细开发日志](./docs/changelog/v<major>.<minor>/vX.Y.Z.md)`——必须用 markdown 链接语法，放最末；上方加 `## 🔗 深入了解` 导航表（CHANGELOG / HANDBOOK / Discussions / npm 四行），锚点见工序三
...
- `release-gate` 是发版闸门、`fresh-eyes` 是独立审查——两者是「是否值得发版」的质量证据，必须列，禁止省略
...
- **BugFix 段在核心变更最末**（不独立成 H2）——`### 🔒 BugFix（上版遗留）`逐字固定，补语不可省（N10），用 `-` 列表
