...
**工序三 · 上一版结构对照（2026-08-19 用户拍板：取代人工过目）**：自检全过后，与上一版 release body 做**结构级并排对照**——title 形式（`vX.Y.Z — emoji 短语`，里程碑 `🎉 vX.Y.0 —` 前缀规则见 N11）/ 定位句有无 + 长度（N9 ≤220 字符）/ 英文 TL;DR 有无 / Install 速查块有无 / H2 骨架 / 质量表 7 项顺序 / BugFix 节标题逐字（N10）/ 破坏性变更迁移命令 / 尾链位置，十要素逐一比对上一版，**结构不一致即重写，直到同构**。机制标准 = **v1.4.2 实际发布物（2026-09-01 拍板的标准锚点，含英文 TL;DR + Install 速查 + 迁移命令 + 深入了解导航表；v1.3.7 为旧三段式锚点，仅历史参考）**——若上一版漂移，以 v1.4.2 为准重写，不追随上一版。对照命令：`gh release view v1.4.2 --json name,body -q '{name, body}'`（锚点）+ `gh release view 上一版 --json name,body`（漂移检测）。
...
- **Body 结构**（定位句 + 五段，固定顺序不可调）：
  ...
  0b. **定位句紧随英文 TL;DR**——`**TL;DR (EN):** {one-line English summary}`（2026-09-01 拍板新增，v1.4.2 首应用）：GitHub Explore 抓取 release 首段、marketplace/awesome 清单引流海外用户——纯中文首段等于对非中文读者隐身。一句话覆盖「这版是什么 + 关键数字」，≤280 字符。**禁用嵌套代码围栏**（Install 块若用 ``` 围栏会截断外层结构，统一用 4 空格缩进块——v1.4.2 实操踩坑）
  ...
  3. **破坏性变更前置加粗 + 迁移命令**——`## ⚠️ 破坏性变更` 段内，每条破坏性变更不只说现象，**必须附「如遇 X，执行 Y」的解法命令**（v1.4.2 教训：doctor 基线变更原版只说改了什么，用户撞墙后要去翻文档找解法）
  4. `📖 [详细开发日志](./docs/changelog/v<major>.<minor>/vX.Y.Z.md)`——必须用 markdown 链接语法，放最末；上方加 `## 🔗 深入了解` 导航表（CHANGELOG / HANDBOOK / Discussions / npm 四行），官方锚点 v1.4.2 body
