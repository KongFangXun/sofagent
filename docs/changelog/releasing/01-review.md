...
| 第二层：盲审兜底 | harness 注入主任务协议（`FORGE/SKILL/fresh-eyes-loop/loop.md`「执行形态」节）+ 执行器 `FORGE/src/fresh-eyes-driver.mjs --worker` | 单盲 A 侧 12 角色步 × 多轮（legacy 双盲 24），单轮 6-10 万 token | 草稿「待取证」项多 / 大版本变更 / 草稿结论存疑时全量跑 |
