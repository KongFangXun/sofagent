...
>
> 开发完成快照（2026-08-09 第三批收尾）：11 项交付 · 1712 tests across 12 packages 全绿 · check-version 67/67 · FORGE smoke 11/11
>
...
- [ ] config.yml `memory_backends` 段 schema 校验通过；`enabled: false`（缺省）时不加载任何外部依赖、不发起任何外部请求
- [ ] `enabled: true` 时 memory_search / memory_write / skill_extract / wiki_search 四个工具可调用
- [ ] sensitivity → ACL 映射正确（restricted Agent 拿不到 private 记忆）
- [ ] endpoint 不可达时优雅降级（warn + skip，不 crash）
- [ ] MA4：`FORGE_MEMORY_BACKEND` unset 时 FORGE 行为与 v1.2.9 完全一致；启用时 worker 间经验可互检
- [ ] MA5：@daily 正确提取高频决策写入 `forge/decisions`
- [ ] MA6：`type: "workbuddy"` 正确调用 conversation_search；`type` 缺省 `"mcp"`
- [ ] MA7：`extractRuleContext()` 按 rule 聚合写入 `rules/why`，不启用后端时不影响 MA5
- [ ] FDE/GUIDE.md 段落已添加
- [ ] `npm test` 全绿
...
- [ ] think.md / knowledge/ / 状态文件写入全部走 atomic-write（无裸 writeFileSync 残留）
- [ ] mtime 检测：他人改写 → 重读合并再写，不盲目覆盖
- [ ] dream-cycle 全流程回归（写经验不丢）
- [ ] `npm test` 全绿
