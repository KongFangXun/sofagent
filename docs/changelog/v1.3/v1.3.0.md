...
> 来源：OpenFDE/ChatDemo `decisions.jsonl`（ROADMAP 第 324 行 #3，最高优先）+ OpenFDE 主仓 INDUC / Judgment Unit（ROADMAP 第 333 行起）
...
来源可追溯：ChatDemo `decisions.jsonl` schema `{kind, moment, why, spec_ref}` 现场即时记、会后喂 FDE Loop→INDUC→Judgment Unit（ROADMAP 第 324 行）；主仓 INDUC 把「经验→判断」显式成阶段、Judgment Unit = 专家判断资产化、可开关、可版本化（ROADMAP 第 339 行）。
...
| `FORGE/tools/fresh-eyes-driver.mjs` | wrapToolCall 处调用 `emitDecision`（运行时决策） | 编辑（实现轮） |
...
**`moment` 阶段枚举（LoopPhase）** — 决策发生的循环阶段，对齐 FDE Loop 五阶段（ROADMAP 第 335 行 `OBSERVE→ELICIT→INDUC→ACT→EVOLVE + DEPLOY/ATTRIBUTION`），并映射 FDE 四阶段十二步：
...
| **T03** | 受控 API 暴露 + 运行时接入 | `engine/audit/src/public-api.ts`（emitDecision 导出）、`FORGE/tools/fresh-eyes-driver.mjs`（wrapToolCall 调 emitDecision）、`engine/audit/src/index.ts`（包导出） | T02 | P1 | v1.3.0 |
