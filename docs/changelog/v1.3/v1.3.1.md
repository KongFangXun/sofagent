# v1.3.1 开发日志 — Ontology 升级为可运行推理底座（EXECUTE 阶段能力深化）

> ✅ 已开发 · 2026-08-09 交付（五波次 + 补漏波次 6 收尾）
> 前置依赖：v1.3.0（运行时审计最小闭环 + 激活链 Phase 4 收尾）
> 交付：14 项 · **1962 tests across 12 packages 全绿** · acceptance 177 场景 · 本体建模要求对齐 `runRules({gb48000:true})` opt-in
> 激活链设计文档：[docs/guides/fde-activation-chain.md](../../guides/fde-activation-chain.md)

## 背景

v1.3.0 完成运行时审计最小闭环（wrapToolCall middleware 包 createReactAgent）后，v1.3.1 是激活链 EXECUTE 阶段的**能力深化版本**——14 项交付并行推进，围绕三条主线：①Ontology 从「描述性元数据」升级为「可运行推理底座」；②并行编排闭环（DAG 并行 + 审计卡关 + MergeQueue 合并 + Durable 续跑）；③工程鲁棒性（身份码/审批/Trace/错误处理）。各交付间的协同与依赖见开发 Prompt §十八任务依赖图。

### 交付清单速览

| # | 优先级 | 交付 | 一句话 | 详见 |
|---|:---:|------|--------|------|
| 1 | P0 | 🧬 Ontology 运行时层 | Action 注册表 + validator 三态 + Schema 定稿 | §一 |
| 2 | P1 | 📋 本体建模要求对齐 GB/T 48000.3-2026 | 审计/Ontology 合规参考基线（opt-in） | §二 |
| 3 | P0 | ⚡ 并行编排 | LangGraph DAG 波次并行 + 审计卡关 | §三 |
| 4 | P1 | ⏸️ Durable Execution L1+L2 | checkpoint 续跑 + 工具幂等性 | §四 |
| 5 | P1 | ✏️ Ontology CRUD 补全 | update/delete entity/concept（强制人审） | §五 |
| 6 | P1 | 🆔 Agent 身份码 Ed25519 | 跨设备验证 + 审计绑定（原 v1.3.2 拆入） | §六 |
| 7 | P1 | 🔗 跨设备审计轨迹聚合 | 按 agentId 合并 + trust 优先级裁决 | §七 |
| 8 | P0 | 🚀 Onboard Agent L1 | FORGE 产品化第一刀（工程判定层） | §八 |
| 9 | P1 | 📊 Benchmark 评测 | 隔离执行 + HMAC 链（PenguinHarness 借鉴） | §十二 |
| 10 | P1 | 🔒 工具审批四模式 | wrapToolCall 运行时拦截增强 | §十三 |
| 11 | P1 | 📜 LLM 调用级 Trace | 全量 token/耗时/stopReason 追踪 | §十四 |
| 12 | P1 | 🔄 错误处理升级 | stop_reason 六值 + 指数退避 + 收敛为消息 | §十五 |
| 13 | P1 | 🔀 MergeQueue | 并行编排的并发一致性底座 | §十六 |
| 14 | P2 | 📚 L4 渐进加载增强 | 索引注入 + 按需读取（~4000→~1500 token） | §十七 |

> P0 = 核心承诺（不做不能发版）；P1 = 能力深化；P2 = 体验优化。§九/§十/§十一为待明确事项/依赖/后续版本依赖，非交付项。

---

## 一、🧬 Ontology 运行时层（P0 · 操作型本体论落地）
...
**① 统一元模型**：对齐 Harness 规则（A1-A23 共 23 条 + E1/E2/E4 共 3 条 = 26 条注册，其中 E3 并入 A11 不独立计数；实际生效 24 条——17 默认 A1-A11+A18-A23 + 7 扩展 A14-A17+E1/E2/E4）+ 记忆 Ledger-Views-Policy。Ontology 的 Action Type 与审计引擎七步管线对齐（OLAF-I 五块骨架印证，见 [VALIDATION §三](../../VALIDATION.md)）。
...
**⑤ Ontology Schema 定义（v1.3.6 接口外化前移 · 模型层对接前置）**：entity / concept / relations 的数据结构**随本体落地同步定稿**为机器可读 JSON Schema（对齐内核契约 CORE-OBJ / CORE-LNK 的字段约束）。v1.3.6 的「Ontology 标准 Schema + 注册接口」只做**对外暴露面**（MCP 注入接口），格式定义在本版定稿——避免 v1.3.6 补 schema 时返工或做兼容层。Schema 即单一事实源：validator 校验、审计对齐、未来模型层注入全部复用同一份定义。

> 🔗 **与 v1.3.6 的关系**：本版定 Schema 格式（数据结构层），v1.3.6「接口外化第一批」做注册接口 + 外部注入（暴露层）——格式先定，暴露后做。
...
| 文件 | 改动 | 说明 |
|------|------|------|
| `engine/orchestrator/src/ontology/schema/entity.schema.json` | 新建 | entity JSON Schema（含 frontmatter 字段） |
| `engine/orchestrator/src/ontology/schema/concept.schema.json` | 新建 | concept JSON Schema |
| `engine/orchestrator/src/ontology/schema/relations.schema.json` | 新建 | relations JSON Schema（方向 + 基数，对齐 CORE-LNK） |
| `engine/orchestrator/src/ontology/schema/index.ts` | 新建 | schema 导出（validator 与未来注册接口复用） |
...
| `engine/orchestrator/src/middleware/progress-mw.ts` | 修改 | 集成 Ontology validator（v1.3.0 交付的 middleware 上扩展） |

### 验收标准

- [x] Action 注册表存在且 Ontology Action → 工具映射可用
- [x] validator 三态（PASS/WARN/FAIL）在执行前拦截
- [x] entity/concept/relations 三份 JSON Schema 定稿（零依赖手写校验器）
- [x] CORE-OBJ/ACT/LNK/STM 四类内核契约完整
- [x] `tools.ts` wrapToolsWithGate 不传 ontologyValidator 时行为 = v1.3.0（零变化）

---

## 二、📋 本体建模要求对齐 GB/T 48000.3-2026（P1）

### 交付

将 GB/T 48000.3-2026《标准数字化 第 3 部分:本体建模要求》作为审计层 / Ontology 层的合规参考基线。不是「通过认证」，而是「对齐本体建模要求条款、作为 ontology 合规映射依据」——**非行为审计国标**。

### 工作内容

- 映射本体建模要求条款 → sofagent Ontology 元模型字段
- 审计报告新增「本体建模要求对齐」维度（opt-in，不影响默认行为）
- 文档标注：哪些条款已对齐、哪些部分对齐、哪些不适用

### 验收标准

- [x] `engine/audit/src/gb48000.ts` 条款映射清单存在（8 条：已对齐 5 / 部分对齐 2 / 不适用 1）
- [x] 审计报告 `runRules({gb48000:true})` opt-in 维度生效（ruleClass='工程规范' 按 name 排除 exitCode）
- [x] 默认行为零变化（不传 `gb48000` 时审计输出与 v1.3.0 一致）

---

## 三、⚡ 并行编排（P0 · LangGraph 原生 DAG 波次并行）
...
- [x] StateGraph 支持 Send API 并行波次（parallel_wave 可选节点，默认串行零变化）
- [x] 审计节点全 PASS → 合并；任一 FAIL → 丢弃对应 worktree
- [x] 文件隔离复用 v1.2.3 git worktree（不重建隔离底座）
- [x] 任务 ID 查重与 §四 Durable L2 共用同一机制

---

## 四、⏸️ Durable Execution L1+L2（P1 · Pydantic AI 启发）
...
| `FORGE/src/audit-middleware.mjs` | 修改 | 工具调用前写登记簿（复用 v1.3.0 middleware） |
...
- [x] L1：daemon 重启后扫描 `.sofagent/checkpoint/` → 有未完成 graph → 从中断节点恢复
- [x] L2：`side-effects.jsonl` append-only 登记簿 + 续跑前查重（幂等）
- [x] checkpoint 默认保留 7 天可配
- [x] 外部副作用（PR/webhook）查重，不重复执行
- [x] LLM 节点崩溃后续跑用缓存结果或跳过（不重放）

---

## 五、✏️ Ontology CRUD 补全（P1 · MCP 覆盖度审计缺口）

### 背景

v1.2.4 交付了 `create_entity` / `create_concept`（含 D1-D5 数据审计），但本体只有「建」没有「改/删」——过期/错误的 entity 只能手工清理。MCP 覆盖度审计将本体 CRUD 补全列为 P1 缺口，用户确认归入本版。
...
- [x] `update_entity` 支持字段级更新/改 domain/改名（区别于 create 全量覆盖）
- [x] `delete_entity` / `delete_concept` 强制人审 confirmed（无人审不执行）
- [x] 三个 tool 全程 D1-D5 审计留痕

---

## 六、🆔 Agent 身份码 Ed25519 + KYA 完整版（P1 · 原 v1.3.2 拆入）
...
| 文件 | 改动 | 说明 |
|------|------|------|
| `engine/core/src/agent-identity.ts` | 修改（已存在轻量版） | 升级为 Ed25519 签发/验证完整版 |
| `engine/core/src/identity-store.ts` | 新建 | 身份注册表（本地 data/ 存储） |
| `engine/audit/src/audit-history.ts` | 修改 | 审计记录增加 agentId 字段（AuditHistoryEntry 扩展） |
| `engine/mcp/src/tools/agent-identity.ts` | 新建 | MCP `agent_identity` tool（查自己/查他人身份） |

### 验收标准

- [x] `agent-identity.ts` Ed25519 keypair 签发/验证完整版可用
- [x] `identity-store.ts` 本地注册表（data/ 存储）
- [x] 审计记录带 agentId 字段（AuditHistoryEntry 扩展）
- [x] MCP `agent_identity` tool 可查自己/查他人身份

---

## 七、🔗 跨设备审计轨迹聚合（P1 · 原 v1.3.2 拆入）
...
- [x] `audit-merge.ts` 按 agentId 合并 + HMAC 验签 + trust 优先级裁决（低 trust 不覆盖高 trust，tampered 丢弃）
- [x] `audit-trail.ts` @daily 巡检聚合完整轨迹
- [x] MCP `audit_trail` tool：按 agentId 查完整轨迹 / 无 agent_id 列出全部
- [x] 注册进 tool-registry + mcp-server + inspectors/index（MCP 工具数 38→39）

---

## 八、🚀 Onboard Agent L1（P0 · FORGE 产品化第一刀 · 工程判定层）
...
- [x] `judge.ts` L1 判定器：crash/error/超时三态（不判语义对错）
- [x] `driver.ts` 循环驱动：activate→run→judge→fix→re-run（convergeToolError 收敛不中断）
- [x] 调试记录带 agentId（与 §六 身份码协同）
- [x] llm-call-trace（§十四）定位失败点
- [x] MCP `loop_debug` tool 可触发调试循环 + 查结果
...
| v1.3.7 权限体系 + v1.3.8 代理网关 | 身份码 + 审计轨迹（权限判定依据） |

---

---

## 十二、📊 Benchmark 评测体系（P1 · Onboard Agent L1 前置能力）

> 📖 **设计来源**：PenguinHarness (github.com/Prism-Shadow/penguin-harness, Apache-2.0) 的 `benchmark-design` + `agent-evaluation` 方法论。完整设计文档见 [agent-methodology.md](../../archive/design/agent-methodology.md)。
...
- [x] statement/rubric 物理分离：被测 Agent 无法访问 rubric 目录
- [x] Pilot 校准：初稿跑一轮 → 分析 → 调难度 → 第二轮
- [x] Freeze：Benchmark revision + Agent 版本号一起冻结
- [x] Formal Baseline：记录到 evaluation-log.jsonl（HMAC 链）
- [x] 隔离执行：每次评测独立 workspace，只暴露 statement
- [x] 四种失败码：invalid_request / benchmark_invalid / version_changed / evaluation_failed
...
## 十三、🔒 工具审批模式（P1 · wrapToolCall 运行时拦截增强）

> 📖 **设计来源**：PenguinHarness CLI `--approve` 四模式（allow-all / deny-all / read-only / always-ask）。完整设计文档见 [agent-methodology.md](../../archive/design/agent-methodology.md) §6.1。
...
- [x] 四种模式正确工作：allow-with-audit 全放行 / deny-all 全拦截 / read-only 只放行 r / always-ask 每次问
- [x] 保守默认：SDK 未传审批回调时默认拒绝一切
- [x] 审批继承：子 Agent 继承父 Agent 审批模式
- [x] Benchmark 评测时 Test Agent 强制 read-only（无法写文件/执行命令）
- [x] 每次审批决定记录为 `approval_decision` 事件（运行时审计日志）
- [x] 拒绝产生合成中止消息返回给 Agent（不崩溃）
...
> 🧠 **与 v1.3.7 沙箱的关系**：read-only 审批模式是 v1.3.7 完整沙箱的前置能力——先有审批模式控制权限范围，v1.3.7 再加虚拟文件系统 + 网络出站白名单做硬隔离。审批模式是「软隔离」（逻辑层），沙箱是「硬隔离」（OS 层），两者互补。

---

## 十四、📜 LLM 调用级 Trace（P1 · 全量追踪）

> 📖 **设计来源**：PenguinHarness OmniMessage 协议 + Sessions & Traces 的「Trace 是唯一恢复源」设计。完整设计文档见 [agent-methodology.md](../../archive/design/agent-methodology.md)。
...
- [x] 每次 LLM 请求写一条调用记录（token/耗时/stopReason/error）
- [x] 记录进 HMAC 链（防篡改）
- [x] Benchmark 评测失败可按 taskId 回放模型调用链
- [x] cost-baseline 可消费调用级 token 数据
- [x] `npm test` 全绿
...
## 十五、🔄 错误处理升级（P1 · stop_reason 分类 + 指数退避 + 错误收敛为消息）

> 📖 **设计来源**：PenguinHarness Agent Loop 的六值 stop_reason + 自动重连（≤5 次指数退避，auth 永不重试）。完整设计文档见 [agent-methodology.md](../../archive/design/agent-methodology.md)。
...
- [x] auth 错误（401/403）永不重试
- [x] timeout/malformed/failed 按退避阶梯重连（2s→4s→8s→16s→30s，≤5 次）
- [x] 工具失败收敛为结构化消息给模型（不抛异常中断）
- [x] stop_reason 写入调用 Trace（§十四）
- [x] `npm test` 全绿
...
## 十六、🔀 MergeQueue 并发合并（P1 · Message Flow 参考）
...
- [x] 多 SubAgent 并发产出经 MergeQueue 有序 yield（不丢不乱）
- [x] 进模型上下文前重排回原始 taskId 顺序
- [x] 每个 SubAgent 任务恰一个结果（taskId 配对）
- [x] 审计节点全 PASS 才合并（与 §三 卡关一致）
- [x] `npm test` 全绿
...
- [x] 热点 2 篇全文注入（保持现有行为）
- [x] 索引 9 条只注入摘要（≤150 字符/条）
- [x] 总注入量从 ~4000 token 降到 ~1500 token
- [x] Agent 可按索引用 read_file 读取全文
- [x] 既有测试全绿（不破坏现有注入语义）
...
## 实现纪要（2026-08-09 交付 · 五波次）

> 五波次开发全部在 `dev/v1.3.1` feature branch 完成（每步 commit，不 push）。

### 波次 1 — 核心地基（交付 6/10/11/12）

- **交付 6 Agent 身份码**：`engine/core/src/agent-identity.ts` 升级 Ed25519 签发/验证完整版 + `identity-store.ts` 本地注册表 + `engine/mcp/src/tools/agent-identity.ts` MCP 查询工具
- **交付 10 工具审批模式**：`engine/rules/src/approval-mode.ts` 四模式（allow-with-audit / deny-all / read-only / always-ask）+ `FORGE/src/audit-middleware.mjs` 审批分支（保守默认拒绝 + 审批继承 + approval_decision 审计事件）
- **交付 11 LLM 调用级 Trace**：`engine/core/src/llm-call-trace.ts` append-only + HMAC 链（复用 core 原语，白名单脱敏）
- **交付 12 错误处理升级**：`engine/core/src/stop-reason.ts` 六值分类 + 指数退避（2s→4s→8s→16s→30s）+ `convergeToolError` 工具失败收敛为消息

### 波次 2 — Ontology 运行时层 + CRUD（交付 1/5）

- **交付 1 Ontology 运行时层（P0）**：`engine/orchestrator/src/ontology/`——Action 注册表（action-registry.ts）+ 执行前 validator 三态（validator.ts）+ CORE-OBJ/ACT/LNK/STM 四类内核契约（contracts.ts）+ entity/concept/relations 三份 JSON Schema 定稿（schema/，零依赖手写校验器）+ `tools.ts` wrapToolsWithGate 可选 ontologyValidator 集成（不传 = v1.3.0 行为零变化）
- **交付 5 Ontology CRUD 补全**：`engine/mcp/src/tools/update-entity.ts`（字段级更新 + 改名）/ `delete-entity.ts` / `delete-concept.ts`（强制人审 confirmed）+ D1-D5 审计留痕

### 波次 3 — 并行编排 + Durable + MergeQueue（交付 3/4/13）

- **交付 13 MergeQueue**：`engine/orchestrator/src/loop/merge-queue.ts` 多生产者→单消费者——到达序 yield + 原始 taskId 序重排 + 配对保证（重复 push 拒绝）
- **交付 3 并行编排**：`loop/parallel-scheduler.ts`（波次并发分发 + worktree 隔离）+ `loop/merge-gate.ts`（波次审计卡关：全 PASS 合并 / 任一 FAIL 丢弃）+ `loop/graph.ts` parallel_wave 可选节点（默认串行零变化）
- **交付 4 Durable Execution L1+L2**：`durable/checkpoint-manager.ts`（保留 7 天可配）+ `durable/resume.ts`（daemon 重启续跑）+ `durable/side-effect-ledger.ts`（JSONL 登记簿）+ `durable/idempotency-check.ts`（续跑查重）+ `FORGE/src/audit-middleware.mjs` 执行前登记 + daemon 启动 LOOP 续跑检查

### 波次 4 — Onboard L1 + Benchmark + L4（交付 8/9/14）

- **交付 8 Onboard Agent L1（P0）**：`loop-agent/judge.ts`（crash/error/超时三态，不判语义）+ `loop-agent/driver.ts`（activate→run→judge→fix→re-run，convergeToolError 收敛不中断 + llm-call-trace 定位 + 调试记录带 agentId）+ `engine/mcp/src/tools/loop-debug.ts`
- **交付 9 Benchmark 评测**：`benchmark/benchmark-designer.ts`（题库设计 + Pilot 校准 + Freeze）+ `benchmark/case-evaluator.ts`（隔离 workspace + statement/rubric 物理分离 + 强制 read-only + 四失败码 + 0..100 评分）+ `benchmark/evaluation-log.ts`（HMAC 链，先脱敏再签名）+ `engine/mcp/src/tools/evaluate.ts`
- **交付 14 L4 渐进加载**：`engine/harness/src/knowledge-index.ts` 索引构建 + `buildConstrainedSystemPrompt` 注入格式改「热点全文 2 + 索引 9」（注入量 ~4000→~1500 token）

### 波次 5 — 本体建模要求对齐 + bump + 文档同步（交付 2）

- **交付 2 本体建模要求对齐 GB/T 48000.3-2026**：`engine/audit/src/gb48000.ts` 条款映射清单（8 条：已对齐 5 / 部分对齐 2 / 不适用 1）+ 审计报告「本体建模要求对齐」维度 opt-in（`runRules({gb48000:true})`，ruleClass='工程规范' 按 name 排除 exitCode，默认行为零变化）；条款映射表见 [ARCHITECTURE.md §本体建模要求对齐](../../ARCHITECTURE.md)
- **版本 bump 1.3.0 → 1.3.1**：全仓版本号 + @sofagent/* 依赖对齐 + package-lock 更新 + rules 链接修复（npm install 重链 workspace@1.3.0，case-evaluator 恢复官方 shouldApprove 单一事实源）
- **文档同步**：CHANGELOG / ROADMAP / README / ARCHITECTURE / LIMITATIONS / WIKI / SKILL（工具数 32→38）+ acceptance-test.sh 新增 10 场景

### 波次 6（补漏）— 跨设备审计轨迹聚合（交付 7）

> 主理人波次编排漏排，QA 核实确认缺失后补做。

- **交付 7 跨设备审计轨迹聚合**：`engine/daemon/src/federation/audit-merge.ts`（按 agentId 合并 + HMAC 验签 + trust 优先级裁决——低 trust 设备记录不覆盖高 trust，tampered 丢弃）+ `engine/daemon/src/inspectors/audit-trail.ts`（@daily 巡检，按 agentId 聚合完整轨迹）+ `engine/mcp/src/tools/audit-trail.ts`（MCP `audit_trail` tool——按 agentId 查完整轨迹 / 无 agent_id 列出全部）；注册进 tool-registry + mcp-server + inspectors/index + daemon index（MCP 工具数 38→39）
...
> 新增测试分布：波次 1 ~81 + 波次 2 43 + 波次 3 43 + 波次 4 52 + 波次 5（gb48000 8 + 既有更新）+ 波次 6（audit-merge 7 + inspector 5 + mcp audit-trail 4 = 16）≈ 243+，从 v1.3.0 的 1719 → 1962。
...
- [x] `npm test` 全绿（**1962 tests** across 12 packages）
- [x] `acceptance-test.sh` 177 场景全绿（SSOT 口径，见脚本头部声明）

### 质量循环

- [x] fresh-eyes-loop（五波次开发期间持续审查）
- [x] release-gate-loop verdict=PASS

### 版本号

- [x] SSOT 1.3.1，全仓版本号同步（含 @sofagent/* 依赖对齐）
- [x] 文档头日期统一 2026-08-09
- [x] ROADMAP「现在在哪」更新为 v1.3.1

### 发布

- [x] CHANGELOG 索引新增 v1.3.1 条目
- [x] README / ARCHITECTURE / LIMITATIONS / WIKI / SKILL 文档同步（工具数 32→38→39）
...
v1.3.0 完成运行时审计最小闭环后，v1.3.1 是激活链 EXECUTE 阶段的能力深化——Ontology 从「描述性元数据」升级为「可运行推理底座」（Agent 调用必经 Action 定义），并行编排闭环（DAG 并行 + 审计卡关 + MergeQueue + Durable 续跑）。

### ✨ 新功能

- **🧬 Ontology 运行时层 + CRUD**：Action 注册表 + validator 三态 + Schema 定稿——Agent 调用必经 Ontology 定义，无法绕过；update/delete entity/concept 强制人审 + D1-D5 审计留痕
- **⚡ 并行编排**：LangGraph DAG 波次并行 + 审计节点卡关（全 PASS 合并 / 任一 FAIL 丢弃）
- **⏸️ Durable Execution**：长任务 checkpoint 续跑 + 工具幂等性（崩溃后从中断点继续，不重放副作用）
- **🆔 Agent 身份码 Ed25519**：跨设备验证 + 审计绑定（每次审计记录带签发者身份）
- **🚀 Onboard Agent L1**：FORGE 产品化第一刀——企业 AI 节点跑崩自动检测 + 调试循环
- **📊 Benchmark 评测**：隔离执行 + statement/rubric 物理分离 + HMAC 防篡改评分链
- **🔒 工具审批四模式**：allow-with-audit / deny-all / read-only / always-ask
- **📜 LLM 调用级 Trace**：全量 token/耗时/stopReason 追踪（append-only + HMAC 链）
- **🔄 错误处理升级**：stop_reason 六值分类 + 指数退避（auth 永不重试）+ 错误收敛为消息
- **📋 本体建模要求对齐 GB/T 48000.3-2026**：审计报告 `runRules({gb48000:true})` opt-in 维度（默认行为零变化）

### ⚠️ 破坏性变更

- MCP 工具数 32→39（新增 7 个：update_entity / delete_entity / delete_concept / agent_identity / audit_trail / loop_debug / evaluate）
- `InterceptVerdict` 新增 `permission?: 'r' | 'rw'` 字段（engine/rules/src/types.ts）
- Ontology Schema 定稿（entity/concept/relations JSON Schema）——v1.3.6 注册接口将以此为准
