...
> 交付：14 项 · **1962 tests across 12 packages 全绿** · acceptance 177 场景 · 国标对齐 `--gb48000` opt-in
...
| 2 | P1 | 📋 国标对齐 GB/T 48000.3-2026 | 审计/Ontology 合规参考基线（opt-in） | §二 |
...
- [x] 审计报告 `--gb48000` opt-in 维度生效（ruleClass='工程规范' 不计 exitCode）
- [x] 默认行为零变化（不加 `--gb48000` 时审计输出与 v1.3.0 一致）
...
### 波次 5 — 国标对齐 + bump + 文档同步（交付 2）

- **交付 2 国标对齐 GB/T 48000.3-2026**：`engine/audit/src/gb48000.ts` 条款映射清单（8 条：已对齐 5 / 部分对齐 2 / 不适用 1）+ 审计报告「国标对齐」维度 opt-in（`--gb48000`，ruleClass='工程规范' 不计 exitCode，默认行为零变化）；条款映射表见 [ARCHITECTURE.md §国标对齐](../../ARCHITECTURE.md)
...
- **📋 国标对齐 GB/T 48000.3-2026**：审计报告 `--gb48000` opt-in 维度（默认行为零变化）
