...
**⑤ Ontology Schema 定义（v2.0 接口前移 · 模型层对接前置）**：entity / concept / relations 的数据结构**随本体落地同步定稿**为机器可读 JSON Schema（对齐内核契约 CORE-OBJ / CORE-LNK 的字段约束）。v2.0 的「Ontology 标准 Schema + 注册接口」只做**对外暴露面**（MCP 注入接口），格式定义在本版定稿——避免 v2.0 补 schema 时返工或做兼容层。Schema 即单一事实源：validator 校验、审计对齐、未来模型层注入全部复用同一份定义。
