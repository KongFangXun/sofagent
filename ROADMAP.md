...
> v0.91 · 2026-06-24 · 评审落地 + sofagent-audit MVP + 文档瘦身
>
> **先证明纪律层增量是真的，再做其他任何事。**
...
| **分布式反思同步** | 🔧 | 🟡 | 多个 Agent 的 think.md 反思区定期汇聚到路由器。A 设备踩的坑 → B 设备自动避开。置信度标记设备来源，防止单一设备的偏见污染全局经验。**信号文件夹模式**——每个 Agent 产出标准化信号文件（type + source + confidence + count），路由器聚合为共享信号池。Markdown 格式，和现有文件系统哲学一致。**架构选型**参考 CAP 框架：内网场景用黑板模式（集中向量库 + HNSW 索引）；公网场景用联邦记忆（Gossip 协议 + TTL 防风暴）。冲突解决用信任加权投票（基于 Agent 历史正确率的动态权重矩阵），与 sofagent「不追求完美方案」哲学一致 |
