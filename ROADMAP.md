...
- **审计闭环六步（P1）**：检测→分类→根因→改进→回归→上线——从 OpenAI Tax AI 案例（「AI 建议→会计师修改→结构化归类→根因分析→代码修改→全量回归评测→上线」）。审计从单次判定升级为持续改进闭环
- **FDE 客户画像五类（P1）**：外贸 / 跨境电商 / 中小财务 / 培训 / 律所——从 FDE 研报（「先选择人少、高毛利、流程清晰、老板可拍板的客户」）。FDE Skill 加具体客户画像
- **量化目标基线化（P1）**：收集 v0.93-v0.95 期间的审计数据，确定当前违规率基线，对齐 Karpathy 41%→11% 的量化范式
...
- 治理层上下文成本 ≤ 窗口大小的 5%（设计上限声明）——从 MCP 扩展架构笔记
