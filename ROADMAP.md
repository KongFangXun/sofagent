...
| 交付物 | 类别 | 说明 |
|------|:--:|------|
| **治理加固约束级别标注** | 修复 | engine.md / loop-check.md 每项治理加固前加标注行：`[硬约束·OpenClaw]`（平台级硬拦截）或 `[软约束·全平台]`（prompt 级君子协定）。让技术负责人一眼区分机制保障 vs 提醒 |
| **Release Notes 显著警告** | 修复 | v0.81 的 Release Notes 在显著位置标注「5 项治理加固未经平台验证」，不只是 ROADMAP 角落提一句 |
| **daemon.json 迁移触发条件** | 修复 | daemon-lib.sh 加注释 `# TODO-v0.9: 当 daemon.json 字段 >10 个或出现嵌套时，迁移到 jq 或 python3 -c`，防 grep+sed 方案崩溃 |
| **daemon 最小消费动作** | 修复 | v0.82 给 daemon 一个最小下游消费（检测 think.md 变化 → 写 daemon.log + 下次 Agent 启动注入「反思已更新」提醒），否则文档明确说「v0.81 daemon 是纯骨架，对 Agent 体验零影响」 |

**P1 — 可维护性 / 可信度**

| 交付物 | 类别 | 说明 |
|------|:--:|------|
| **文档去重** | 修复 | 考虑把 ARCHITECTURE §三（已知局限 17 条）独立成 `LIMITATIONS.md`，其他文档只引用不摘抄，减少同步成本 |
| **最小可信验证器** | 修复 | bash 脚本查 task/logs 里有无测试 exit code / lint 结果，有标 `[已验证]`，无标 `[未验证]`。从「纯 LLM 自评」到「有客观证据」的第一步，外部评估器完整版推到 v0.9 |

**P2 — 工程打磨**

| 交付物 | 类别 | 说明 |
|------|:--:|------|
| **CHANGELOG 版本号说明** | 打磨 | 加注「v0.76-v0.80 为 daemon 开发内部版本，未对外发布」 |
| **README 安装路径调整** | 打磨 | `git clone + bash install.sh` 提为推荐路径，`curl pipe bash` 降为备选（企业安全敏感用户友好） |
...
> 💡 这三个方向都不依赖 daemon——也就是说，即使 daemon 方向最终被平台原生能力吃掉，这三个产品化方向依然成立。这是 sofagent 的 B 计划。
