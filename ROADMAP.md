...
> v0.96 · 2026-06-29 · 诚实收缩——README 六段式重构（373→166 行）+ AI 中台叙事贯通 + bash→TS 第一波（僵尸清理 + task-orchestrate）+ 铁律重排 + 审计 A9/A10/A11 规则草案 + 编排引擎定位澄清。详见 [v0.96 开发日志](./docs/changelog/v0.96.md)
>
> **先证明纪律层增量是真的，再做其他任何事。**
...
> v0.96 是 **诚实收缩版本**——不做大功能，做减法。核心改动：README 从 373 行重构为 166 行六段式 + AI 中台纪律底座叙事贯通 + bash→TS 迁移第一波（僵尸清理 + task-orchestrate 迁移）+ 铁律顺序重排 + 审计 A9/A10/A11 规则草案 + 编排引擎定位澄清（从核心拆出，FDE 专用）。两份外部全面审查吸收。详见 [v0.96 开发日志](./docs/changelog/v0.96.md)。

| 级别 | 交付 | 状态 |
|------|------|:----:|
| P0 | README 六段式重构（373→166 行）+ AI 中台叙事贯通 + 3 CI Badge + emoji 清理 | ✅ |
| P0 | 知识简报吸收 6 条（污染理论/90-10框架/规则分级/记忆风险/三元原语/verdict信号） | ✅ |
| P0 | bash→TS 第一波：3 个僵尸脚本删除 + task-orchestrate.sh → TS 迁移 | ✅ |
| P0 | 铁律重排：6 条按重要性重新排序，精炼描述，去掉（底线）（拐杖）标签 | ✅ |
| P0 | 审计 A9/A10/A11 规则草案（不纳注入/不引毒源/不滥资源） | ✅ |
...
| **推** | FDE 经验抽象闭环 → v0.99（需要真实客户） | |
| **砍** | FDE 客户画像五类（v0.95 已有，重复） | |
| **砍** | docs/ralph-loop-and-sofagent.md（ARCHITECTURE 已覆盖） | |

> 📖 [详细开发日志](./docs/changelog/v0.96.md)
...
### v0.9x — 纪律层验证 + 工程基底（v0.96 进行中）

> v0.93-v0.95 已交付。v0.96 是诚实收缩版本（README 重构 + 铁律精简 + bash→TS 第一波 + 审查吸收）。v0.97-v0.99 三步走到 v1.0。

#### v0.97：证据版本（🔴 最重要——纪律层增量生死验证）

- **纪律层证伪实验（P0）**：真实 Skill 加载链 × 反转设计 × 5 任务 × n=5 × 盲评——结论必须发表
- **非 OC 平台加载链实测（P0）**：WorkBuddy 30 次 / Claude Code 20 次 / Codex 20 次 / Hermes 20 次——共计 90 次新会话，统计真实加载率
- **审计 A9/A10 代码实现（P0）**：prompt injection 检测（注入特征 + 任务范围偏离）+ 供应链检测（package.json 新增依赖 → WARN）。A11 资源耗尽从 git diff 检测不到，推迟到 daemon 运行时职责
- **编排引擎精简重构（P1）**：从 sofagent 核心拆出 → FDE Skill 专用。砍四级深度/三档自由度/渐进减薄/信任等级。保留两档拆解 + Agency Agent 模板匹配（引用 AO Compose 自带模板库）+ think.md 反馈回路
- **bash→TS 第二波（P1）**：6 个核心脚本 + daemon 家族迁移
- **非 OC 平台降级方案（P0）**：实测前先写好预案——加载率 <50% 标「实验性支持」/ 50-70% 补缓解措施 / >70% 维持现状。别等坏结果出来才想怎么办
- **最坏情况预案**：如果增量无法复现——诚实发表，不影响审计层路线

> 💡 HANDBOOK 按场景重组推迟到 v0.98——实验结论会影响文档怎么写。
...
- **bus factor 缓解（P1）**：目标 ≥3 个外部 PR 合入，如做不到在 v1.0 诚实声明
