...
| **v1.2.4** | **知识进化** | ① 分层巡检 L1/L2/L3（@daily/@weekly/@monthly 三级 + 读写回路对标）② skillopt 自动触发（失败模式 3 次自动优化）③ 失败清单驱动优化（负面样本为主要燃料）④ conflict-check CLI + 联邦蒸馏 ⑤ Skill 升级策略（若 v1.2.2 未完成）⑥ **eval + ab-test 半成品补全**（golden set 42 条 + CLI + 数据落 data/ + 进化引擎接通，trust-but-verify 从 14.3% 提升至 100%）（详见 [开发日志](./docs/changelog/v1.2/v1.2.4.md)）|
