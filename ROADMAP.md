...
| **LangSmith 可观测性集成（v2.x+ 探索）** | LangSmith 是 LangChain 生态的 Agent 可观测性平台（trace 追踪 / 调试 / 评测 / Token 监控）。sofagent 目前用自建审计日志（git diff 硬证据 + jsonl 状态文件），Dashboard v1.2.x 自带成本曲线和 SubAgent 面板。LangSmith 作为**可选的补充层**——当企业需要更深层的 Agent 行为追踪（如工具调用链路可视化、LLM 推理过程回放）时集成。定位：自建审计是"审计每一次变更"（文件级），LangSmith 是"追踪每一步思考"（推理级），两者互补不替代 |
