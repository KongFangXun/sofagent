...
- [**现在在哪：v0.84**](#现在在哪v084) — 能用的、还不太行的、三个债
- [**迭代历程**](#迭代历程) — v0.1 → v0.84，一路怎么走来的
...
- **效果有数据了，但增量维度测错了**。v0.84 期间用 v0.81-v0.83 代码跑了 5 组 A/B（WorkBuddy 对话 + CLI 一击两轮 + 独立测试者代码重构，v0.84 改动尚未纳入测试）。约束层（拒绝/追问）增量低——被模型安全训练 + 用户加载链 + CLI 绕过三层压缩。纪律层（先读再用/验证再继续/谨慎修改）有 promising 信号——独立测试者报告纪律性 +2、首次通过率 +40%。但有方法论局限（知识传递效应未排除），需反转顺序重跑确认。详见 [v0.84 开发日志](./docs/changelog/v0.84.md#战略启示测错了维度)
...
| 优先级 | 动作 | 截止 | 来源 |
|:------:|------|------|------|
| P0 | ~~A/B benchmark 数据（10 任务 × 带不带 sofagent，含 token / 调用次数 / 循环状态）~~ | ✅ v0.84 | 评审 |
| — | ↳ **5 组数据已回**（v0.81-v0.83 代码实测，v0.84 改动未纳入）：约束层增量低（被三层压缩），纪律层有 promising 信号（独立测试者纪律性 +2） | v0.85 | v0.84 洞察 |
| — | ↳ **下一步**：反转顺序重跑 + benchmark 增加纪律层任务（用 sofagent-test-suite） | v0.85 | v0.84 洞察 |
