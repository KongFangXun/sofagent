...
> - **`npm i sofagent`**: the bare-name umbrella `sofagent` (`engine/umbrella/`, forwarding to the audit / mcp / orchestrator / daemon sub-packages) pulls a **dependency tree of 771 packages** (measured: the `added N packages` figure npm reports after `npm i sofagent`, 2026-09-25), 5 of which carry native modules and install scripts (`node-pty` / `koffi` / `@google/genai` / `protobufjs` / `@deepseek-ai/dsh-subprocess-local`) — new npm versions do not run unreviewed install scripts, so those native builds / postinstalls are **silently skipped**. If you only want the CLI, do not install it.
...
> 🧪 **Engineering credibility** (current): 5408 tests / 13 module packages + 11 plugins (7 DSH + 4 OpenClaw) · 25 audit rules · fresh-eyes independent review continuously running.
