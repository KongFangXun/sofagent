...
<p align="center">
  ...
  <a href="./CHANGELOG.md"><img src="https://img.shields.io/badge/Version-v1.4.6-16B8F3" alt="Version" /></a>
</p>
...
## v1.4.7: Commercial Platform Interface Edition

🔌 All data and action interfaces the commercial platform needs, delivered in one pass — the organization can **see** (capability gaps / performance), collaboration can **turn** (PR lifecycle / workflow-as-object), and data can **flow in** (data_push dual gate). `workflow_gaps` capability-gap query (three classes: missing people / missing capability / upgrade due; convertible to bounty PRs) · `contribution_query` performance export (three-source aggregation: PR weight + decision traces + audit changes; humans and digital workers on the same standard, org-tenant filtering) · `pr_submit` / `pr_review` / `pr_merge` PR lifecycle tools (state machine + two-state confidence, branch→trunk merge linkage) · workflow-as-object CRUD (four tools; owner edits trunk, others open branches; `trigger.schedule` cron triggers) · G7 multi-tenancy v0 (`data/<tenant>/` path foundation + orgId attribution field) · `onboard_prompt` onboarding-prompt generator · workflow burned into USB keys (auto-load after signature verification) · two-layer audit trail (reducer aggregation + PROV-O export) · TrainChannel cloud-training standard interface (hosted clouds integrate per spec; ssh channel built in-engine) · `data_push` standard data-push entry (schema + triage/compliance dual gate) · static-encryption daemon wiring closed (ciphertext-at-rest once the key is ready) · runtime-audit repo-hash isolation (llm-calls / data-sovereignty split per git repo; legacy history readable in place). (Point-in-time figures: MCP 84→95 tools, tests 4107→4249 — see devlog; current scope under [Core Features](#core-features).) Full details in the [devlog](./docs/changelog/v1.4/v1.4.7.md) · earlier versions in [CHANGELOG](./CHANGELOG.md).
...
```bash
curl -fsSL https://raw.githubusercontent.com/KongFangXun/sofagent/refs/tags/v1.4.6/bootstrap.sh -o bootstrap.sh
...
- **Is it production-ready?** Currently a single-machine, single-user design — multiple Agents share one knowledge base / audit history (tenant isolation is on the [ROADMAP](./docs/ROADMAP.md)); task/logs are currently plaintext — at-rest encryption is not yet wired in (scheduled v1.4.7), and even once wired, task/logs fall outside its coverage (the directory sits outside the main audit-history chain, see [LIMITATIONS §三](./docs/LIMITATIONS.md)). Read [SECURITY](./SECURITY.md) · [LIMITATIONS](./docs/LIMITATIONS.md) before enterprise deployment. `config.yml` is non-fail-closed by default; for strict-compliance scenarios use CI fallback + file-permission lock.
