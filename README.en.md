...
sofagent does not build its own Agent — execution is delegated to mature hosts (model + tools + sessions). What it delivers is the **FDE Harness layer**. **FDE Harness = FDE methodology × Harness engineering** — not a bundle of two features, but two stages of one job: **the FDE generates judgment on entry** (where AI belongs, what "done right" means, who signs off) and freezes it into machine-checkable deliverables (workflow.yml + ontology); **the Harness retains that judgment after departure** (executing against the deliverables 24/7, auditing every change, writing back as it evolves). The deliverables are the seam — without FDE, the constraint layer has no criteria to enforce; without the constraint layer, FDE judgment evaporates the moment the engineer leaves. The five constraint-layer capabilities: inject · audit · rollback · distill · evolve — slots into any existing Agent; and it keeps every model (general or bespoke) under control (register / rollout / train / deploy fully audited).
...
```bash
curl -fsSL https://raw.githubusercontent.com/KongFangXun/sofagent/refs/tags/v1.4.5/bootstrap.sh -o bootstrap.sh
