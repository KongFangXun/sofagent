...
| `check/check-paired-records.mjs` | 判决类记录成对门禁（N 态判据：声明 N 态即须落 N 态。登记表 = 脚本内 REGISTRY（判决家族 × N 态 × 生产写入点）；三判定：①缺侧 ②未登记判决写入点（新增判决 kind 必须同批登记配对侧）③非判决豁免集与 DecisionKind 枚举对账；盲区显式登记（RULE_TOGGLE/ESCALATE_REPORT/COST 各带理由+触发条件）；`--self-test` 脚本内双探针故障注入） | pre-push 第 3d 步 / 新增判决类 kind 后 |
