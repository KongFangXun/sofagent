...
# ── 场景 79: Work模板市场 命令验证 ───────────────────────────
scenario 79 "Work模板市场 命令验证（模板市场 + work模板市场 CLI）"
...
# 模板市场/templates 目录非空
if $S79_OK; then
  S79_TMPL=$(ls "$PROJECT_ROOT/模板市场/templates/" 2>/dev/null | wc -l | tr -d ' ')
  echo "  模板市场/templates/: $S79_TMPL 项"
  if [ "$S79_TMPL" -ge 1 ]; then
    :
  else
    fail "模板市场/templates/ 为空"
    S79_OK=false
  fi
fi
