...
# 56: LOOP 独立产品（LOOP/ 目录 + loop-install.sh + work模板市场 隔离）
scenario 56 "LOOP 独立产品（目录结构 + install 脚本 + work模板市场 隔离）"

LOOP_DIR="$PROJECT_ROOT/LOOP"
WORK模板市场_DIR="$PROJECT_ROOT/work模板市场"
LOOP_PROD_OK=true
...
# work模板市场/ 目录（社区模板市场，与 LOOP 内部编排隔离）
...
# EvidenceMode/经验共享/buildConstrainedSystemPrompt/A14/A15/Work模板市场
# ============================================================
...
S79_OK=true
WFHUB_CLI="$PROJECT_ROOT/sofagent/work模板市场/dist/cli.js"

if [ ! -f "$WFHUB_CLI" ]; then
  fail "work模板市场/dist/cli.js 不存在（需 build）"
  S79_OK=false
fi

if $S79_OK; then
  S79_HELP=$(node "$WFHUB_CLI" --help 2>&1 || true)
  echo "  work模板市场 --help: $(echo "$S79_HELP" | head -1)"
  # help 应含 list / deploy 子命令
  S79_SUBS=$(echo "$S79_HELP" | grep -c "list\|deploy" || echo "0")
  if [ "$S79_SUBS" -ge 2 ]; then
    :
  else
    fail "work模板市场 --help 未含 list/deploy 子命令"
    S79_OK=false
  fi
fi

# work模板市场/templates 目录非空
if $S79_OK; then
  S79_TMPL=$(ls "$PROJECT_ROOT/work模板市场/templates/" 2>/dev/null | wc -l | tr -d ' ')
  echo "  work模板市场/templates/: $S79_TMPL 项"
  if [ "$S79_TMPL" -ge 1 ]; then
    :
  else
    fail "work模板市场/templates/ 为空"
    S79_OK=false
  fi
fi
