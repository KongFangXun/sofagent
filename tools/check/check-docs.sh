...
#     v1.4 / v1.5 / v1.9 / v2.0 与 releasing SOP，含当天新写入的内容）——**10 处已全部修净**，
#     含落在已发版 v1.4.5 的那 2 处。口径：**路径修复不算篡改**——冻结节制保护的是历史读数与
#     历史表述，不是一条指向不存在文件的链接；修路径不改一字、不改一数，故冻结区亦得修。
#     ⇒ 故「排除面死链」**应稳定为 0**；**任何非零即为新引入**，须按回归处理。
...
# C. 主 README 声称的规则数（从 "25 条规则" 这种措辞提取；run-02 P1-5 口径勘误：注释示例 21→24）
...
#   时，误将同名二级标题又插了一处，致该档出现同名 H2「## 五、激活链架构（…）」两处
#   （锚串定位：grep -n '^## 五、激活链架构' 命中 2 行；行号随版本漂移，故此处记锚串不记行号）。
#   三门禁（check-docs / check-anchors / check-forms）均未拦截——
