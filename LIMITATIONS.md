...
当前 388 个测试全绿（审计核心；全 workspace 690），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试 388 个（函数级，审计核心；全 workspace 690）、verify.sh 约 44-48 项（动态）
