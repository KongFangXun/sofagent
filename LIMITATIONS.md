...
当前 388 个测试全绿（审计核心；全 workspace 660），但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试 388 个（函数级，审计核心；全 workspace 660）、verify.sh 约 44-48 项（动态）
- **发版前手动覆盖**：acceptance-test.sh 50 场景（CLI 端到端，步骤 2.3）、OpenClaw 验收 5 场景（Agent 端到端，步骤 2.5）
