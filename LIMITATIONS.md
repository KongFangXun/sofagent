...
**现有防御**：冷启动保护（前 5 次只记录不判断）+ LLM 自评权重 ×0.3。但这些只是缓冲层，不是解决方案。根治需要独立验证环（见 ROADMAP v0.92 验证门控 + v1.x 外部评估器）。
...
| 项目 | 当前状态 (v0.90) | 计划 | 风险 |
|------|---------|----------|------|
| think.md / task/logs | 明文存储 | 明文（不加密） | 敏感信息可能被 LLM 无意写入 |
| 加密方案 | 无 | age 加密（v1.0+ 计划） | v0.90 不实现，只声明 |
| 数据保留策略 | rules.md 可配（默认 90 天） | 已有 | cleanup.sh 自动清理过期日志 |

**v0.90 不实现加密，只做诚实声明**——告诉企业用户当前是明文、计划何时加密、在那之前的风险是什么。age 加密推到 v1.0+。
...
本项目核心宣称（越用越聪明、纪律性提升）已有 11 个实测 Case（含第三方，见 [docs/evidence/evidence.md](./docs/evidence/evidence.md)），但全部为一次性测试，缺乏持续使用 ≥1 周的样本和 A/B 对照数据。v0.84 跑了 5 组 A/B（基于 v0.81-v0.83 代码实测）——约束层增量天花板低（1/10、0/16），纪律层有 promising 信号（独立测试者纪律性 +2、首次通过率 +40%）但存在方法论局限（知识传递效应未排除）。v0.85 设计了 45 组对照实验来证实或证伪，v0.92 执行。

---

### 运行时治理 vs 提交时审计

> v0.85 新增。来自评审（GLM-5.2 + DeepSeek V4 Pro）的核心洞察。
...
颜色常量在 7 个脚本中重复定义，日志函数在 6 个脚本中重复，平台探测在 3 个脚本中重复，`sanitize()` 在 2 个脚本中重复。总计 ~450 行重复代码，占 5,254 行的 8.6%。
...
> v0.85 自述。来自 DeepSeek 评审：「宪法、铁律、编排、daemon、断路器、步数闸、渐进减薄、四级编排深度、三级信任等级、权重门禁、反思区、scoring、任务闸、执行闸、验收闸、闭环清单……一个新来的贡献者看到这些会直接关掉页面。」
...
**现状**：loop-check.md 的 failure 分支有「记录失败 + 交还人类」但缺少自动回退逻辑。编排引擎的 think.md 反馈回路可以部分覆盖（跑崩加回来），但这是手动操作，不是自动恢复。
