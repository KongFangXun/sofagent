...
| 2. 规则检查 A1-A17 | ✅ 生产可用 | 19 条规则（A1-A17 + E1-E4）全部有测试覆盖 |
...
当前 417 个测试全绿，但覆盖范围集中在审计规则和核心逻辑（diff-parser、reporter、config-loader、rules/*.ts）。以下模块没有独立测试：
...
- **CI 已覆盖**：单元测试 417 个（函数级）、verify.sh 约 44-48 项（动态）
