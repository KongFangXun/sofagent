...
- **动作授权状态** → 归口 [ROADMAP · 探索方向](./docs/ROADMAP.md#探索方向)的「**事前授权（mandate）补环**」——与该条描述的「缺事前环」是同一件事，其动手条件不变。**这处无法折进既有规则**（24 条的检查对象为变更产物 / 动作形态 / 声明清单三类，**声明 ≠ 授权**，无一条面向**运行时授权过程**），只能随新能力面推进。
