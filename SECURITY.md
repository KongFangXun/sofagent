...
- [三、编排安全](#三编排安全)
- [四、审计与存储安全](#四审计与存储安全)
  - [25 条审计规则完整清单（文档级 SSOT）](#25-条审计规则完整清单文档级-ssot)
- [五、工程安全](#五工程安全)
...
| A24 | 交付物落点 | 工程规范 | 路径前缀匹配与白名单声明面是纯函数，**留代码**；主判定不进判定层 | 刚性 + 留代码 |
...
2. **诚实报告结果，包括坏消息**——按结果收费的模式里最大的道德风险是粉饰结果。sofagent 工程呼应：审计模块 git diff 硬证据（25 条规则零 token 纯静态判定，不靠模型「自评」）+ HMAC 链防篡改（§四审计与存储安全）+ 运行时审计日志按 git 仓库隔离（FORGE 自托管路径与约束层侧 data-sovereignty / llm-calls 均已按 repo-hash 隔离；commit 级 history.jsonl 保持全局存储，见 §四）。
...
| ASI09 人机信任滥用 | Agent 虚假汇报骗取人工批准 | 25 条规则 git diff 硬证据（不采信模型自评）+ A7 不存盲改（无读取记录的修改 FAIL）+ A5/A19 msg 质量 + entry-gate 🔴 高风险强制人工审批 + daemon 文件监控事后对账 | Agent 可伪造 task/logs（§四已知绕过路径）；HITL 依赖人真正审阅 |
...
> 📌 阅读提示：表中「§N」指本 SECURITY.md 对应章节；规则 A1-A24 编号见 §四「25 条审计规则完整清单」；AST 规则见 §四「AST 规则引擎 SSOT」。
>
...
| 预披露 | 修复发布后公开 advisory（报告人同意时致谢；默认匿名） |
