...
判定层的接管不改变规则条数，只改触发方式：规则从「自带判定器」降为「判据声明 + 证据采集器」。**分界线是规则自身的语义类**——`ruleClass` 为业务底线者说的是「是不是」（密钥提交没有「大概」），判定模型只能**加签**不能**代签**；能力拐杖与工程规范说的是「够不够」（越界程度、记录充分性），判定层承担主判定并用概率压误报。「仍需确定性」列的四种取值：**刚性** = 确定性判定为唯一防线，语义层只能把 PASS 升为 ASK、不得把 FAIL 降为放行；**加签** = 确定性判定保留，语义层可追加一级 ASK；**阈值须校准** = 确定性骨架保留、体内常数换成带校准依据的概率阈值；**否** = 判定层承担主判定。
...
| A11 | 不滥资源 | 业务底线 | 确定性 FAIL 不可降级；体内阈值改概率阈值 | 阈值须校准 |
| A18 | 垃圾文件 | 能力拐杖 | Score：文件名语义与仓库约定的偏离度 | 否 |
| A19 | msg 质量 | 工程规范 | 长度/黑名单阈值改带校准依据的概率阈值 | 阈值须校准 |
...
| A17 | 异常批量变更 | 工程规范 | 确定性可配阈值保持，主判定不进判定层 | 刚性 |
| E1 | 不落测试 | 能力拐杖 | Score：测试文件位置的语义合理性 | 否 |
| E2 | TODO 未声明 | 能力拐杖 | Score/Noul：TODO 与任务声明的语义关联度 | 否 |
| E4 | 低注释率 | 能力拐杖 | 体内阈值改概率阈值；注释有效性交判定层 | 阈值须校准 |

**三条纪律**：① 判定层无否决权——任何规则若被列为「刚性」，其确定性 FAIL 不得被语义层的判断覆盖放行；② 概率不是修辞——凡以概率阈值替换硬编码常数者，须先有校准依据（实测该阈值下的准确率）才可写进代码，否则阈值只是把拍脑袋数字换成了另一种拍脑袋；③ 降级为判据者仍受自测约束——判据声明（正负样例 + 拦截理由）随规则全量在位，样例一旦与规则行为不符即 fail-closed 曝红。
