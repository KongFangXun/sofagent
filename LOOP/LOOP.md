...
- **去人化口径（L3）**：行业有主张「去掉人」（L4 Hill-Climbing 去人化）。sofagent 反其道——human-in-the-loop 不是能力缺陷，而是**可靠优先于自主**的差异化卖点：人在 loop 中可尽量简单（高风险才确认，常规受信自动），但**必须存在**。与「约束层永远在线 + 审计硬证据」同源——可靠不靠更聪明的模型，靠「人在关键处 + 机器在每处」。
