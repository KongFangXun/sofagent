...
>
> sofagent 是一底座·三引擎·一能力组成的治理系统——约束底座管行为，编排引擎拆任务，审计引擎盯变更，回溯能力保安全，进化引擎持续优化。FDE 用这个工具包帮企业梳理工作流、构建本体模型、识别节点与量化、装上底座。**FDE 工作用自己产品，给别人部署完让别人也用自己产品。**
>
...
3. **部署 sofagent 到设备**（核心步骤）→ 流程走完后，找一台闲置设备（服务器/旧电脑），`bash sofagent/scripts/install.sh` 把 sofagent 一底座·三引擎·一能力装上去——约束底座 + 编排引擎 + 审计引擎 + 回溯能力 + 进化引擎就绪，上面开始跑你的 workflow AI 节点
