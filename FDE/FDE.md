...
**对外一句话（城市自来水比喻）**：想象一座城市——城边有一条大江（像黄浦江之于上海），江水是好水，但你不敢直接舀着喝。大厂造了这条江（大模型 LLM 是水，Agent 平台是河床），但企业真要用上水，得有人修堤坝、建自来水厂、铺管网、装水龙头、挂水表。sofagent 干的就是这个——**让原水变成企业敢喝的直饮水**。三秒听懂：不是水不好，是没经过处理你不敢用。

用一个核心比喻理解整个格局：大厂在「造河」——大模型 LLM 是**水**（原始、不受控的 AI 能力，像水一样没方向、会泛滥），大厂的 Agent 平台是**河床**（统一入口，引导水形成河）。没有河床，水只是一片汪洋、无法定向使用；而 sofagent **不做河**。

那么 sofagent 在河上做什么？它是**堤坝（Harness 约束层）**——只有堤坝在，河水才不会侵蚀城市。围绕「水怎么流到业务」这一主线，其余组件可顺势理解为：

- **管网 = Workflow**：把模型能力（水）引到具体业务环节；
- **水龙头 / Subagent = 具体用水设备**：只有它在业务里干具体活，水才真正作用上；
- **蓄水池 = AI 知识库**：河跑起来后自动积累的记忆（详见 §9.3）。

**战略一句话**

大厂造河（LLM + Agent 平台），我们做堤坝（约束层）+ 管网（Workflow）+ 水龙头（Subagent）——让企业安全地用自己的 AI 能力流进千家万户（业务）。

**战略判断：模型吞噬一切**

skill / prompt engineering / context engineering / 以 skill 形式做的 harness engineering，本质都是**文字形式的约束**。每次注入到模型 = 每次投喂 = 每次训练——模型会训练得越来越强，**必然吞噬文字形式的约束**。sofagent 对策：把 Skill + Harness 能力封装进 Subagent（代码级，非文字注入）+ 防投喂机制（防止输入素材变成大模型训练材料）。生存位：细分业务 workflow 上对业务最终结果的可约束性——这个不会被模型吞噬。

> **水龙头怎么长出净水能力——不是裁大模型，是选小基座 + QLoRA 精调**：有人会误以为我们要「从 72B 大模型上剪枝」。不是。剪枝 / 蒸馏 / 量化都是大厂造小模型时用的上游技术——你看到的 Qwen2.5-0.5B 就是蒸馏+剪枝+量化后的开源产物。sofagent 做产业链下游最后一环：直接下载已开源的小基座（0.5B-3B），用企业 workflow 数据做 **QLoRA 微调**（4-bit 量化基座 + 低秩适配器，比全参 LoRA 更轻——对 0.5B 这种小模型，QLoRA 的显存占用更低、Mac Mini 上可跑，适配器仅几 MB），教它只干这一个 workflow。详见 ROADMAP「Subagent 内置专精小模型」。
...
perception:
  ...
  push_target: "webhook://feishu/xxx"  # v1.2.x 规划：Webhook 推送目标；当前 v1.1.8 仅本地存储，不对外推送
  reports:
