...
> ⚠️ **如果你只有 5 分钟，记住这个——"一纸一会两周"**：企业 AI 落地失败的首要原因不是技术选错，是**顺序搞反了**——上来就选模型、搭平台、买 Agent，结果发现没人用。正确顺序是：先做管理动作，再选 AI 工具。
>
> **一纸**：写下当前最痛的一个业务场景（例：客户联系率只有 27%）→ **一会**：第二天早会就执行一个最小动作（例：每人汇报 5 个客户跟进情况）→ **两周**：不看 AI、不买工具，只看这个管理动作跑了两周后的数据变化。
>
> 这个方法的来源是一位帮几百家企业跑通 AI 落地的创始人——他发现真正的瓶颈从来不是"AI 不够好"，而是**企业还没搞清楚自己的业务流程，就想让 AI 来接管**。80 人销售团队仅新增早会 5 分钟提问环节（未采购任何 AI 工具），4 周内客户联系率从 27% → 81%。这就是"管理动作先行"——先让人跑通流程，再用 AI 加速。
>
> sofagent 的 FDE 方法论（梳理 workflow → 判定 AI 节点 → 部署 Agent → 离场交付）正是照着这个逻辑设计的——**第一步梳理工作流，本身就是管理动作**。只是我们把这个过程系统化了：Agent 辅助梳理、审计引擎看门、知识库持续积累。下面的四阶段十二步详解，就是"一纸一会两周"的工程化版本。
...
> **Palantir FDE 三要素——Echo-Delta 结构 + 碎石路到高速公路**：Palantir 的 FDE 模型不是单兵作战，而是有明确的组织架构支撑。**Echo 团队**（领域专家，来自客户所在行业）负责识别真正值得解决的问题——他们知道什么坏了、想修好它；**Delta 团队**（工程师，专注执行）在信息不完整、需求持续变化的环境中快速构建方案——优先速度和影响，而非完美设计。两者组成一个"迷你创业团队"嵌入每个客户现场。更重要的是**碎石路到高速公路的反馈循环**：FDE 在客户现场先铺"碎石路"（快速、务实的定制方案，解决眼前问题），核心产品团队观察各客户碎石路的共同模式，提炼为标准功能——"铺装公路"。正因这个循环，每一笔定制都转化为产品能力的积累，而非一次性咨询交付。sofagent 的 FDE 四阶段十二步流程（梳理→判定→部署→离场）正是照着这个模型设计的：FDE 进场铺碎石路，离场时交付标准化的 Agent 约束体系和审计引擎。详见 [fde.academy](https://fde.academy/blog/how-palantir-invented-the-forward-deployed-engineer-model)。
