...
>
> 今天你看到这个文件名还是 "Forward Deployed Engineer"。我们正在把这个词从岗位 title 升级为能力模型：**FDE 不是一款软件，而是一种能力**——它不一定是你的 job title，但应该是你的底层能力。
...
> 🏞️ **一条河的比喻（对外叙事）**：**River 是一条河**——企业统一 AI 入口 / AI 中台，大厂抢着建，不是小公司能做的。**水 = 模型**：原始、不受控的 AI 能力，像水一样没方向、会泛滥。**堤坝 = sofagent 约束层**（约束+编排+审计+回溯+进化）：只有堤坝，河水才不侵蚀城市。（⚠️ 是堤坝不是河床——河床是河的一部分，堤坝是外在守卫，sofagent 是平台的外部层。）**管网 = Workflow**：把水（模型能力）引到要用水的业务环节。**水龙头 / 用水设备 = Subagent**：真正用水的设备，只有它干具体活，水才真正作用上。**城市 / 企业 / 工厂 / 社区 = 业务环节**，真正用水的地方。**市政管网规划 = Ontology**：规划水怎么被用到业务里。**蓄水池 = AI 知识库**：河跑起来后自动积累的记忆。
>
> **战略一句话**：大厂建江+供水（AI 中台 = 江，模型 = 水），我们做堤坝+管网+水龙头（约束层+Workflow+Subagent），让企业安全地用自己的水（AI 能力）流进千家万户（业务）。
>
> **演进**：以前我们的 workflow 汇聚成 river；未来换掉自建 workflow，借用 river+水的能力，让能力到达业务侧。
