...
>
> **最终目标不是培养更多 FDE 工程师，而是让 SMB 与 OPC 的每个人，都具备 FDE 的能力：掌握完整上下文、打破岗位边界、对结果负责。**
...
FDE 交付的本质不只是一个个独立的 AI 节点——而是把这些节点之间的**关联关系**梳理清楚，让企业的 AI 能力（模型）真正流到业务侧。

> 🏞️ **一条河的比喻（对外叙事）**：**大厂造河——大模型 LLM 是水，Agent 平台是河床**，没有河床水只是一片汪洋。**水 = 大模型 LLM**（⚠️ 是 LLM 大语言模型，不是 LM）：原始、不受控的 AI 能力，像水一样没方向、会泛滥。**河床 = 大厂的 Agent 平台**：统一入口，引导水形成河。没有河床，水只是一片汪洋无法定向使用。Agent 是入口不是应用层。**河（River）= 大厂造的整体**（水 + 河床 = 河流）——sofagent 不做河。**堤坝 = Harness 约束层**（sofagent）：只有堤坝，河水才不侵蚀城市。**自来水厂 = 沙箱/安全机制**：让水从"能喝"到"敢喝"。**管网 = Workflow**：把水（模型能力）引到要用水的业务环节。**水龙头 / 用水设备 = Subagent**：在具体业务用水，只有它干具体活，水才真正作用上。**城市 / 企业 / 工厂 / 社区 = 业务环节**，真正用水的地方。**市政管网规划 = Ontology**：规划水怎么被用到业务里。**蓄水池 = AI 知识库**：河跑起来后自动积累的记忆。**水表/水质监测屏 = Dashboard**：让企业看得见 AI 用水情况——sofagent 产品的可见性窗口。
>
> **排序约定**：堤坝 + 自来水厂 + 管网 + 水龙头（自来水厂在堤坝后面）。
>
> **战略一句话**：大厂造河（LLM + Agent 平台），我们做堤坝 + 自来水厂 + 管网 + 水龙头（约束层 + 沙箱安全 + Workflow + Subagent），让企业安全地用自己的 AI 能力流进千家万户（业务）。
>
> **战略判断——模型吞噬一切**：skill / prompt engineering / context engineering / 以 skill 形式做的 harness engineering，本质都是**文字形式的约束**。每次注入到模型 = 每次投喂 = 每次训练——模型会训练得越来越强，**必然吞噬文字形式的约束**。sofagent 对策：把 Skill + Harness 能力封装进 Subagent（代码级，非文字注入）+ 防投喂机制（防止输入素材变成大模型训练材料）。生存位：细分业务 workflow 上对业务最终结果的可约束性——这个不会被模型吞噬。

> **水龙头怎么长出净水能力——不是裁大模型，是选小基座 + LoRA 精调**：有人会误以为我们要「从 72B 大模型上剪枝」。不是。剪枝 / 蒸馏 / 量化都是大厂造小模型时用的上游技术——你看到的 Qwen2.5-0.5B 就是蒸馏+剪枝+量化后的开源产物。sofagent 做产业链下游最后一环：直接下载已开源的小基座（0.5B-3B），用企业 workflow 数据做 **LoRA 微调**（挂一个小插件，不动基座参数），教它只干这一个 workflow。详见 ROADMAP「Subagent 内置专精小模型」。

> **任务价值分流——代码/强推理直接用最好的模型**：本地小模型（中文 **Qwen2.5-0.5B** / 英文 **Llama-3.2-1B**，均 ≤1B）只覆盖**业务 workflow**——省钱、数据不出域、零投喂。但代码生成、复杂推理、多步规划这类"值得用最强智能"的高价值任务，用户明确**直接选用云端最强 LLM**（Claude / GPT / Gemini），不强行本地化。本地小模型管"可窄域替代的业务"，云端最强模型管"高价值智能任务"——两头都吃到。私有部署优先铁律针对业务数据，与高价值智能任务走云端不冲突。详见 ROADMAP「Subagent 内置专精小模型」。

**River = 多个 Workflow（管网）的关联集合。** 如果把每个 Workflow 看作一段管网，River 就是这些管网接入的统一入口——模型能力（水）在管网之间回流、上下文在节点之间传递、最终从统一入口流回用户。

```
FDE 进场 → 梳理 Workflow A、B、C（每段管网）——梳理结果可直接沉淀为企业 Workflow 模板，见 [Workflow Hub](../workflow-hub/)。
         → 定义它们之间的数据回流关系（A 的产出是 B 的输入）
         → 部署 sofagent 底座（约束 + 编排 + 审计 + 回溯 + 进化）——即河上的堤坝
         → 企业得到一个统一入口：@River
              ↓
         员工只需要 @River 提任务
         River 内部按 Workflow（管网）拓扑自动调度
         结果从同一个入口返回
```

| FDE 交付了什么 | 对应的 River 组件 |
|------|------|
| §4-5 梳理的 Workflow 节点 + 关联关系 | River 的管网规划（水怎么送到业务侧） |
| §7-8 部署的 sofagent 底座 | River 的堤坝（约束 + 编排 + 审计 + 回溯 + 进化） |
| Ontology 说明书（9.5） | River 的市政管网规划（实体关系 + 动作矩阵 + 约束拓扑） |
| AI 知识库（9.3） | River 的蓄水池（跑起来后自动积累） |

> Dashboard 的 River 模块（v1.1.0+）展示的就是这张管网流向图——哪些 Workflow 在互联、数据怎么回流、任务怎么从入口分发到各段管网再汇总回来。不是聊天窗口，是河流的流向图。
...
> 🚀 **人走，FDE Agent 不走。** 上面 §10-§12 是「人（FDE 工程师）离场」的检查清单。但 FDE Agent 本身不离场——离场那一刻，它从 deploy 模式自动切到 **sustain 模式**：周度巡检知识库健康度、月度汇总审计趋势、季度回顾 Skill 退化。见文末[持续优化模式（Sustain）](#持续优化模式sustain)。客户看到的不是「FDE 走了系统就没人管」，而是「FDE 一直在，只是换了个工作节奏」。
