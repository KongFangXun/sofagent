...
> **sofagent 是一套开源（MIT）的 FDE（前线部署工程）工具包——FDE 不是软件，而是一种能力。** 我们基于你自选的大厂 Agent 和大模型，用自研的一底座·三引擎·一能力（约束底座 + 编排/审计/进化引擎 + 回溯能力）做问责底座，帮 SMB 与 OPC 梳理 workflow、搭建本体模型、部署专有 Sub Agent。
...
> 一底座·三引擎·一能力（约束底座 + 编排/审计/进化引擎 + 回溯能力）都**服务于 FDE 工具包**——这是本项目的核心点。下文四阶段流程讲的就是如何把这套底座用起来：无论你是帮自己还是帮客户做 FDE，都能照着走完。
