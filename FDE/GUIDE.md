...
> v1.4.2 · 2026-08-28 · 孔放勋 · 面向想系统掌握 FDE 方法论的人。读完你能独立做一次 FDE 诊断，并能判断客户的 AI 成熟度。
...
> 📖 来源：OpenAI FDE 岗位招聘定义（经 hugozhu《FDE 的 20 个问题，不该由人一个一个问》核验引用，2026-08，微信 · hugozhu.site）
...
| 层 | 是什么 | 回答什么问题 | 本文档覆盖 |
|:--:|--------|-------------|-----------|
| **层 1 · 约束层** | 一个层四种能力（约束层：注入·审计·回溯·进化） | "怎么保证每次执行都做对" | 贯穿（审计引擎是 FDE 的问责底座） |
| **层 2 · 生命周期** | 诊断 → 激活 → 编排 → 执行 → 进化 | "企业 AI 从诊断到自运转怎么走" | 本文档 = 第一环（诊断）；激活链见 [5.11](#511-交付之后激活链从交付物到自动运转activateorchestrateexecutesustain-完整交付) |
...
> 💡 来源：FDE 中国社区「20 问」+ hugozhu 评注（2026-08，微信 · hugozhu.site）
...
### 5.8c 行业 overlay 标注（进场时 · v1.4.2 起）
...
>
> ⚠️ **口径声明**：以上是**传统 FDE 行业的验收阈值参考**（以 1650 测试 / 158 场景为例），**非 sofagent 当前验收标准**。sofagent 自有量化体系：**3594 测试 / 24 条规则 / 284 验收场景**（实测见 `tools/check/test-count.sh`；场景数口径 = `acceptance-test.sh` 中 `scenario` 调用行数，SSOT 由 `check-review-system.sh` 对账，**不等于最大 S 编号**——S1–S348 之间有历史空洞号，勿按编号去重计数）。此处仅作行业参考，帮助 FDE 评估自身交付成熟度。
...
### 5.11 交付之后：激活链——从交付物到自动运转（ACTIVATE→ORCHESTRATE→EXECUTE→SUSTAIN 完整交付）
...
- FDE 中国社区「20 问」+ hugozhu 评注——discovery 是组织上下文的采集协议，其宿命是被产品化：「知识一旦被写成清单，下一步必然是被写成代码」；企业 AI 竞争的不是模型，是谁先把「问」变成「查」（sofagent 自证：`fde_interview` 把五要素访谈变成结构化落盘 + profile 自动重算）
