...
> v1.3.5 · 2026-08-16 · 孔放勋 · 面向想系统掌握 FDE 方法论的人。读完你能独立做一次 FDE 诊断，并能判断客户的 AI 成熟度。
...
FDE 的交付物远不止「一次梳理」。梳理（第①层）是入口，后面三层——编排（②）、治理（③）、专属模型（④）——都是这次梳理的延续价值。**FDE 卖的不是一份 workflow 文档，是企业 AI 运行的完整形态**：

| 层 | FDE 交付/牵出 | 客户得到的延续价值 |
|---|--------------|------------------|
| ① 梳理与转换 | 访谈 → workflow graph + ontology（双视图并行产出） | 「我的业务第一次被结构化地看清」 |
| ② 编排与执行 | workflow + ontology → 受约束图（LangGraph 编排层 + DSH/fallback 执行层） | 「业务图变成 AI 真的能跑的节点」 |
| ③ 插件服务 | sofagent plugin：审计·验收·审批·门禁（事件域全域生效） | 「每一步可审计、可验收、危险事有人审」 |
| ④ 底层模型 | 不同节点调用企业专属后训练模型（商业模型层） | 「用我自己的数据训出我自己的模型」 |

> 这句是 FDE 的销售话术升级：「我们从梳理你的业务开始（①），把业务编成能跑的 AI（②），让它的每一步都可信（③），最后用你的数据训出你的模型（④）——四层一次交付，越用越强。」**梳理不是 FDE 的终点，是四层价值链的起点**——这也是 sofagent 区别于「直接在运行时上做垂直产品」的玩家的根因：他们从③开始造，我们从①开始梳理，梳理过的企业插件才装得进去。
...
| 横向要素 | 含义 | FDE 交付对照 |
|---------|------|-------------|
| **仓库 = 树** | 项目 / 部门 / 任务，每个仓库是一棵独立生长的树 | 每个交付的 AI 节点群 = 一片林子 |
