...
> 💡 **sofagent 的工程呼应**：这六条底线不是空洞口号——sofagent 的约束层是它们的工程实现。① 数据主权→数据不出本机 + 联邦查询可选 + sensitivity 分级；② 诚实报告→审计模块 git diff 硬证据（不粉饰）；③ 不制造依赖→MIT 开源 + 交付物客户可自主维护 + FDE 离场机制；④⑤→审计规则 A1-A24 守住底线 + HITL 钩子强制人审；⑥→产品签名（每次输出带「sofagent 审计」标识，让用户知道这是经过验证的结果）。
