...
> v1.5.3 · 2026-09-26（UTC）· ✅ 已发版 · 孔放勋 · 面向想系统掌握 FDE 方法论的人。读完你能独立做一次 FDE 诊断，并能判断客户的 AI 成熟度。
...
### 5.4.1 离线介质交付：U 盘两档（重定位 · 2026-09-26）

> 🔄 **旧「U 盘 = 完整离线 AI 节点」叙事已退役**（[v2.0.0 §七 B 表](../docs/changelog/v2.0/v2.0.0.md)——旧「四位一体」口径作废）；U 盘重定位为 **DSH 节点 + 判定件的交付介质**（「机器 + USB = 无头判定节点」），交付形态按 [v1.8.0 §五](../docs/changelog/v1.8/v1.8.0.md) 分两档：

| 档 | 形态 | 适用 |
|----|------|------|
| **A 安装器档** | 插上即把 DSH 节点 + sofagent + 判定件装进目标机器（走既有 install.sh 面改造，同一套 sha256 校验） | 目标机器可常驻 |
| **B 便携运行时档** | 插上即跑、拔出零残留（复用 usb-key/usb-runtime/usb-signature 骨架：验签 fail-closed + 密钥仅驻内存 + 拔出清空） | 无头设备 / 临时节点 |

**判定件为何挑机器不挑人**：encoder 单次前向（0.3–0.5B 档），纯 CPU 档可跑——旧电脑 + U 盘即成判定节点，是一体机的低成本末端形态。安全模型不变：介质即信任根（AES key 存介质 federation.json），物理持有 = 信任，拔掉零残留。
