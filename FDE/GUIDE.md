...
>
> ⚠️ **口径声明**：以上是**传统 FDE 行业的验收阈值参考**（以 1650 测试 / 158 场景为例），**非 sofagent 当前验收标准**。sofagent 自有量化体系：**3582 测试 / 24 条规则 / 284 验收场景**（实测见 `tools/check/test-count.sh`；场景数口径 = `acceptance-test.sh` 中 `scenario` 调用行数，SSOT 由 `check-review-system.sh` 对账，**不等于最大 S 编号**——S1–S348 之间有历史空洞号，勿按编号去重计数）。此处仅作行业参考，帮助 FDE 评估自身交付成熟度。
