...
> **引擎为生命周期提供能力，生命周期让引擎有活干**——审计引擎在执行阶段每步把关，进化引擎在闭环阶段吃 think.md 回写。能力底座是"怎么保证做对"，生命周期是"让什么跑起来"。FDE 的四阶段流程（第一章到第五章）就是生命周期第一环（诊断）的完整展开。
...
### 3.5 entity / concept / relations 怎么建
...
>
> ⚠️ **口径声明**：以上是传统 FDE（软件交付）的验收阈值，**非 sofagent 当前验收标准**。sofagent 自有量化体系以**测试覆盖率（1317 测试）+ 审计规则通过率（21 条）+ 验收场景（111 场景）**为准。此处仅作行业参考，帮助 FDE 评估自身交付成熟度。
