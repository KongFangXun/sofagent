...
// v1.3.6 新增：从 sofagent-audit --doctor 迁移至 @sofagent/core
// v1.3.6 维护：新增 post-commit hook 存在性检查
// v1.3.6 新增：每项 fail/warn 附修复命令 + --repair 自动修复模式
//
// 检查项：
//   1. 环境检查（Node / git / npm / disk / bash）
//   2. 配置检查（.sofagent/config.yml 是否存在且有效）
//   3. 数据目录结构（v1.3.6：data/ 用户可见数据 + .sofagent/ 引擎内部状态）
