...
// v1.3.6 新增：Node.js 原生 fetch（Node 18+ 内置）调 OpenAI 兼容接口
// v1.3.6 交付 12：isRetryableError 字符串匹配 → stop_reason 六值分类
//   + 指数退避重连（2s→4s→8s→16s→30s，≤5 次）+ auth 永不重试（铁律）
//   + 工具失败收敛为结构化消息（convergeToolError，不 throw）
// v1.3.6 交付 11：调用前后打点写 LLM 调用级 Trace（llm-calls.jsonl），
