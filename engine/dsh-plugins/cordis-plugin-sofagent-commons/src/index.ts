  ...
  apply(ctx: unknown): void {
    ...
    try {
      ...
      if (runner && typeof runner.define === 'function') {
        const res = runner.define({
          name: 'sofagent-commons',
          purpose: '能力公地五环——发布/发现/调用/评价/养护（品牌色 #16B8F3）',
          code: {
            host: [
              'module.exports = {',
              '  async main(ctx, args) {',
              '    return { ok: true, source: "sofagent-commons", message: "能力公地五环——发布/发现/调用/评价/养护" };',
              '  }',
              '};',
            ].join('\n'),
          },
        ...
        console.error('[sofagent-commons] dynamicCordisRunner.define 成功:', JSON.stringify(res));
      }
    } catch (err) {
      console.error('[sofagent-commons] define 失败:', err instanceof Error ? err.message : String(err));
    }
    ...
    try {
      ...
      if (settings?.register) {
        ...
        settings.register('sofagent-commons', s.object({ enabled: s.boolean(), brandColor: s.string() }), { base: { enabled: true, brandColor: '#16B8F3' } });
        console.error('[sofagent-commons] settings.register 成功');
      }
    } catch (err) {
      console.error('[sofagent-commons] settings.register 失败:', err instanceof Error ? err.message : String(err));
    }
  },
};
