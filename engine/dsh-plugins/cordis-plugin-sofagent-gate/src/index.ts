// cordis-plugin-sofagent-gate · DSH 反向插件（v1.4.8：98 行样板收敛到 @sofagent/dsh-plugin-kit）
// seam 挂载：agent/turn-stopping    # 语义：Turn 结束前停止条件判定——验收不过不放行
...
const kit = createSofagentPlugin(
  {
    ...
    seamSemantics: 'Turn 结束前停止条件判定——验收不过不放行',
    capability: '验收硬门禁（define_acceptance 机器可判定）',
    bridgePkg: '@sofagent/audit',
    bridgeApi: 'runRules',
    description: '验收不过不放行——机器可判定验收 + 人审',
  },
