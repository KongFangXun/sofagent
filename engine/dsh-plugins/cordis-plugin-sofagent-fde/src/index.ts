// cordis-plugin-sofagent-fde · DSH 反向插件（v1.4.8：98 行样板收敛到 @sofagent/dsh-plugin-kit）
// seam 挂载：non-seam:tool-set    # 语义：非宿主事件接入（tool 集）——FDE 六 tool 按需调用
...
const kit = createSofagentPlugin(
  {
    ...
    seamSemantics: '非宿主事件接入（tool 集）——FDE 六 tool 按需调用',
    capability: 'FDE 进场方法论（梳理/判定/量化/派生/蒸馏/部署）',
    bridgePkg: '@sofagent/ontology',
    bridgeApi: 'generateOntologyView',
    description: 'FDE 进场方法论桥接——本体数据视图生成（fde_* 六 tool 已交付）',
  },
