// cordis-plugin-sofagent-ontology · DSH 反向插件（v1.4.8：98 行样板收敛到 @sofagent/dsh-plugin-kit）
// seam 挂载：non-seam:tool-set    # 语义：非宿主事件接入（tool 集）——本体查询 / 知识检索按需调用
...
const kit = createSofagentPlugin(
  {
    ...
    seamSemantics: '非宿主事件接入（tool 集）——本体查询 / 知识检索按需调用',
    capability: '本体数据（业务语义底座）',
    bridgePkg: '@sofagent/ontology',
    bridgeApi: 'generateOntologyView',
    description: '共享语义底座 + 知识检索',
  },
