...
本插件随 sofagent 主线版本发布，走**宿主挂载通道**：`skillhub install cordis-plugin-sofagent`。⚠️ **本插件不在 npm 通道**——14 款插件（10 DSH + 4 OpenClaw）均为 `private`，npm 装不到；npm 侧另有**库通道** `npm i -g sofagent`（总包 `engine/umbrella`，聚合 `@sofagent/audit` / `mcp` / `orchestrator` / `daemon` 四个能力包，**不含本插件**）。两条通道按所在环境二选一，互不依赖、互不为前置。版本号与 sofagent 主线对齐。
