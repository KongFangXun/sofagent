...
// v1.4.8 ⑩：外部 Python 依赖摘除 —— 默认走**自研 native gate**（零外部依赖，部署确定性），
//   仅 `SOFAGENT_EVOLVE_GATE=cli` 时才回退到外部 CLI 兼容层。
// v1.4.9 G-11：清理一次**过宽全局替换**留下的污染字面量。
//   污染面（实测）：本文件里 `evolve-gate（v1.4.8 自研）` 同时被写进 ① 探活二进制名
//   ② 文档里的 CLI 名 ③ 状态目录路径 `<project>/.evolve-gate（v1.4.8 自研）/staging/`。
