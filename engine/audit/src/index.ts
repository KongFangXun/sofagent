...
// v1.4.8 · 审计闭环六步（检测+分类+根因+改进+回归+上线）
...
// v1.4.8 交付 2：国标对齐 GB/T 48000.3-2026（条款映射清单 + 覆盖度评估）
...
// v1.4.8 T1/T4: hook 安装核心抽取——core.hooksPath 尊重 + 用户 hook 链式保留
import { resolveHooksDir, installHooks } from './hook-install';
// v1.4.8 T8: sanitizePatterns 编译复用 ReDoS 双层防护（静态检测 + 运行时对抗测试）
...
// v1.4.8 交付三：成本审计维度（cost_query MCP 与外部脚本 import 用）
...
/**
 * v1.4.8 交付十三：beforeAfter 结构化摘要——从 diff 提取变更前/后值。
...
function buildBeforeAfterSummary(diffFiles: DiffFile[]): { before?: string; after?: string } | undefined {
  ...
  // v1.4.8 T2: 落盘前过 sanitizeFreeText（REDACTION_PATTERNS 管道）——
...
interface Args {
  ...
  verifyCommit?: string;
  /** v1.4.8 交付 2：--gb48000 国标对齐维度（opt-in 默认 false，不影响默认审计行为） */
  gb48000: boolean;
  ...
  /** v1.4.8: agent-shield 子命令（AgentShield 五类配置面扫描） */
  agentShieldCommand?: boolean;
  /** corpus 子命令：训练语料导出三件套 */
  corpusCommand?: boolean;
  /** v1.2.9: support-bundle 子命令 */
  supportBundle: boolean;
  /** v1.4.8: 审计 session 产物（默认开启，--no-session 关闭） */
  noSession: boolean;
  /** v1.4.8: --commit-msg 完整 commit message（hook 场景传完整 body 供 A9 扫描） */
  commitMsgArg?: string;
  ...
  /** v1.4.8 #15: --warn-as-error 让 WARN 返回 exit 2（安全优先，CI 阻断） */
  warnAsError: boolean;
  /** v1.4.8 #15: --warn-as-info 让 WARN 返回 exit 0（CI 不阻断，仅信息性） */
  warnAsInfo: boolean;
}


/**
 * 顶层子命令白名单（v1.4.8 补 agent-shield）。
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--diff') {
      ...
      // v1.4.8 交付 2：国标对齐维度（opt-in——信息条目，不影响 exitCode）
      args.gb48000 = true;
    } else if (argv[i] === '--warn-as-error') {
      // v1.4.8 #15: WARN 视为 error（exit 2），CI 阻断。安全优先。
      args.warnAsError = true;
    } else if (argv[i] === '--warn-as-info') {
      // v1.4.8 #15: WARN 视为 info（exit 0），CI 不阻断。仅信息性输出。
      ...
      // v1.4.8 #48: --no-daemon flag——跳过 daemon 注册（init 流程使用）。
 ...
 * v1.4.8 H-01: 补装 pre-commit（.sofagent/ 永不入库主防线——staged 清理在
 * commit 对象生成前生效，规避 macOS git 内存 index 快照时序问题）。
 */
function installHook(): void {
  // v1.4.8 T1: hook 落点改经 resolveHooksDir（core.hooksPath 优先，缺省 .git/hooks）。
...
// 同步加载 snapshot 模块（v1.4.8 从 @sofagent/daemon 迁移到 @sofagent/core，消除循环依赖）
...
async function main(): Promise<void> {
  ...
  // v1.4.8 G-01：--baseline 作为 --reset-baseline 的显式别名——语义都是「计算当前
  ...
  if (rawArgs.includes('--doctor') || wantsBaseline) {
    try {
      ...
      // v1.4.8 (F-23): doctor 仅在 error 时返回非零，warning 时返回 0——
  ...
  // v1.4.8：agent-shield 子命令（AgentShield 五类配置面扫描 CLI）
  ...
  if (config?.rules) {
    ...
    // v1.4.8: 规则清单改从 rules 注册表派生（单一事实源）——新增规则自动纳入，无需再手动同步
  ...
  // 5. 运行规则
  // v1.4.8 T5: 改经 runRulesMonitored——审计模块超时自动降级（full→rules-only→minimal），
  ...
  // v1.4.8 交付三: 成本审计维度（opt-in WARN only——不配 budget 不审计；
  ...
  if (!args.json) {
    ...
    if (costBudget && (costBudget.maxTokensPerRun || costBudget.maxCostPerDay)) {
      try {
        // v1.4.8 G-05: 数据目录解析收编进 data-paths SSOT getDataDir()（与 getHistoryFilePath
  ...
  // v1.4.8: 优先 CLI --webhook/--webhook-url，回退 config.yml audit.webhook.{platform,url}，
  ...
  if (webhookPlatform && webhookUrlFinal) {
    try {
      // v1.2.9 编译自定义脱敏正则
      // v1.4.8 T8: 编译复用 ruleset-loader 的 compileSanitizePattern（含
  ...
  try {
    ...
    const historyEntry: AuditHistoryEntry = {
      ...
      actionGovernance: {
        ...
        timestamp: govTimestamp,
        targetEntity,
        // v1.4.8 交付十三：beforeAfter 结构化摘要回填——从 diff 提取前/后值（截断至 200 字符 + 脱敏，
        ...
        decisionProvenance: {
          who: actor,
          when: govTimestamp,
          // v1.4.8 交付十三：whichDataVersion 契约就位——FDE 知识库版本化未就绪时留空（不报错），
  ...
  // v1.4.8：snapshot helpers 已从 @sofagent/daemon 迁移到 @sofagent/core，循环依赖已消除
  ...
  // v1.4.8 #15: WARN 退出码可配——默认 WARN→exit 1（安全优先）。
...
export function printResults(results: AuditResult, diffFiles: DiffFile[], json: boolean, ci: boolean, silent?: boolean): void {
  ...
  if (ci || silent) {
    ...
    // ★ v1.4.8: 无条件向 stdout 输出一行结论（session 可见性核心）
...
// v1.4.8: 仅作为 CLI 入口时执行 main，避免被测试 import 时触发副作用（如 process.exit）
