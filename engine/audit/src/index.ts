#!/usr/bin/env node
// v1.5.1 阶段七：运行期崩溃兜底——引擎异常时用专属退出码 4（区别于 0=全绿/1=警告/2=违规），
// 使 hook 的「非 0/1/2 ⇒ fail-loud 阻断」分支能识别崩溃，避免 fail-open 静默放行。
// v1.5.1 P1-15：**3 → 4**。原用 3 与 cli-quick 的「非 git 仓库 ⇒ exit 3」撞码（实测两义并存），
...
// v1.5.1 · 审计闭环六步（检测+分类+根因+改进+回归+上线）
...
//   4 = 引擎崩溃（v1.5.1 P1-15 起为专属码；完整引擎自身不使用 3）
...
// v1.5.1 扩展三：并发 Git 纪律守卫（orchestrator/daemon 执行侧消费）
...
// v1.5.1 T1/T4: hook 安装核心抽取——core.hooksPath 尊重 + 用户 hook 链式保留
import { resolveHooksDir, installHooks } from './hook-install';
// v1.5.1 T8: sanitizePatterns 编译复用 ReDoS 双层防护（静态检测 + 运行时对抗测试）
import { compileSanitizePattern } from './ruleset-loader';

// Re-export for external consumers —— doctor 等外部调用方需要通过
// v1.5.1: checkHistoryChainIntegrity 已移除（退役公告见 CHANGELOG 索引）——
...
function buildBeforeAfterSummary(diffFiles: DiffFile[]): { before?: string; after?: string } | undefined {
  ...
  // v1.5.1 T2: 落盘前过 sanitizeFreeText（REDACTION_PATTERNS 管道）——
...
interface Args {
  ...
  /** v1.5.1: agent-shield 子命令（AgentShield 五类配置面扫描） */
  agentShieldCommand?: boolean;
  /** corpus 子命令：训练语料导出三件套 */
  corpusCommand?: boolean;
  /** v1.2.9: support-bundle 子命令 */
  supportBundle: boolean;
  /** v1.5.1: 审计 session 产物（默认开启，--no-session 关闭） */
  noSession: boolean;
  /** v1.5.1: --commit-msg 完整 commit message（hook 场景传完整 body 供 A9 扫描） */
  commitMsgArg?: string;
  ...
  /** v1.5.1 #15: --warn-as-error 让 WARN 返回 exit 2（安全优先，CI 阻断） */
  warnAsError: boolean;
  /** v1.5.1 #15: --warn-as-info 让 WARN 返回 exit 0（CI 不阻断，仅信息性） */
  warnAsInfo: boolean;
}


/**
 * 顶层子命令白名单（v1.5.1 补 agent-shield）。
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 2; i < argv.length; i++) {
    if (argv[i] === '--diff') {
      ...
      // v1.5.1 #15: WARN 视为 error（exit 2），CI 阻断。安全优先。
      args.warnAsError = true;
    } else if (argv[i] === '--warn-as-info') {
      // v1.5.1 #15: WARN 视为 info（exit 0），CI 不阻断。仅信息性输出。
      ...
      // v1.5.1 #48: --no-daemon flag——跳过 daemon 注册（init 流程使用）。
      ...
      // v1.5.1 F2: 断链接线——daemon.sh:172 自 v0.82 起调用本参数，但 CLI 从未
 ...
 * v1.5.1 H-01: 补装 pre-commit（.sofagent/ 永不入库主防线——staged 清理在
 * commit 对象生成前生效，规避 macOS git 内存 index 快照时序问题）。
 */
function installHook(): void {
  // v1.5.1 T1: hook 落点改经 resolveHooksDir（core.hooksPath 优先，缺省 .git/hooks）。
...
// 同步加载 snapshot 模块（v1.5.1 从 @sofagent/daemon 迁移到 @sofagent/core，消除循环依赖）
...
async function main(): Promise<void> {
  ...
  // v1.5.1 G-01：--baseline 作为 --reset-baseline 的显式别名——语义都是「计算当前
  ...
  if (rawArgs.includes('--doctor') || wantsBaseline) {
    try {
      ...
      // v1.5.1 (F-23): doctor 仅在 error 时返回非零，warning 时返回 0——
  ...
  // v1.5.1：agent-shield 子命令（AgentShield 五类配置面扫描 CLI）
  ...
  if (!args.json) {
    ...
    if (costBudget && (costBudget.maxTokensPerRun || costBudget.maxCostPerDay)) {
      try {
        // v1.5.1 G-05: 数据目录解析收编进 data-paths SSOT getDataDir()（与 getHistoryFilePath
  ...
  // v1.5.1: 优先 CLI --webhook/--webhook-url，回退 config.yml audit.webhook.{platform,url}，
  ...
  if (webhookPlatform && webhookUrlFinal) {
    try {
      // v1.2.9 编译自定义脱敏正则
      // v1.5.1 T8: 编译复用 ruleset-loader 的 compileSanitizePattern（含
  ...
  try {
    ...
    // v1.5.1 F-16: 对账键加内容指纹——记**本次提交**的 tree SHA。
    ...
    //
    // v1.5.1 P0-1（B 方案）：hook 场景（--commit-msg 由 hook 传入）下 commit 对象
    ...
    const historyEntry: AuditHistoryEntry = {
      ...
      parentSha,
      // v1.5.1 F-16: 内容指纹（HEAD^{tree}）——post-commit 对账三重键之一
      treeSha,
  ...
  // v1.5.1：snapshot helpers 已从 @sofagent/daemon 迁移到 @sofagent/core，循环依赖已消除
  ...
  // v1.5.1 #15: WARN 退出码可配——默认 WARN→exit 1（安全优先）。
...
export function printResults(results: AuditResult, diffFiles: DiffFile[], json: boolean, ci: boolean, silent?: boolean): void {
  ...
  if (ci || silent) {
    ...
    // ★ v1.5.1: 无条件向 stdout 输出一行结论（session 可见性核心）
...
// v1.5.1: 仅作为 CLI 入口时执行 main，避免被测试 import 时触发副作用（如 process.exit）
