...
// v1.3.8 · 审计闭环六步（检测+分类+根因+改进+回归+上线）
...
// v1.3.8 B22: installHook 路径复用 init 的 gitignore 保障——此前 --install-hook 不写 .gitignore，
...
// v1.3.8: re-export P0 数据主权 + skill 安全审查，供 daemon/mcp/orchestrator/skillopt 消费
...
// v1.3.8 交付⑭：分级降级梯队（韧性设计——workflow never stops）
export {
  DegradationManager,
  getCapability,
  filterRulesForLevel,
 ...
 *
 * v1.3.8 P0-RC3: hook 安装清单与 --init 对齐——installHook() 也安装 post-commit。
...
function installHook(): void {
  ...
  // v1.3.8 B22: 与 --init 路径对齐——装 hook 前确保 .sofagent/ 被 .gitignore 排除，
  ...
  // v1.3.8 P0-RC3: commit-msg 与 post-commit 共用此逻辑，保证两个入口（--init / --install-hook）安装清单一致
  ...
  // v1.3.8 P0-RC3: 补装 post-commit（--no-verify 绕过检测）
...
async function main(): Promise<void> {
  ...
  // v1.3.8：--reset-baseline 自动路由到 doctor（不带 --doctor 也不报未知参数——
  ...
  if (config?.rules) {
    ...
    if (disabledCount > 0) {
      ...
      // v1.3.8 #38: 路径解析与主审计历史统一——此前用 resolveAuditDir()（只感知
  ...
  // v1.3.8: 工作区垃圾残留扫描（WARN 不阻断——附带的巡检能力，防「实验残留」积压）
  if (!args.json) {
  ...
  try {
    ...
    // v1.3.8 B9 修复：git log -1 在 unborn HEAD（首次 commit，hook 运行于 commit 对象生成前）
  ...
  // 8.5 session 产物（P0：审计结果 session 可见性）——v1.3.8
  if (!args.noSession) {
    try {
...
export function printResults(results: AuditResult, diffFiles: DiffFile[], json: boolean, ci: boolean, silent?: boolean): void {
  ...
  if (!results.rules.some((r) => r.number > 11)) {
    // v1.3.8 P2-23: E3 已并入 A11（不滥资源），不再单独列出，避免用户误以为存在 E3 规则
