// ============================================================
// delete-entity.ts · MCP tool：删除 entity 页（v1.3.7 交付 5）
// ============================================================
//
// 破坏性操作——强制人审确认（human-confirm 语义）：
...
/**
 * 删除 entity（v1.3.1 交付 5）——强制人审确认。
 *
 * @param args 删除参数（name + confirmed）
 * @returns 结构化结果（text + data）
 */
export function deleteEntity(args: DeleteEntityArgs): DeleteEntityResult {
  ...
  if (confirmed !== true) {
    return {
      text: `[sofagent] 删除 entity "${name}" 需要人工确认：请显式传 confirmed:true 后才执行（破坏性操作，D1-D5 全程留痕）`,
      data: {
        action: 'delete',
        path: filePath,
        confirmed: false,
  ...
  if (!existsSync(filePath)) {
    return {
      text: `[sofagent] entity "${name}" 不存在，无法删除`,
  ...
  if (!before) {
    return {
      text: `[sofagent] entity "${name}" 内容解析失败，已中止删除（保护性拒绝）`,
      data: { action: 'delete', path: filePath, confirmed: true, executed: false, auditVerdict: 'FAIL', isError: true },
    };
  }

  // 2. 构造变更记录（delete：after = undefined）
  const change = diffDataChange('entity', name, before, undefined);
  ...
  if (auditResult.hasFail) {
    const failSummary = auditResult.violations
      ...
      .join('; ');
    return {
      text: `[sofagent] 数据审计拦截（FAIL）· entity "${name}" 未删除\n  ${failSummary}`,
      data: { action: 'delete', path: filePath, confirmed: true, executed: false, auditVerdict: 'FAIL', isError: true },
    };
  }
  ...
  try {
    generateDataThink([change], auditResult, `delete_entity: ${name}`);
  } catch {
    // 非致命
  }
  ...
  let text = `[sofagent] entity "${name}" 已删除（人工确认）`;
  if (hasWarn) {
