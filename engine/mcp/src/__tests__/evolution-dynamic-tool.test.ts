...
describe('L4 进化工具 MCP 动态面（第七章三）', () => {
  ...
  it('静态计数口径铁律：TOOLS 静态数=93（tool-registry 顶层 name）+ 1 动态（evolution-dynamic-bridge 运行时注册）= 94，L4 注册后静态数不变（动态面独立计数）', () => {
    // 前置锚：静态数=97（v1.4.7 G14 84→88 + G2 89 + 章八 90 + G13 93 + G4 94 + 章十三 95 + v1.4.9 G9 设备注册面 97；tool-registry TOOLS 顶层 name）
    ...
    // 静态面不变；动态面 +1；tools/list 语义 = 97 + 1（v1.4.9 G9 后静态基线 97）
    expect(TOOLS.length).toBe(97);
