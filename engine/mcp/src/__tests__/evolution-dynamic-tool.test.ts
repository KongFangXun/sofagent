...
describe('L4 进化工具 MCP 动态面（第七章三）', () => {
  ...
  it('静态计数口径铁律：TOOLS 静态数=90（tool-registry 顶层 name）+ 1 动态（evolution-dynamic-bridge 运行时注册）= 91，L4 注册后静态数不变（动态面独立计数）', () => {
    // 前置锚：静态数=90（v1.4.7 G14 四 tool 84→88 + G2 88→89 + 章八 onboard_prompt 89→90；tool-registry TOOLS 顶层 name）
    // + 1 动态（evolution-dynamic-bridge 运行时注册）= 91
    ...
    // 静态面不变；动态面 +1；tools/list 语义 = 90 + 1（v1.4.7 G14+G2+章八后静态基线 90）
    expect(TOOLS.length).toBe(90);
