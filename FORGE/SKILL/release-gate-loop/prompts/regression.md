...
- 候选版本：以 driver 注入的「验证对象」为准（禁止自行推测或填写其他版本号）。
  🔴 **precheck 维度输出中的 npm/tag/ssot/包版本号（上一版号）是仓库 SSOT 实测值，属发版时序正常态（SSOT bump 在阶段九步骤五）——它们不是目标版本的「多维度佐证」，引用它们填写目标版本即违反本铁律**。报告版本锚点一律写 driver 注入的候选版本；SSOT 滞后如实记录一行即可，不得列为任何级别的发现
