...
2. **版本口径裁定（run-06 P1-1 定谳规则）**：三份报告中的版本号引用遵循 driver 注入的统一口径——候选版本 = 本次发版目标（target）；仓库 package.json / git tag 的 SSOT 此刻仍指上一版**属发版时序正常状态**（SSOT bump 在 SOP 阶段九步骤五，闸门跑在阶段五），**不构成版本矛盾、不构成阻塞项**。发现各报告版本号不一致时，按「候选版 vs 上一版 SSOT」口径归一理解，如实记录滞后即可，**禁止**将 SSOT 滞后升格为 P1/P0 发现。
