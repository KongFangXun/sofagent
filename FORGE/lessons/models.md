...
| F（修复者） | deepseek-v4-flash | DeepSeek API（按量） | v1.2.8 起 F 修复链 |

> **v1.3.9 起：A/B/V/F 统一切 deepseek-v4-flash**（低成本档，~0 成本）。
...
> **v1.3.9 前**：A（Qwen3.8-max）是 thinking-only 模型（始终思考、无法关闭），B（GLM-5.2）通过参数显式启用 thinking。切换 deepseek-v4-flash 后**不再需要** thinking 参数特殊处理——以下为历史记录，保留供换回 thinking 类模型时参考：
>
