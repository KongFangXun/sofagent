...
> **定位**：fresh-eyes-loop 审查体系的校准历史与工程规则档案——从 `fresh-eyes-review.md` 外移（v1.4.4）。
>
> **读者**：审查工具的维护者（driver / 管道 / 门禁脚本的改者）与报告判读者（consolidate / verify 阶段）。
> **worker 不加载本文件**——审查者的输入是留白式视角 prompt，规则堆进 prompt 会锚定注意力、压缩直觉（v1.4.4 校准：六节笔记 + 三组规则 ≈250 行已超过 16 视角本身，哲学被附录淹没）。
>
> **增长纪律**：新校准往本档案加，`fresh-eyes-review.md` 不再因校准笔记涨行（其警戒线 455 冻结于 v1.4.4 外移时点）。每节标注版本来源。
...
- **静态视角缺动态面**：跨组件契约（包间接口假设）、构建产物三态（源码→dist→安装态）、执行证据（功能存在≠被执行过）三个面在 16 视角中零覆盖——v1.4.4 增设视角十七/十八/十九补位。
