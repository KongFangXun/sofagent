...
> **worker 不加载本文件**——审查者的输入是留白式视角 prompt，规则堆进 prompt 会锚定注意力、压缩直觉（校准笔记 + 规则条目已超过 21 个视角本身，哲学被附录淹没）。
>
> **增长纪律**：新校准往本档案加，`fresh-eyes-review.md` 不再因校准笔记涨行（其警戒线 480 冻结于视角 20/21 收编时点）。每节标注来源。
