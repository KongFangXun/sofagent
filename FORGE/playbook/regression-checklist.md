...
**行数警戒线（当前值）**：`regression-checklist.md` ≤ 1800 行、`acceptance-test.sh` ≤ 4216 行（4130→4216：v1.4.5 第七章二/三 S373/S374 两行为实测场景 + S370 锚点双词化 + S219 阈值校准注释——S265 归并入 S264 对销后净增 +79，含 bbe54dc4 S144 sha256 实测 +12 前账）；fresh-eyes 警戒线见 04-review-system.md 风格守护段。三条上调铁律：① 三判据全否方可调——(i) 新增非旧维度可扩展子项的独立审查面；(ii) 非既有场景/维度的重复覆盖；(iii) 真实防回归价值非归并压缩可消化；② 连续上调禁令——连续两版已调，本版须先真实归并对销方可再调；同版同侧只调一次（同版二次冻结）；③ **上调只记一行**——`旧值→新值（原因一句话）`，历史上调链不在此处累积（完整过程 git 历史可溯）。
...
[ "$WC_ACC" -le 4216 ] && echo "✅ acceptance $WC_ACC (≤4216)" || echo "⚠️ acceptance $WC_ACC 超 4216"
```
## 你的身份
