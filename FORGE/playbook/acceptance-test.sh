...
$S398_OK && pass "v1.4.7 tool 描述四原则：95 tools 描述全非空/≤400 字/零占位词 + inputSchema 全在位" || fail "S398 锚点回潮——见上方 ✗ 行"
...
scenario 407 "v1.4.8 模型与进化族——modelPreference 未注册抛错 + evolve native 默认 + 旧包名清零"
S407_OK=true; S407_OUT=$(PROJECT_ROOT="$PROJECT_ROOT" node -e "const r=process.env.PROJECT_ROOT; const fs=require('fs'); const bad=[]; const mr=require(r+'/engine/orchestrator/dist/model-resolver.js'); if(typeof mr.ModelPreferenceError!=='function')bad.push('缺 ModelPreferenceError'); const ev=fs.readFileSync(r+'/engine/evolve/src/evolve-integration.ts','utf8'); if(!/SOFAGENT_EVOLVE_GATE \?\? .native./.test(ev))bad.push('native 默认缺失'); const pkg=fs.readFileSync(r+'/package.json','utf8'); if(pkg.includes('@sofagent/skillopt'))bad.push('旧包名残留'); process.stdout.write(bad.length?('S407_FAIL:'+bad.join('|')):'ASSERT_OK');" 2>&1) || S407_OUT="S407_FAIL:crash"; echo "$S407_OUT" | grep -q ASSERT_OK || { echo "  ✗ S407: $S407_OUT"; S407_OK=false; }; $S407_OK && pass "v1.4.8 模型与进化族：显式失败语义 + native 默认 + 旧名清零" || fail "S407 锚点回潮——见上方 ✗ 行"
