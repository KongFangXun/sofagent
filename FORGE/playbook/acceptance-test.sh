...
# 场景数：162 个场景（SSOT：所有文档引用此值，由 check-test-count.sh 校验）
#   口径 = scenario 定义行去重数（check-test-count.sh L316 守卫）；最大编号 228 为编号上限，非场景数；S197 归并至 S164；v1.3.0 新增 S225-S228
#   ⚠️ 口径注意（P2-31）：底部输出的「$PASSED 通过」是**断言通过数**（含跳过的场景也计 PASS），
#   与「162 场景」不同——162 是 scenario 定义数，PASSED 可能 >162（条件跳过的场景也 +1）。
...
for f in docs/DEVELOPMENT.md docs/LIMITATIONS.md; do grep -q "162" "$PROJECT_ROOT/$f" || { fail "$f 缺少 acceptance 场景数 162"; S165_OK=false; }; done
$S165_OK && pass "关键数字跨文档一致（${TEST_COUNT:-N/A} / 24 / 162）"
