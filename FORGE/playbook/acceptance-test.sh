...
# SKILL.md 工具清单同步（对账面：训练域 tools 行 + corpus_export 双入口登记——
# v1.4.4 归并：原 S364 corpus_export 双入口对账并入本场景，MCP 注册锚 L3525 已覆盖）
...
# run-06 定谳：① P1-3 误报——README「24 条」正确（A21+E3），探针漏 E 系列已修
# ② P1-6 半误报——四文档 158 已零命中，仅 S165 标题残留（已改）③ P1-7 已修——pass() 透传描述
S360_OK=true
...
# 章九 B 批代码族（d6907083）：#2 cost 透传 / #58 IPv6 剥方括号 / #42 audit.md 脱敏 / #59 退出码白名单；
# A 批文档族代表：#24 退役 API / #3 环境变量披露 / #36 automerge 实态 / #48 幽灵依赖 / #61 幽灵旗标；
# #32+47 守护死亡告警（daemon-health 退出码 + doctor 死亡态）；章一样本聚合（P1-2）；章七 13 包门面（P1-3）
...
# S375 · v1.4.5 阶段四 A 类行为锁：train 五新面行为实测（SOFAGENT_DATA/SOFAGENT_KEY_PATH 隔离子进程，对齐 S368 手法）
scenario 375 "v1.4.5 阶段四：train deliverable 打包+HMAC verify 篡改拒绝 + compliance PII findings+provenance + retention symlink 拒绝保留源 + serve 三 tools 注册面"; S375_OK=true
...
S375EOF
S375_OUT=$(cd "$S375_TMP" && env SOFAGENT_DATA="$S375_TMP/data" SOFAGENT_KEY_PATH="$S375_TMP/test.key" node "$S375_TMP/w.mjs" 2>/dev/null) || S375_OUT="S375_FAIL:crash"
echo "$S375_OUT" | grep -q "ASSERT_OK" || { echo "  ✗ S375: $(echo "$S375_OUT" | head -1)"; S375_OK=false; }
rm -rf "$S375_TMP"
$S375_OK && pass "train 五新面行为锁（deliverable verify 篡改拒绝 + compliance PII+provenance + retention symlink 拒绝保留源 + serve 注册面）" || fail "train 五新面行为回退——见上方 ✗ 行"
