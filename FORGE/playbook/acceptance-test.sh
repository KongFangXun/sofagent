...
for f in docs/DEVELOPMENT.md docs/LIMITATIONS.md docs/ROADMAP.md; do grep -q "$S165_SCEN_COUNT" "$PROJECT_ROOT/$f" || { fail "$f 缺少 acceptance 场景数 ${S165_SCEN_COUNT}（数字漂移）"; S165_OK=false; }; done
...
scenario 287 "v1.3.6 交付⑨：验收 MCP tool 先行版——define/check_acceptance 复用 Benchmark 判定引擎"; S287_OK=true
...
scenario 318 "v1.3.9 交付十：ATTRIBUTION 归因引擎——决策归因落盘 + 三维查询 + byAgent 联结（P2 库级验收）"; S318_OK=true
R318=$(node -e "
...
R320=$(SOFAGENT_REPO="$PROJECT_ROOT" node "$PROJECT_ROOT/FORGE/playbook/federation-e2e.mjs" 2>&1 || true)
...
# ③ 静默跳过 → 可见信号：三类病因均 WARN 并点出「被合并引擎跳过」
...
(async () => {
  ...
  if (/child_process/.test(fs.readFileSync('$PROJECT_ROOT/engine/orchestrator/src/train/train-scheduler.ts', 'utf8'))) bad.push('scheduler 仍直引 child_process');
...
# ─── v1.4.7 商业平台接口批（阶段五 coverage 闭环：G 系列工具面 + PR 域收口 + 云通道接线 + 质量循环修复批——release-gate run-01 判定四批零锚点阻塞，对齐 S281/S343/S371 先例补代表锚点）───
# S384：G 系列新增 MCP 工具注册面 + 返回结构（workflow_gaps / contribution_query / onboard_prompt / data_push）
scenario 384 "v1.4.7 G 系列工具面——五新 tool 注册 + 返回结构行为锁（dist 直调）"
S384_OK=true
S384_OUT=$(PROJECT_ROOT="$PROJECT_ROOT" node -e "
const reg = require(process.env.PROJECT_ROOT + '/engine/mcp/dist/tool-registry.js'); const names = reg.TOOLS.map(t => t.name); const need = ['workflow_gaps', 'contribution_query', 'onboard_prompt', 'data_push']; const bad = []; for (const n of need) if (!names.includes(n)) bad.push('未注册:' + n); if (names.length !== 95) bad.push('registry 总数≠95:' + names.length);
const gaps = require(process.env.PROJECT_ROOT + '/engine/orchestrator/dist/gap-analyzer.js'); const os = require('os'), fs2 = require('fs'); const emptyDir = fs2.mkdtempSync(require('path').join(os.tmpdir(), 's384-gap-')); const r = gaps.analyzeWorkflowGaps(emptyDir); if (!r || !r.data || !Array.isArray(r.data.gaps)) bad.push('gaps 返回形态缺 data.gaps 数组');
else {
const valid = ['missing_agent', 'low_capability', 'needs_upgrade']; if (!r.data.summary || r.data.summary.total !== r.data.gaps.length) bad.push('summary.total 与 gaps 数不一致');
for (const g of r.data.gaps) { if (!valid.includes(g.kind)) bad.push('缺口类型非法:' + g.kind); if (!g.workflow_id || !g.node) bad.push('缺口缺 workflow_id/node'); }
}
fs2.rmSync(emptyDir, { recursive: true, force: true });
process.stdout.write(bad.length === 0 ? 'ASSERT_OK' : 'S384_FAIL:' + bad.join('|'));
" 2>&1) || S384_OUT="S384_FAIL:crash"
echo "$S384_OUT" | grep -q "ASSERT_OK" || { echo "  ✗ S384: $S384_OUT"; S384_OK=false; }
$S384_OK && pass "v1.4.7 G 系列工具面：workflow_gaps/contribution_query/onboard_prompt/data_push 四新 tool 注册 + registry 95 + gap-analyzer 三类判定返回结构" || fail "G 系列工具面回潮——见上方 ✗ 行"
# S385：G13/G14 PR 域收口行为锁（合并门真判定 fail-closed + branch→trunk 写回 + verdict fail-fast + contributors 上界——质量循环修复批次 A）
scenario 385 "v1.4.7 PR 域收口——合并门真判定 + 自审拒绝 + verdict fail-fast + weight 上界（SOFAGENT_DATA 隔离 dist 直调）"
S385_OK=true
S385_OUT=$(SOFAGENT_DATA="$(mktemp -d /tmp/sof-s385-XXXXXX)" PROJECT_ROOT="$PROJECT_ROOT" node -e "
const audit = require(process.env.PROJECT_ROOT + '/engine/audit/dist/public-api.js'); const path = require('path'), fs = require('fs'); const dataDir = path.join(process.env.SOFAGENT_DATA, 'data'); const bad = []; let pr = audit.prSubmit({ pr_id: 'pr-s385a', workflow_id: 'wf1', title: 't', submitter: 'alice', merge_criteria: [{ kind: 'unknown-kind' }] }, dataDir); let rev = audit.prReview({ pr_id: 'pr-s385a', reviewer: 'bob', verdict: 'approve' }, dataDir); let mg = audit.prMerge({ pr_id: 'pr-s385a', actor: 'bob' }, dataDir); if (!mg.data.awaitingHuman || mg.data.status === 'merged') bad.push('未知kind未挂起HITL');
audit.prSubmit({ pr_id: 'pr-s385b', workflow_id: 'wf1', title: 't', submitter: 'alice' }, dataDir); let self = audit.prReview({ pr_id: 'pr-s385b', reviewer: 'alice', verdict: 'approve' }, dataDir); if (!self.data.isError) bad.push('自审未拒绝');
let w = audit.prSubmit({ pr_id: 'pr-s385c', workflow_id: 'wf1', title: 't', submitter: 'alice', contributors: [{ contributor_id: 'x', weight: 2.5 }] }, dataDir); if (!w.data.isError) bad.push('weight 2.5 未拒');
audit.prSubmit({ pr_id: 'pr-s385d', workflow_id: 'wf1', title: 't', submitter: 'alice', merge_criteria: [{ kind: 'confidence-min', detail: 'gte:0.7' }], trigger: { source: 'manual', confidence: 'confirmed' } }, dataDir); audit.prReview({ pr_id: 'pr-s385d', reviewer: 'bob', verdict: 'approve' }, dataDir); let mgd = audit.prMerge({ pr_id: 'pr-s385d', actor: 'bob' }, dataDir); if (mgd.data.status !== 'merged') bad.push('confirmed+gte:0.7 应合并');
process.stdout.write(bad.length === 0 ? 'ASSERT_OK' : 'S385_FAIL:' + bad.join('|'));
" 2>&1) || S385_OUT="S385_FAIL:crash"
...
S386_OK=true
S386_OUT=$(PROJECT_ROOT="$PROJECT_ROOT" node -e "
const fs = require('fs'), path = require('path'); const root = process.env.PROJECT_ROOT; const bad = []; const ct = fs.readFileSync(path.join(root, 'engine/daemon/src/tasks/cloud-train.ts'), 'utf-8'); if (!ct.includes('createSshTrainChannel')) bad.push('cloud-train 缺 createSshTrainChannel');
if (!ct.includes('channelAsExecutor')) bad.push('cloud-train 缺 channelAsExecutor');
if (!ct.includes('chainDualChannelEvent')) bad.push('cloud-train 缺 chainDualChannelEvent');
const ts = fs.readFileSync(path.join(root, 'engine/orchestrator/src/train/train-scheduler.ts'), 'utf-8'); if (!/executor\??:\s*Pick<TrainExecutor/.test(ts)) bad.push('scheduler 缺 executor 注入口');
const un = fs.readFileSync(path.join(root, 'tools/check/check-unwired-exports.sh'), 'utf-8'); if (!un.includes('createSshTrainChannel')) bad.push('监控表缺 createSshTrainChannel');
if (!un.includes('chainDualChannelEvent')) bad.push('监控表缺 chainDualChannelEvent');
process.stdout.write(bad.length === 0 ? 'ASSERT_OK' : 'S386_FAIL:' + bad.join('|'));
" 2>&1) || S386_OUT="S386_FAIL:crash"
...
S387_OK=true
S387_OUT=$(PROJECT_ROOT="$PROJECT_ROOT" node -e "
const fs = require('fs'), path = require('path'); const root = process.env.PROJECT_ROOT; const bad = []; const cli = fs.readFileSync(path.join(root, 'engine/daemon/src/cli.ts'), 'utf-8'); if (!cli.includes('initDataEncryption')) bad.push('daemon cli 缺 initDataEncryption 接线');
const ds = require(path.join(root, 'engine/audit/dist/data-sovereignty.js')); const p1 = ds.resolveSovereigntyLogPath('2026-09-11', '/tmp/sof-s387-home', 'testhash12'); if (!p1.includes('testhash12')) bad.push('路径缺 repo-hash 段: ' + p1);
if (!fs.readFileSync(path.join(root, 'engine/audit/src/data-sovereignty.ts'), 'utf-8').includes('legacy')) bad.push('缺旧路径 fallback');
process.stdout.write(bad.length === 0 ? 'ASSERT_OK' : 'S387_FAIL:' + bad.join('|'));
" 2>&1) || S387_OUT="S387_FAIL:crash"
echo "$S387_OUT" | grep -q "ASSERT_OK" || { echo "  ✗ S387: $S387_OUT"; S387_OK=false; }
$S387_OK && pass "v1.4.7 daemon 接线收口：initDataEncryption 启动路径接线 + resolveSovereigntyLogPath repo-hash 段 + 旧路径 fallback" || fail "daemon 接线收口回潮——见上方 ✗ 行"echo -e "  验收测试结果：${GREEN}$PASSED 通过${NC} / ${RED}$FAILED 失败${NC} / ${YELLOW}$WARNED 跳过${NC} / 共 $((PASSED + FAILED + WARNED))"
