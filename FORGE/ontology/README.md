...
| **项目** | sofagent 开源项目本体——代码仓库、文档体系、发布渠道 | 27 个 workspace（13 模块包 + load-chain + 插件家族，以发版时点现值为准） + Skill 分发 + 文档体系 |
| **工具链** | 支撑开发迭代的自动化工具与脚本 | 统一单模型（六角色 A/B/C/D/V/F 全走 glm-5.3-flash，权威源 `FORGE/models/profile.mjs`——早期「三模型分工」为历史演进） / 开发环境 / 自动化测试 |

三者关系简述：**人**（孔放勋）通过**工具链**（统一单模型六角色 + 自动化脚本）驱动**项目**（sofagent）按照双层循环（内环 FORGE loop / 外环 releasing 十一阶段，见 [FORGE/README](../README.md)）持续迭代。人提供方向与决策，工具链执行编码/审查/测试，项目积累版本与文档。任何一个环节缺失——无人决策则方向失控，无工具链则效率归零，无项目则无迭代对象。

---

## 二、版本迭代闭环（核心 workflow）

> 🔄 **本节为历史演进记录（早期六阶段/三模型分工口径）**——现行权威口径是**双层循环架构**（内环 FORGE loop + 外环 releasing 十一阶段）与**统一单模型**，见 [FORGE/README](../README.md) 与 [quick-start](../quick-start.md)。以下流程图保留作演进对照。
...
> **防膨胀机制**：验证文件膨胀有警戒线口径（现行值以 [tools/check/check-docs.sh](../../tools/check/check-docs.sh) 与 review-system 纪律为准，不在此处写死数字——两处写数必致一处腐坏）。每版本发版做一轮瘦身。
