...
describe('21 视角零删减 + 三层分工', () => {
  it('playbook 视角标题数 = 21（含十七~十九动态面 + 二十/二十一手动层扩展）', () => {
    // 匹配「视角一 [1]」~「视角二十一 [21]」形态的正式视角节标题
    // （排除「文档治理规则（…视角的常驻敏感）」这类非视角小节）
    const perspectiveHeadings = PLAYBOOK_CODE.match(/^### .*视角[一二三四五六七八九十]+ \[\d+\]/gm) || [];
    expect(perspectiveHeadings.length).toBe(21);
