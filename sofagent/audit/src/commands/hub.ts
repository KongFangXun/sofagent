// ============================================================
// commands/hub.ts · Work模板市场 CLI
// v1.0.5 新增：hub deploy / hub list 命令
// ============================================================
...
/**
 * 查找 Work模板市场 目录
 * 优先级：--with-hub submodule > 本地 work模板市场/
 */
function findHubDir(): string | null {
  ...
  // 2. 检查本地 work模板市场/ 目录（开发模式）
  const localPath = join(cwd, 'work模板市场');
  if (existsSync(join(localPath, 'CATALOG.md'))) {
    return localPath;
  }

  // 3. 检查项目根目录下的 work模板市场（相对于 sofagent/audit）
  const projectPath = join(cwd, '..', '..', '..', 'work模板市场');
...
export function listHubTemplates(): string[] {
  ...
  if (!hubDir) {
    console.log('未找到 Work模板市场 模板目录。');
    console.log('');
    console.log('安装 Work模板市场：');
    console.log('  git submodule add https://github.com/KongFangXun/sofagent-work模板市场 .sofagent/workflows/hub');
    console.log('  或在项目根目录创建 work模板市场/ 目录');
    return [];
  }

  const templatesDir = join(hubDir, 'templates');
  if (!existsSync(templatesDir)) {
    console.log('Work模板市场 模板目录为空。');
    return [];
  }
...
/**
 * 部署 Work模板市场 模板到 .sofagent/workflows/
...
export async function hubDeploy(templateName: string, options: HubDeployOptions): Promise<void> {
  ...
  if (!hubDir) {
    ...
    console.error('   git submodule add https://github.com/KongFangXun/sofagent-work模板市场 .sofagent/workflows/hub');
    process.exit(1);
  }
