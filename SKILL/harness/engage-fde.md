...
| "业务流梳理""Agent 节点规划""sofagent 配置" | 弱信号 | 询问是否 FDE 部署 |
...
| 挖掘 | §4-§6 | 逐岗位追问五要素 → 三问判定 → 量化 | 业务流节点图 + 分类清单 + 价值清单（回写画像） |
...
> 没有单独的 .yaml 配置层——节点文档（.md）同时服务人读和编排引擎读，配置信息用表格写在 .md 里。
...
- **衔接点**：§7 产出的 `.md` 是编排引擎的输入
- **反馈点**：节点执行后审计引擎写 think.md，下次编排优化

> §8 起节点全部交编排引擎，engage-fde.md 只负责引导和文档产出。
...
- **弱信号没确认就激活**——用户说"业务流梳理"，Agent 直接启动引导。后果：用户可能只想理清自己的业务流，激活引导反而打扰。
