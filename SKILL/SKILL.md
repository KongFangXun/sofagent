...
| MCP Server | 97 tools 能力面（审计/本体/进化/训练/工作明细/PR 协同/设备注册） | `bash install.sh --platform <平台>` 自动配置，装完即连 |
...
| 运维观测（9） | `health_check` `snapshot_restore`（强制人审）`worklog_query` `cost_query` `device_register` `device_list`（G9 设备注册面，v1.4.9） |
