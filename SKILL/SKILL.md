...
| MCP Server | 103 tools 能力面（审计/本体/进化/训练/工作明细/PR 协同/设备注册/设备数据面/连接器/模板） | `bash install.sh --platform <平台>` 自动配置，装完即连 |
| DSH 插件家族 | 10 款 cordis-plugin（9 款原子 + 1 款聚合整装） | `skillhub install cordis-plugin-sofagent-<名>`（整套用裸名 `cordis-plugin-sofagent`），详见 `AGENTS.md` |
...
| 运维观测（13） | `health_check` `snapshot_restore`（强制人审）`worklog_query` `cost_query` `device_register` `device_list`（G9 设备注册面，v1.4.9）`device_data_query` `device_data_push`（G10/G11 设备数据面，v1.4.9）`connector_register` `connector_list`（G5b 连接器注册面，v1.4.9） |
