...
[ "$S360_NUMS" = "25" ] || S360_OK=false  # index.ts 注册 25 条（17 默认 + 8 扩展）
...
scenario 442 "v1.5.2 第二章 约束导出与证据链外部可验 + 第三章 运行时 should-run 判定链——规则元数据导出面（export-metadata / ruleset_export tool）与 verify-chain --selftest 外部可验 + SHOULD_RUN_ORDER 五问链在唯一 EventBus 构造点注入 gate"
probe_assert s442 "v1.5.2 章二+章三：约束导出与证据链外部可验（selftest 13/0）+ 运行时 should-run 判定链 fail-fast" "约束导出/判定链行为锁回潮——见上方 ✗ 行"
...
scenario 446 "v1.5.2 第一章 MCP audit 数据对外（S446）——audit_query/ruleset_export 注册面（107 tools）+ audit_query 只读行为锁（无写链调用）+ isError 两态与 [sofagent] 前缀（对齐 S430 注册锚先例，release-gate P0-2 闭环）"
probe_assert s446 "v1.5.2 章一 MCP audit 对外面：注册面+只读行为锁+isError 两态" "audit 对外面锚点回潮——见上方 ✗ 行"
