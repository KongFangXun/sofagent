...
scenario 444 "v1.5.2 第七章 事前授权补环 + 第九章 DSH 插件 npm 首发面——mandate 三元素与 gate 注入（wrapToolsWithGate 第 4 参）+ 7 个 cordis-plugin 包仓内就绪（v1.5.2 审查面登记收口）"
probe_assert s444 "v1.5.2 章七+章九：事前授权补环（no-mandate/covered/out-of-scope）+ DSH 插件 npm 首发面仓内就绪 7 包" "授权补环/插件首发面回潮——见上方 ✗ 行"
