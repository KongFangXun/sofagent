...
scenario 434 "v1.5.1 第一章 事件驱动执行触发——上游产出触发下游链（webhook→n1→n1 产出事件→n2）+ 触发链可还原 + 投递留痕 HMAC 可验 + 超时进死信且重放后 REPLAYED 可见"; S434_OK=true
