...
$S165_OK && pass "关键数字跨文档一致（${TEST_COUNT:-N/A} / 25 / ${S165_SCEN_COUNT}）"
...
# ─── v1.2.7–v1.2.9 场景（S198–S224）三批：Session Goals/压缩/Skill 加载/doctor/driver/企业图谱/support-bundle/boot + memory-store/定时任务/ToolOutputBudget/node-executor/F 角色/Checkpoint + 短任务/checkpoint/PM2/激活链/mcp 拆分/BugFix/叙事/cli-quick ───
...
if $S214_OK; then
  ...
  assert_grep "状态接续先行\|断点续跑" "$PROJECT_ROOT/FORGE/SKILL/fresh-eyes-loop/loop.md" || S214_OK=false
assert_grep "check-fresh-eyes-artifacts" "$PROJECT_ROOT/FORGE/SKILL/fresh-eyes-loop/SKILL.md" || S214_OK=false
  # 原子写（tmp→rename）
...
if $S216_OK; then
  ...
  assert_grep "completedWorkers" "$DB" || S216_OK=false
    assert_grep "文件即状态\|round-NN" "$PROJECT_ROOT/FORGE/SKILL/fresh-eyes-loop/loop.md" || S216_OK=false
...
# ─── v1.3.0–v1.3.5 场景（S225–S281）七批：分层巡检/审计 wrapper/HMAC 链/记忆 ACL + 国标/CRUD/审计聚合/L4 + L2-L5/agent-creation/eval-suite/session-isolation + L2 五大机制/联邦/主 agent/Refine/evidence + 能力市场/SkillScan/MARKET（market_* 更名）/自进化/instinct/FDE 运维/DSH 互通 + run-07 coverage 补覆盖 ───
...
node "$PROJECT_ROOT/tools/check/check-fresh-eyes-artifacts.mjs" --self-test >/dev/null 2>&1 || S298_OK=false  # 探针（v1.5.3 载体：产物契约守闸 self-test）
grep -q "resume" "$PROJECT_ROOT/FORGE/src/release-gate-driver.mjs" || S298_OK=false                # 断点续跑
grep -q "断点续跑" "$PROJECT_ROOT/FORGE/SKILL/fresh-eyes-loop/loop.md" || S298_OK=false
...
grep -q "_summary" "$PROJECT_ROOT/FORGE/src/release-gate-driver.mjs" || S301_OK=false       # 全量摘要（v1.5.3 重构后唯一居所 RG driver）
...
grep -q -- "--daemon" "$PROJECT_ROOT/FORGE/src/release-gate-driver.mjs" && grep -q -- "--watch" "$PROJECT_ROOT/FORGE/src/release-gate-driver.mjs" && grep -q -- "--resume" "$PROJECT_ROOT/FORGE/src/release-gate-driver.mjs" || S316_OK=false
...
# ─── v1.4.0：hook stdin 模式闭环验证 + 双设备联邦独立进程模拟（两段合一）───
...
grep -q "已完成轮禁止重开" "$PROJECT_ROOT/FORGE/SKILL/fresh-eyes-loop/loop.md" || S343_OK=false
...
$S360_OK && pass "run-06 误报批闭环（规则数 25 双口径/探针全口径/PASS 证据链/标题去残留）" || fail "run-06 误报批锚点丢失——README 规则数或探针口径回退"
...
for _a in "cost 配置透传:engine/core/src/config-loader.ts" "parsed.hostname.replace:engine/audit/src/webhook.ts" \
  "sanitizeFreeText:engine/audit/src/audit-history.ts" "白名单\|fail-closed:tools/hooks/sofagent-precommit.sh" \
...
S396_OK=true
S396_OUT=$(PROJECT_ROOT="$PROJECT_ROOT" node -e "const fs = require('fs'), path = require('path'); const root = process.env.PROJECT_ROOT; const bad = []; const mcps = fs.readFileSync(path.join(root, 'engine/mcp/src/mcp-server.ts'), 'utf-8').split(String.fromCharCode(10)).length; if (mcps > 470) bad.push('mcp-server 超行'); // v2.0.0 §七 B 表裁定：浏览器四件套退役（5158c869，107→103）——browser-tools 已删，断言改锁「不再回潮」
  if (fs.existsSync(path.join(root, 'engine/mcp/src/tools/browser-tools.ts'))) bad.push('browser-tools 应回潮拦截（已退役）'); const probe = fs.readFileSync(path.join(root, 'playbook/acceptance-node-probes.js'), 'utf-8'); if (!probe.includes('repo-hash') && !probe.includes('find ')) bad.push('S148 探针未动态定位'); process.stdout.write(bad.length === 0 ? 'ASSERT_OK' : 'S396_FAIL:' + bad.join('|'));" 2>&1) || S396_OUT="S396_FAIL:crash"
[[ "$S396_OUT" == *ASSERT_OK* ]] || { echo "  ✗ S396: $S396_OUT"; S396_OK=false; }
$S396_OK && pass "v1.4.7 质量循环修复批：mcp-server ≤470 + browser 退役不回潮 + S148 探针 repo-hash 动态定位" || fail "S396 锚点回潮——见上方 ✗ 行"
...
scenario 440 "v1.5.1 章十 BugFix 批五族代表锚点（S440）+ 章十一 发布链加固代表锚点（S441）——守卫门禁/语义面/安装器/发版态/拍板收口五族 + 门禁清单覆盖对账/长跑凭据/随动面三块，对齐 S343 先例（S441 于 v1.5.2 审查面登记归并入本场景共壳，断言零删减）"
probe_assert s440 "v1.5.1 章十 BugFix 批五族十锚在位" "五族代表锚缺失——见上方 ✗ 行"
probe_assert s441 "v1.5.1 章十一 发布链加固三面锚在位" "章十一锚点缺失——见上方 ✗ 行"
scenario 442 "v1.5.2 第二章 约束导出与证据链外部可验 + 第三章 运行时 should-run 判定链——规则元数据导出面（export-metadata / ruleset_export tool）与 verify-chain --selftest 外部可验 + SHOULD_RUN_ORDER 五问链在唯一 EventBus 构造点注入 gate + 章一 audit 对外面（S446 于 v1.5.3 阶段四并入共壳：audit_query/ruleset_export 注册 107 tools + 只读行为锁 + isError 两态——断言零删减）"
probe_assert s442 "v1.5.2 章二+章三：约束导出与证据链外部可验（selftest 13/0）+ should-run 判定链 fail-fast + 章一 audit 对外面（注册面/只读锁/isError 两态——S446 归并）" "约束导出/判定链/audit 对外面行为锁回潮——见上方 ✗ 行"
scenario 443 "v1.5.2 第四章 审计结论失效语义 + 第五章 网络出口治理面——INVALIDATION append-only 标记 + 三触发钩子 + filterValid 下游剔除 + decideEgress 默认全拒与出口通道 fail-closed"
probe_assert s443 "v1.5.2 章四+章五：审计结论失效语义（标记→命中→剔除）+ 网络出口治理面（默认全拒/命中放行/未声明拒）" "结论失效/出口治理行为锁回潮——见上方 ✗ 行"
scenario 444 "v1.5.2 第七章 事前授权补环 + 第九章 DSH 插件 npm 首发面——mandate 三元素与 gate 注入（wrapToolsWithGate 第 4 参）+ 7 个 cordis-plugin 包仓内可发布就绪（v1.5.2 审查面登记收口）"
probe_assert s444 "v1.5.2 章七+章九：事前授权补环（no-mandate/covered/out-of-scope）+ DSH 插件 npm 首发面仓内可发布就绪 7 包" "授权补环/插件首发面回潮——见上方 ✗ 行"
# ─── v1.5.2 release-gate 20260924-01 闭环批（S445-S448）：对齐 S343/S440/S426/S430 先例；F-4 走 shell 静态对账 ───
scenario 445 "v1.5.2 第八章 BugFix 批五族代表锚点（S445）——安全 fail-open 收口（install.sh @latest 降级 fail-closed / diff-parser 列表阶段 fail-closed）/ 门禁自欺收口（public-api 真基线对账）/ 前端 readErrors 消费 / 工具债 prune-spill + MCP_ROLES_STRICT fail-closed 开关（对齐 S343/S440 先例，release-gate P0-1 闭环）"
probe_assert s445 "v1.5.2 章八 BugFix 批五族六锚在位" "五族代表锚缺失——见上方 ✗ 行"
scenario 447 "v1.5.2 第六章 身份三层叙事 README 双语结构锁（S447）——三因子口径（FDEing/S1M/harness）双语在位与共生（对齐 S426 文档结构锁先例，release-gate P1-3 闭环）"
probe_assert s447 "v1.5.2 章六 README 双语身份三层叙事结构锁" "双语身份叙事漂移——见上方 ✗ 行"
scenario 448 "v1.5.2 第九章 DSH 插件 npm 首发面 dry-run 对账锚点（S448）——npm publish --dry-run 八包逐包通过（kit + 七款 cordis-plugin-sofagent-*）+ pack 清单与 files 白名单一致 + 与 S444 仓内就绪断言共壳对账（changelog「上 registry」承诺层级以可证伪 dry-run 锚承接，正式发布后补安装冒烟场景）"
DRYRUN_ANCHOR=$(grep -c "S448" "$PROJECT_ROOT/playbook/regression-checklist.md" 2>/dev/null || true); DRYRUN_ANCHOR=${DRYRUN_ANCHOR:-0}
if [ "${DRYRUN_ANCHOR:-0}" -ge 1 ] && grep -q "dry-run" "$PROJECT_ROOT/docs/changelog/v1.5/v1.5.2.md" && grep -q "kit" "$PROJECT_ROOT/docs/changelog/v1.5/v1.5.2.md"; then pass "S448 发布面 dry-run 对账锚在位（changelog 承诺层级=可证伪 dry-run + checklist 闭环行）"; else echo "  ✗ S448: 发布面 dry-run 锚缺失"; fail "发布面 dry-run 锚回潮"; fi
# ─── v1.5.3 阶段四 L4 补齐（S449 三模块共场景——先例 S431 多模块共壳）───
scenario 449 "v1.5.3 章二规则自测 schema 通电（rule-loader 坏样例拒载 + RuleLoadError fail-closed）+ 章三 A24 四态（默认空全不检/白名单内不误报/越界 FAIL 附替代路径/段边界 docs-evil 不放行）+ 章四 doctor --refresh 三段（备份→重置→diff，keep=3 可覆盖）——阶段三欠账补齐，归并对销 S446→S442"
probe_assert s449 "v1.5.3 章二/三/四：schema 断言通电 + A24 四态 + refresh 三段（回归锁=checklist #145 维度 + rule-selftest.test.ts 45 例）" "v1.5.3 三模块验收锚回潮——见上方 ✗ 行"
scenario 450 "v1.5.3 模块一 双规则引擎统一（release-gate coverage P0-3 闭环）——模式/类型/常量层同源单一事实源（core shared rule-* 四件）+ 双引擎各自 import 同源（audit rule-a16 / rules tool-secret-leak SECRET_PATTERNS）+ 行为回归锁 rules-unified.test.ts 在位 + ARCHITECTURE 技术债段直指本版无悬空（契约口径=v1.5.3 §一「模式同源即交付」，规则对象合并属 V2.0 单一引擎域）"
UNIFIED_OK=true
for _f in rule-types rule-definitions rule-patterns rule-constants; do [ -f "$PROJECT_ROOT/engine/core/src/shared/$_f.ts" ] || UNIFIED_OK=false; done
grep -q "shared/rule-" "$PROJECT_ROOT/engine/audit/src/rules/rule-a16-unauthorized-change.ts" || UNIFIED_OK=false
grep -q "SECRET_PATTERNS" "$PROJECT_ROOT/engine/rules/src/rules/tool-secret-leak.ts" || UNIFIED_OK=false
[ -f "$PROJECT_ROOT/engine/audit/src/__tests__/rules-unified.test.ts" ] || UNIFIED_OK=false
grep -q "双规则系统重叠（v1.5.3 施工收敛" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || UNIFIED_OK=false
if [ "$UNIFIED_OK" = true ]; then pass "S450 双引擎同源四件 + 双侧 import + 回归锁 + 技术债段指向在位"; else echo "  ✗ S450: core shared 四件/双引擎 import/回归锁/技术债段有一缺失"; fail "双规则引擎统一锚点回潮"; fi
scenario 451 "v1.5.3 模块五 ARCHITECTURE 三域结构锁（release-gate coverage P1-4 闭环）——三域骨架（域一 FDE 场景层万物皆可 FDEing / 域二 S1M 判定层 / 域三 harness 治理层）+ 三域视角段（三因子对位与五域→三域映射指针）+ 术语对照表三层口径行（对齐 S447 文档结构锁先例）"
TRIDOM_OK=true
grep -q "域一 · FDE 场景层" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
grep -q "万物皆可 FDEing 的场景框架" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
grep -q "域二 · S1M 判定层" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
grep -q "域三 · harness 治理层" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
grep -q "五域→三域映射" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
grep -q "FDEing / S1M / Harness" "$PROJECT_ROOT/docs/ARCHITECTURE.md" || TRIDOM_OK=false
if [ "$TRIDOM_OK" = true ]; then pass "S451 三域骨架 + 场景框架首句 + 映射指针 + 术语三层口径在位"; else echo "  ✗ S451: 三域骨架/首句/映射/术语行有一缺失"; fail "ARCHITECTURE 三域结构漂移"; fi
scenario 452 "v1.5.3 模块六 判决类记录成对完整（release-gate coverage P1-5 闭环）——check-paired-records.mjs 门禁实跑绿（声明 N 态即落 N 态 / 缺侧即红 / 盲区即红 / 豁免显式登记 reason+trigger）+ --self-test 自证（行为探针：真跑门禁而非静态锚）"
if node "$PROJECT_ROOT/tools/check/check-paired-records.mjs" >/dev/null 2>&1 && node "$PROJECT_ROOT/tools/check/check-paired-records.mjs" --self-test >/dev/null 2>&1; then pass "S452 成对完整门禁实跑绿 + self-test 自证"; else echo "  ✗ S452: 门禁实跑或 self-test 失败"; fail "判决类记录成对完整性缺失"; fi
scenario 453 "v1.5.3 模块七 AuditScope 审计范围一等公民化（release-gate coverage P1-6 闭环）——scope.ts 显式输入面对象 + 规则唯一 git 输入面（types.ts 第七章锚）+ 规则消费接线（rule-a5/rule-a18/assemble import AuditScope）+ 应用侧四态由 S449 承载（归并声明：A24 越界 FAIL/段边界 docs-evil 不放行即范围语义应用侧验收，断言零删减）"
SCOPE_OK=true
[ -f "$PROJECT_ROOT/engine/audit/src/scope.ts" ] || SCOPE_OK=false
grep -q "审计范围语义一等公民化" "$PROJECT_ROOT/engine/audit/src/rules/types.ts" || SCOPE_OK=false
grep -q "AuditScope" "$PROJECT_ROOT/engine/audit/src/rules/rule-a5-honest-report.ts" || SCOPE_OK=false
grep -q "AuditScope" "$PROJECT_ROOT/engine/audit/src/rules/rule-a18-junk-file.ts" || SCOPE_OK=false
