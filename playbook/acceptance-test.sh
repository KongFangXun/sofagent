...
scenario 422 "v1.4.9 第七章 蒸馏偏好对构造——buildDistillPairs 配对/择优/过滤 + toRecords 衔接（dist 直调纯函数）"; S422_OK=true
...
$S422_OK && pass "v1.4.9 蒸馏偏好对：同 prompt 配对方向正确 + 缺源跳过计数 + 择优 + dpo records 衔接" || fail "蒸馏配对面回潮——见上方 ✗ 行"
