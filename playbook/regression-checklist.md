...
grep -q "103 tools" SKILL/SKILL.md || echo "⚠️ SKILL 工具速查漂移（口径103）" # 演进：52→60→61→66→67（train_submit）→79→80（corpus_export）→84→95（v1.4.7 商业平台接口版 +11）→97（v1.4.9 G9 设备注册面 +2）→99（v1.4.9 G10/G11 设备数据面 +2）→103（v1.4.9 G5b/G1 连接器与模板面 +4），随 SSOT；锚词与 SKILL.md §MCP 工具速查同步升级
