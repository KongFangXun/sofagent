...
> ⚠️ Skill 超 100 个后，全量语义匹配 token 开销显著上升。此时走两阶段：
> 1. 召回——先用索引卡片关键词 + scoring/ 评分做轻量过滤，取前 20 候选
> 2. 重排——LLM 只对候选做精细语义匹配，选最优
> 本质是内部检索系统。当前 Skill < 100，直接匹配足够——这是远期工程预留。
...
> 来源：「Agent 记忆机制设计指南」(2026-06-20)。记忆机制的核心判断：先识别记忆类型，再做分层存储——"记什么比存多少更重要"。

| 记忆层级 | 笔记定义 | sofagent 文件 | 存储策略 |
|------|------|------|------|
| 当前窗口 | 维持眼前对话连续性 | 平台 session 上下文 | 平台管理，sofagent 不干预 |
| 近期摘要 | 压缩最近确认/修改/否定/约束为清单 | `think.md` 反思区（≤2K token） | Markdown 追加；当前局限：只追加不合并，缺冲突检测 |
| 用户档案 | 保存长期稳定信息 | `rules.md` 用户规则 | Markdown `key: value` 格式（雏形结构化）；当前局限：缺写入/更新规则 |
| 历史事件 | 项目经历、决策原因、案例经验 | `task/logs/` + `orchestrator/` | Markdown 日志；当前局限：无检索能力，Agent Read 全文 |

**核心原则**（来自笔记六）：

- **写入规则**：记什么？稳定模式 / 重复出现的错误 / 用户明确确认的偏好。不记什么？单次异常 / 临时环境问题 / LLM 自身的推测和信心判断（这些是笔记三警告的"单轨迹噪声"）
- **更新规则**：新旧信息冲突时——检测→合并或覆盖→重心明确，不是简单追加两行
- **遗忘规则**：`cleanup.sh` 已有定时清理，缺"低价值信息自动压缩"（将不常访问的反思条目压缩为一句话摘要而非直接删除）

**已知局限**：think.md 当前为追加模式，不具备冲突检测和合并能力——3 条互相矛盾的反思条目可以并存，Agent 无法判断哪条更可靠。与笔记三「Skill 自进化仍处于经验记录阶段」是同一架构债的两个侧面。规划版本 v0.9。
