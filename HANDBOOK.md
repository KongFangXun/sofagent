...
| 已知局限 | 核心效果待社区验证；复盘 LLM 自评；明文存储 | [LIMITATIONS.md](./LIMITATIONS.md) |
...
| 理解债 | Loop 交付你没写过的代码越快，理解鸿沟越大 | task/logs 只追加不修改，永远可回溯 |
| 认知投降 | 最舒服的状态是不再有自己观点 | fde.md 随时加规则覆盖；编排可回滚 |
...
> 编排引擎是可选增强（~800 token 附加开销）。个人开发者不需要——只装纪律层就够了。
