...
| **企业部署安全吗？** | v0.90 起 Skill 安全审查（22 条正则 + LLM 语义审查）已就位。数据明文存储，age 加密计划 v1.0+。提交时审计 sofagent-audit 在 v0.92。详见下方「FDE 场景」|
...
| 企业担心 | sofagent 怎么管 | 状态 |
|---------|---------------|:----:|
| Agent 自动拉来的社区 Skill 安全吗？ | Skill 安全审查——22 条正则硬门（恶意命令/密钥泄露/注入攻击）+ LLM 语义审查软门（隐蔽后门/伪装代码） | ✅ v0.90 |
| 敏感数据怎么存？ | 诚实声明：当前明文 + age 加密计划 v1.0+。不藏不骗 | ✅ v0.90 |
| Agent 改的代码能直接进生产吗？ | 提交时审计 sofagent-audit——看 git diff，不依赖 Agent 自觉 | 🔨 v0.92 |

> ⚠️ **诚实定位**：v0.90 的 FDE 能力是"安全审查 + 数据声明"两件基础工作。审计层（v0.92）和多设备协同（v2.x）还在路上。sofagent 当前是 FDE 场景的纪律底座，不是完整的 FDE 解决方案。如果你是 FDE 从业者，v0.90 能帮你守住 Skill 安全和数据合规两道门，但代码审查和团队协同还需要等后续版本。
